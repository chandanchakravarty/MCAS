--- FINAL QUERY POLICY_RISK AND COVERAGES FOR SHEET ALBA_CLUBE_05
--BEGIN TRAN

--- START INSERT INTO RISK LEVEL
--DROP TABLE ALBA_CLUBE_05
--------------------------------------------------------------------------------------------------------------------------------
GO
IF NOT EXISTS(SELECT  COLUMN_NAME FROM INFORMATION_SCHEMA.COLUMNS 
                  WHERE TABLE_NAME = 'ALBA_CLUBE_05' 
                        AND COLUMN_NAME = 'NEW_CODE') 
                  BEGIN
                        ALTER TABLE ALBA_CLUBE_05
 ...
 IF EXISTS( SELECT COLUMN_NAME FROM INFORMATION_SCHEMA.COLUMNS
				...
				BEGIN 				
					UPDATE ALBA_CLUBE_05 SET NEW_CODE=CODE 
...
IF EXISTS(SELECT  COLUMN_NAME FROM INFORMATION_SCHEMA.COLUMNS 
  ...
  GO
-------------------------------------------------------------------------------------------------------------------------------------             
 -- DELETE  FROM POL_PERSONAL_ACCIDENT_INFO WHERE CUSTOMER_ID=3805 AND POLICY_ID=5 AND POLICY_VERSION_ID=1
 --DELETE FROM POL_PRODUCT_COVERAGES WHERE CUSTOMER_ID=3805 AND POLICY_ID=5 AND POLICY_VERSION_ID=1                 
 -------------------------------------------------------------------------------------------------------------------------------------------   
     SELECT APPLICATION_NO,COAPP,SUBLOB,COAPP_CNPJ,INDIVIDUAL_NAME,IS_SPOUSE_OR_CHILD,GENDER,CPF_CNPJ,STATE,
                 DATE_OF_BIRTH,REGIONAL_IDENTIFICATION ,REGIONAL_ID,REGIONAL_ID_ISSUES,POSITION,CITY_OF_BIRTH ,NEW_CODE INTO #TEMP_ALBA_CLUBE_05 FROM ALBA_CLUBE_05             
------------------------------------------------------------------------------------------------------------------------------------                             
DECLARE @PERSONAL_INFO_ID INT,
	@POLICY_ID INT=6,
	@POLICY_VERSION_ID INT=1 ,
	@CUSTOMER_ID INT=3805,
	@COVERAGE_ID_MAX INT,
	@COUNT INT=0,
	@COUNTER INT=1
...
0 AS ORIGINAL_VERSION_ID
 from #TEMP_ALBA_CLUBE_05 with(nolock)
 ...
 PERSONAL_INFO_ID INT ,
  ...
  ( 
   ...
   FROM POL_PERSONAL_ACCIDENT_INFO PPAF WITH(NOLOCK)
                  LEFT OUTER JOIN ALBA_CLUBE_05 H WITH(NOLOCK) ON PPAF.CODE=H.NEW_CODE
