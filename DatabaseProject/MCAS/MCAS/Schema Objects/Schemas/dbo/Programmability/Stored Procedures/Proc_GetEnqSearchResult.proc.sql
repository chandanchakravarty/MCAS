CREATE PROCEDURE [dbo].[Proc_GetEnqSearchResult]            
...
SELECT distinct isnull(u.PolicyId,0) as PolicyId,g.AccidentClaimId,u.PolicyNo,u.ProductId, u.CedantId, u.SubClassId,u.PolicyEffectiveTo,u.PolicyEffectiveFrom,c.ProductCode,c.ProductDisplayName,isnull(co.UserDispName,'') as ClaimOfficer,g.DutyIO,isnull(p.ClassDesc,  
    
        
'') as ClassDescription,isnull(g.ClaimNo,'') as ClaimNo,isnull(g.VehicleNo,'') as VehicleNo,d.CedantCode,isnull(org.OrganizationName,'') as CedantName,isnull(g.DriverName,'') as DriverName,isnull(clm.ClaimantName,'') as TPSurname,isnull(g.IsComplete,0) as
  
    
        
 ClaimStatus,g.AccidentDate,g.IPNo,clm.ClaimDate,clm.VehicleRegnNo,clm.ClaimantStatus,ISNULL(g.IsReported,0) AS IsReported,ISNULL(g.IsReadOnly,0) AS IsReadOnly, ISNULL(g1.ClaimNo,'') AS LinkedClaimNo,clm.ClaimType,clm.ClaimID        
