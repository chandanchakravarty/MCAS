CREATE PROCEDURE [dbo].[Proc_GetPaymentMinutes] 
...
DECLARE @TEMPTABLE TABLE(            
  PaymentId INT ,Payee NVARCHAR(50) ,CPSTotalPaymentDue NUMERIC(18,2),Address1 NVARCHAR(100),Address2 NVARCHAR(100),Address3 NVARCHAR(100)            
 ,PostalCodes NVARCHAR(20),Reference NVARCHAR(50) ,SeverityReferenceNo NVARCHAR(50) ,AssignedToSP VARCHAR(50),GroupNameSP VARCHAR(50)            
 ,AssignedToPD VARCHAR(50) ,GroupNamePD VARCHAR(50) ,InformSafetytoreviewfindings VARCHAR(20) ,PayeeName VARCHAR(50)          
 ,GD NUMERIC(18,2),ME NUMERIC(18,2) ,FME NUMERIC(18,2) ,LME NUMERIC(18,2) ,LEC NUMERIC(18,2),LOEAR NUMERIC(18,2)            
 ,LODE NUMERIC(18,2),[TRAN] NUMERIC(18,2),COR NUMERIC(18,2),LOUUN NUMERIC(18,2),LOE NUMERIC(18,2),LOR NUMERIC(18,2),EX NUMERIC(18,2)            
 ,LOU NUMERIC(18,2),OE NUMERIC(18,2) ,IEF NUMERIC(18,2) ,MR NUMERIC(18,2),PTF NUMERIC(18,2) ,SF NUMERIC(18,2),RSF NUMERIC(18,2)            
 ,LPRF NUMERIC(18,2),OPEF NUMERIC(18,2) ,PLC NUMERIC(18,2),PLD NUMERIC(18,2),OLC NUMERIC(18,2),OLD NUMERIC(18,2),TOTAL NUMERIC(18,2)            
 )              
...
IF((ISNULL(@AccidentClaimId, 0) != 0) AND (ISNULL(@ClaimId, 0) != 0) AND (ISNULL(@PaymentId, 0) != 0))      
 BEGIN      
  ...
  --SELECT @cols = STUFF((SELECT ',' + QUOTENAME(LTRIM(RTRIM(CmpCode)))             
  --FROM CLM_PaymentDetails WHERE PaymentId = @PaymentId  FOR XML PATH(''), TYPE ).value('.', 'NVARCHAR(MAX)'),1,1,'')    
  ...
  SET @query = 'SELECT PaymentId,Payee,CPSTotalPaymentDue,Address,Address1,Address2,PostalCodes,Reference,SeverityReferenceNo,AssignedToSP,GroupNameSP,AssignedToPD,GroupNamePD,InformSafetytoreviewfindings,PayeeName, ' + @cols + '             
  FROM ( SELECT CPD.CmpCode,CPD.TotalPaymentDue,CPS.PaymentId,CPS.Payee,CPS.TotalPaymentDue AS CPSTotalPaymentDue,CPS.Address,CPS.Address1,CPS.Address2,CPS.PostalCodes,CSP.Reference,CLM.SeverityReferenceNo,USR1.UserDispName AS AssignedToSP,GRP1.GroupName 
  
    
AS GroupNameSP,USR.UserDispName AS AssignedToPD,GRP.GroupName AS GroupNamePD,CPS.InformSafetytoreviewfindings,Null AS PayeeName            
