...
CREATED DATETIME : 23 DEC 2011          
PURPOSE    : Get RI CESSION PRECENTAGE  
Review      :    
Review By  Date       
Purpose          
DROP PROCEDURE [dbo].[Proc_GET_POLICY_RICOMMISSION_PRECENTAGE]        
***************************************************/          
  
CREATE PROCEDURE [dbo].[Proc_GET_POLICY_RICOMMISSION_PRECENTAGE]      
(            
 @CUSTOMER_ID  INT,            
 @POLICY_ID   INT,            
 @POLICY_VERSION_ID INT          ,  
 @CESSION_PRECNT DECIMAL(18,4) OUT    
)            
AS            
BEGIN            
...
--SUM(written_prem) as PREMIUM      
SUM(CASE WHEN POL.CO_INSURANCE=14548 THEN CAST(WRITTEN_PREMIUM AS DECIMAL(25,2)) * case when (ISNULL(CO.COINSURANCE_PERCENT,0)) = 0 then 100 else (ISNULL(CO.COINSURANCE_PERCENT,0)) end * 0.01        
   ELSE CAST(WRITTEN_PREMIUM AS DECIMAL(25,2))END) AS PREMIUM      
FROM POL_PRODUCT_COVERAGES (NOLOCK) PPC      
JOIN MNT_COVERAGE COVG WITH(NOLOCK) ON  COVG.COV_ID= PPC.COVERAGE_CODE_ID --AND COVG.REINSURANCE_LOB=10963 -- channged asper Itrack 670      
JOIN POL_CUSTOMER_POLICY_LIST POL WITH(NOLOCK) ON  PPC.CUSTOMER_ID=POL.CUSTOMER_ID AND PPC.POLICY_ID=POL.POLICY_ID      
AND PPC.POLICY_VERSION_ID= POL.POLICY_VERSION_ID       
...
RISKS.RISK_ID,TIV,PREMIUM AS PREMIUM,CONTRACT_DESC,CONTRACT_ID,CONTRACT_NUMBER,REIN_COMPANY_ID,      
CONTRACT_TYPE,COMMISSION,LAYER,LAYER_AMOUNT AS ACT_LAYER_AMOUNT,REIN_CEDED,REIN_CEDED_PERCENTAGE,RETENTION_AMOUNT,RETENTION_PERCENTAGE,      
...
CASE       
 WHEN CONTRACT_TYPE=1 THEN -- IF CONTRACT TYPE IS QUOTA SHARE APPLY %AGE ITRACK 910 discussed with Monica(system supposed to have only one layer per contract with increasing layer number      
  CASE       
 ...
 WHEN CONTRACT_TYPE=2 THEN -- IF CONTRACT TYPE IS SURPLUS APPLY RETENTION AMOUNT      
 CASE       
   WHEN CAST(TIV AS DECIMAL(25,2)) >  CAST(REPLACE(LAYER_AMOUNT,',','') AS DECIMAL(25,2))       
...
cast(LAYER_AMOUNT as numeric(25,2)) as LAYER_AMOUNT,      
      
case when CONTRACT_TYPE=14628 then REIN_CEDED_PERCENTAGE else      
cast(100*LAYER_AMOUNT/TIV as decimal(12,3))      
end      
AS LAYER_PER,      
case when CONTRACT_TYPE=14628 then cast(REIN_CEDED_PERCENTAGE * PREMIUM * 0.01 as decimal(25,3)) else      
cast((cast(100*LAYER_AMOUNT/TIV as decimal(12,4)))*PREMIUM * 0.01 as decimal(25,3))      
end      
AS RI_LAYER_PREM,      
case when CONTRACT_TYPE=14628 then cast(REIN_CEDED_PERCENTAGE * PREMIUM * 0.01 * COMMISSION * 0.01 as decimal(25,3)) else      
...
SELECT @CESSION_PRECNT =  SUM(LAYER_PER)--,CUSTOMER_ID,POLICY_ID,POLICY_VERSION_ID   
from RI_PREMIUM WHERE LAYER_AMOUNT > 0     
