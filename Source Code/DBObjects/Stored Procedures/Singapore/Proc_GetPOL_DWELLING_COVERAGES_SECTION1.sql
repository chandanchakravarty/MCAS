...
------   ------------       -------------------------                                       
*/               
--DROP PROC dbo.Proc_GetPOL_DWELLING_COVERAGES_SECTION1   
--Proc_GetPOL_DWELLING_COVERAGES_SECTION1 28718,114,1,1,'N','S1'                        
                                    
ALTER PROCEDURE dbo.Proc_GetPOL_DWELLING_COVERAGES_SECTION1                                    
(                                    
  @CUSTOMER_ID int,                                    
  @POLICY_ID int,                                    
...
BEGIN  
 ...
 OR    
 ( AVC.COVERAGE_CODE_ID IS NOT NULL  AND C.COVERAGE_TYPE='S1')      
...
BEGIN  
 ...
 (  
  SELECT  LIMIT_ID FROM POL_DWELLING_SECTION_COVERAGES  
  WHERE   
  CUSTOMER_ID= @CUSTOMER_ID   
  AND POLICY_ID=@POLICY_ID   
  AND POLICY_VERSION_ID IN   
...
INNER JOIN MNT_LOOKUP_VALUES WITH(NOLOCK)   ON POLICY_SUBLOB=LOOKUP_UNIQUE_ID                                
INNER JOIN  MNT_COUNTRY_STATE_LIST WITH(NOLOCK)   ON MNT_COUNTRY_STATE_LIST.STATE_ID =22                                
WHERE CUSTOMER_ID= @CUSTOMER_ID AND POLICY_ID=@POLICY_ID AND POLICY_VERSION_ID= @POLICY_VERSION_ID                                  
              
--select * from POL_CUSTOMER_POLICY_LIST where CUSTOMER_ID = 28718 and POLICY_ID = 114                   
  
--Table 3                                                                     
--Coverage/Limits info                
--Get The Information (H0-42) from section2 APP_DWELLING_SECTION_COVERAGES                                     
SELECT COUNT(CUSTOMER_ID) AS HO42  
 FROM POL_DWELLING_SECTION_COVERAGES WHERE   
         COVERAGE_CODE_ID IN(266,267,270,271)   
...
SELECT D.REPLACEMENT_COST AS REP_COST,ISNULL(L.IS_PRIMARY,'U') AS IS_PRIMARY,ISNULL(L.LOCATION_TYPE,0) AS LOC_TYPE,  
(  
...
WHERE AVC.CUSTOMER_ID= @CUSTOMER_ID AND             
 ...
 --select * from MNT_COUNTRY_STATE_LIST where STATE_ID
  
--Get The No. WaterCraft                
  
SELECT COUNT(CUSTOMER_ID) AS COUNT_WATERCRAFTS  
