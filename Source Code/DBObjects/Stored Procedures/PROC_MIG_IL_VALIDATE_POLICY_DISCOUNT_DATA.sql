...
/****** Object:  StoredProcedure [dbo].[PROC_MIG_IL_POLICY_DISCOUNT_VALIDATE_DATA]    Script Date: 09/16/2011 12:13:40 ******/
IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[PROC_MIG_IL_POLICY_DISCOUNT_VALIDATE_DATA]') AND type in (N'P', N'PC'))
DROP PROCEDURE [dbo].[PROC_MIG_IL_POLICY_DISCOUNT_VALIDATE_DATA]
GO
...
CREATE PROCEDURE [dbo].[PROC_MIG_IL_POLICY_CLAUSES_VALIDATE_DATA] 
			   ...
			   BEGIN  
					 ...
					 WHERE  IMPORT_REQUEST_ID = @IMPORT_REQUEST_ID AND   
						   ...
						   EXEC  [PROC_MIG_IL_INSERT_IMPORT_ERROR_DETAILS]                   
							 ...
							 @ERROR_SOURCE_TYPE     = 'DISP'     -- DISCOUNT AT POLICY LEVEL
				END
