/****** Object:  StoredProcedure [dbo].[PROC_MIG_IL_INSERT_POLICY_DISCOUNT_DETAILS]    Script Date: 12/02/2011 16:07:41 ******/
IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[PROC_MIG_IL_INSERT_POLICY_DISCOUNT_DETAILS]') AND type in (N'P', N'PC'))
DROP PROCEDURE [dbo].[PROC_MIG_IL_INSERT_POLICY_DISCOUNT_DETAILS]
GO
/****** Object:  StoredProcedure [dbo].[PROC_MIG_IL_INSERT_POLICY_DISCOUNT_DETAILS]    Script Date: 12/02/2011 16:07:41 ******/
SET ANSI_NULLS ON
GO
...
/*----------------------------------------------------------                                                                
Proc Name             : Dbo.[[PROC_MIG_IL_INSERT_POLICY_DISCOUNT_DETAILS]]                                                            
...
drop Proc [PROC_MIG_IL_INSERT_POLICY_DISCOUNT_DETAILS]    1                                                   
------   ------------       -------------------------*/        
CREATE PROCEDURE [dbo].[PROC_MIG_IL_INSERT_POLICY_DISCOUNT_DETAILS]       
...
DECLARE @IMPORT_DISCOUNT_FILE_TYPE INT = 14963 -- FOR POLICY DISCOUNT    
 ...
 FROM   #TEMP_POLICY         
  ...
  BEGIN      
   ...
   -------------------------------------------------------     
    ...
    ELSE IF NOT EXISTS (SELECT * FROM MNT_DISCOUNT_SURCHARGE WHERE DISCOUNT_ID=@TYPE AND [LEVEL]=14706 AND LOB_ID=@LOB_ID)
      SET @ERROR_NO =104 
    ELSE IF EXISTS (SELECT * FROM POL_DISCOUNT_SURCHARGE nolock WHERE CUSTOMER_ID=@CUSTOMER_ID and POLICY_ID=@POLICY_ID and POLICY_VERSION_ID=@POLICY_VERSION_ID and DISCOUNT_ID=@TYPE )
      SET @ERROR_NO =133 
  ...
  -----------------------------------------------------------     
    ...
    EXEC  [PROC_MIG_IL_INSERT_IMPORT_ERROR_DETAILS]                     
     ...
     @ERROR_SOURCE_TYPE     = 'PDSC'       
       ...
       ELSE    
       BEGIN    
    IF(@PROCESS_TYPE =1)--FOR NBS -ADDED BY PRADEEP
    BEGIN  
   ...
   WHERE  POLICY_SEQUENTIAL   = @LOOP_POLICY_SEQUANCE_NO   AND    
    ENDORSEMENT_SEQUENTIAL    = @LOOP_END_SEQUANCE_NO      AND    
          FILE_TYPE           = @IMPORT_DISCOUNT_FILE_TYPE AND    
          [FILE_NAME]         = @IMPORT_FILE_NAME          AND    
          IS_ACTIVE			  = 'Y'    
              
            
    -- IF POLICY DISOUNT  IS EXISTS THEN UPDATE EXISTING POLICY DISCOUNT DETAILS
   IF(@DISOUNT_ROW_ID>0)
   BEGIN
		------------------------------------          
       ...
       SET	    
	     ...
	     (
		    ...
		    FROM   MIG_IL_POLICY_DISCOUNTS_DETAILS 
		    WHERE  IMPORT_REQUEST_ID = @IMPORT_REQUEST_ID AND 
		           IMPORT_SERIAL_NO  = @IMPORT_SERIAL_NO
			   
	     )T ON T.CUSTOMER_ID= PD.CUSTOMER_ID AND T.POLICY_ID=PD.POLICY_ID AND T.POLICY_VERSION_ID=PD.POLICY_VERSION_ID
	     
	     WHERE PD.CUSTOMER_ID =@CUSTOMER_ID AND PD.POLICY_ID=@POLICY_ID AND PD.POLICY_VERSION_ID=@POLICY_VERSION_ID AND PD.DISCOUNT_ROW_ID=@DISOUNT_ROW_ID   
     
   END
   	-----------------------------------------------------------------------------      
    -- IF POLICY DISCOUNT IS NOT EXISTS THEN ADD NEW POLICY DISCOUNT IN POLICY ENDORSEMENT
    -----------------------------------------------------------------------------      
   ELSE
   BEGIN
		  -----------------------------------------------------------          
   ...
   ------------------------------------             
    ...
    @IS_ACTIVE       = 'Y',  
    @IS_PROCESSED      = 'Y',  
    @FILE_TYPE       = @IMPORT_DISCOUNT_FILE_TYPE,  
    ...
    @IMPORT_SEQUENTIAL      = @LOOP_DISCOUNT_SEQUANCE_NO,  
