...
/****** Object:  StoredProcedure [dbo].[PROC_MIG_IL_VALIDATE_POLICY_REINSURENCE_DATA]    Script Date: 09/19/2011 12:06:12 ******/
...
DECLARE @REIN_COMPANY_CODE VARCHAR(10)
  ...
  ------------------------------------        
   ...
   ------------------------------------     
    ...
    FROM   #TEMP_POLICY   
	 ...
	 SELECT		
				@REIN_COMPANY_CODE					=	REIN_COMPANY_NAME,
	...
	AND			IS_PROCESSED		=	'N'
			  ...
			  IF NOT EXISTS( SELECT 1 FROM MNT_REIN_COMAPANY_LIST (NOLOCK) WHERE REIN_COMAPANY_CODE=@REIN_COMPANY_CODE AND IS_ACTIVE='Y')
			   SET @ERROR_NO=121						--WRONG REINSURENCE COMPANY CODE
			  ELSE
			  BEGIN
				
			  ------------------------------------  VALIDATION OF CONTRACT FACULTATIVE
			  IF (@CONTRACT_FACULTATIVE not in (14627,14628))   -- 14627 Contract, 14628 Facultative
				SET @ERROR_NO=122						-- WRONG CONTRACT FACULTATIVE
