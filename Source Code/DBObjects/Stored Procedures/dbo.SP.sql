...
DROP PROC [PROC_SUSEP_PPNG_1663]
GO

-- =============================================          
-- Author:  <ANKIT KUMAR GOEL>          
...
-- =============================================          
CREATE PROCEDURE [dbo].[PROC_SUSEP_PPNG_1663]   
...
BEGIN                
 ...
 SET PF = CASE WHEN  DATEDIFF(DD, SECONDARY_EXPIRY_DATE, SECONDARY_EFFECTIVE_DATE) <> 0 THEN 
				1 -  CAST((DATEDIFF(DD,  SECONDARY_EFFECTIVE_DATE,@DATETIME)) AS DECIMAL) / CAST((DATEDIFF(DD,  SECONDARY_EFFECTIVE_DATE,SECONDARY_EXPIRY_DATE)) AS DECIMAL)
 ...
 RIGHT('000000' +CONVERT(VARCHAR,SEQUENCE_NO),6) AS [ESRSEQ]                
  ...
  ,CASE WHEN ORIGINAL_RISK_PREMIUM>=0.00                
   THEN                
    REPLACE(RIGHT('0000000000'+CONVERT(VARCHAR(16),ORIGINAL_RISK_PREMIUM),13),'.',',')                
    ELSE                
  ...
  ,CASE WHEN SECONDARY_RISK_PREMIUM>=0.00                
   THEN                
     REPLACE(RIGHT('0000000000'+CONVERT(VARCHAR(16),SECONDARY_RISK_PREMIUM),13),'.',',')                
    ELSE                
  ...
  ,POLICY_NUMBER AS APONUM 
  ,ISNULL(ENDORSEMENT_NUMBER,'') AS ENDNUM
  ,ISNULL(UNEARNED_PREMIUM,0.00)  AS PPNG
  ,ABS(ISNULL(COMMISSION_AMOUNT,0.00)) AS COMVALOR
  ,ABS(ISNULL(DEFFERED_ACQUISITION_COST,0.00)) AS DCD 
  --,PF 
