--- FINAL QUERY POLICY_RISK AND COVERAGES FOR SHEET HOSPITAL_9
--BEGIN TRAN

--- START INSERT INTO RISK LEVEL
--DROP TABLE HOSPITAL_9
--------------------------------------------------------------------------------------------------------------------------------
GO
IF NOT EXISTS(SELECT  COLUMN_NAME FROM INFORMATION_SCHEMA.COLUMNS 
                  WHERE TABLE_NAME = 'HOSPITAL_9' 
                        AND COLUMN_NAME = 'NEW_CODE') 
                  BEGIN
                        ALTER TABLE HOSPITAL_9
 ...
 IF EXISTS( SELECT COLUMN_NAME FROM INFORMATION_SCHEMA.COLUMNS
				...
				BEGIN 				
					UPDATE HOSPITAL_9 SET NEW_CODE=CODE 
...
-------------------------------------------------------------------------------------------------------------------------------------             
     ...
     SELECT APPLICATION_NO,COAPP,SUBLOB,COAPP_CNPJ,INDIVIDUAL_NAME,IS_SPOUSE_OR_CHILD,GENDER,CPF_CNPJ,STATE,
                 DATE_OF_BIRTH,REGIONAL_IDENTIFICATION ,REGIONAL_ID,REGIONAL_ID_ISSUES,POSITION,CITY_OF_BIRTH ,NEW_CODE INTO #TEMP_HOSPITAL_9 FROM HOSPITAL_9             
------------------------------------------------------------------------------------------------------------------------------------                             
...
0 AS ORIGINAL_VERSION_ID
 from #TEMP_HOSPITAL_9 with(nolock)
 ...
 PERSONAL_INFO_ID INT ,
  ...
  ( 
   ...
   FROM POL_PERSONAL_ACCIDENT_INFO PPAF WITH(NOLOCK)
                  LEFT OUTER JOIN HOSPITAL_9 H WITH(NOLOCK) ON PPAF.CODE=H.NEW_CODE
