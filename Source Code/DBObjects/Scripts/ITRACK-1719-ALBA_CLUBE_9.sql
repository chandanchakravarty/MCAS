--- FINAL QUERY POLICY_RISK AND COVERAGES FOR SHEET ALBA_CLUBE_9
--BEGIN TRAN

--- START INSERT INTO RISK LEVEL
--DROP TABLE ALBA_CLUBE_9
--------------------------------------------------------------------------------------------------------------------------------
GO
IF NOT EXISTS(SELECT  COLUMN_NAME FROM INFORMATION_SCHEMA.COLUMNS 
                  WHERE TABLE_NAME = 'ALBA_CLUBE_9' 
                        AND COLUMN_NAME = 'NEW_CODE') 
                  BEGIN
                        ALTER TABLE ALBA_CLUBE_9
 ...
 IF EXISTS( SELECT COLUMN_NAME FROM INFORMATION_SCHEMA.COLUMNS
				...
				BEGIN 				
					UPDATE ALBA_CLUBE_9 SET NEW_CODE=CODE 
...
-------------------------------------------------------------------------------------------------------------------------------------             
     ...
     SELECT APPLICATION_NO,COAPP,SUBLOB,COAPP_CNPJ,INDIVIDUAL_NAME,IS_SPOUSE_OR_CHILD,GENDER,CPF_CNPJ,STATE,
                 DATE_OF_BIRTH,REGIONAL_IDENTIFICATION ,REGIONAL_ID,REGIONAL_ID_ISSUES,POSITION,CITY_OF_BIRTH ,NEW_CODE INTO #TEMP_ALBA_CLUBE_9 FROM ALBA_CLUBE_9             
------------------------------------------------------------------------------------------------------------------------------------                             
...
0 AS ORIGINAL_VERSION_ID
 from #TEMP_ALBA_CLUBE_9 with(nolock)
 ...
 PERSONAL_INFO_ID INT ,
  ...
  ( 
   ...
   FROM POL_PERSONAL_ACCIDENT_INFO PPAF WITH(NOLOCK)
                  LEFT OUTER JOIN ALBA_CLUBE_9 H WITH(NOLOCK) ON PPAF.CODE=H.NEW_CODE
