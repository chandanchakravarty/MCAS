IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[Proc_INSERT_CURRENTRATING]') AND type in (N'P', N'PC'))
DROP PROCEDURE [dbo].[Proc_INSERT_CURRENTRATING]
GO
...
Create Proc [dbo].[Proc_INSERT_CURRENTRATING]        
  @COMPANY_ID int,        
...
IF @RATING!=@EXISTING_RATING    
 ...
 UPDATE MNT_CURRENT_RATING_LIST SET RATING=@RATING,EFFECTIVE_YEAR=@EFFECTIVE_YEAR WHERE RATING_ID=@RATINGID   
 RETURN 1  
END
