...
{
	/// <summary>
	/// Database Model for CLM_ACTIVITY_EXPENSE.
	/// </summary>
	public class ClsActivityExpenseInfo : Cms.Model.ClsCommonModel
	{
		private const string CLM_ACTIVITY_EXPENSE = "CLM_ACTIVITY_EXPENSE";
		public ClsActivityExpenseInfo()
		{
			base.dtModel.TableName = "CLM_ACTIVITY_EXPENSE";		// setting table name for data table that holds property values.
			this.AddColumns();								// add columns of the database table CLM_ACTIVITY_EXPENSE
		...
		private void AddColumns()
		{
			...
			base.dtModel.Columns.Add("ADDITIONAL_EXPENSE",typeof(double));
		...
		// model for database field ADDITIONAL_EXPENSE(double)
		public double ADDITIONAL_EXPENSE
		{
			get
			{
				return base.dtModel.Rows[0]["ADDITIONAL_EXPENSE"] == DBNull.Value ? Convert.ToDouble(null) : double.Parse(base.dtModel.Rows[0]["ADDITIONAL_EXPENSE"].ToString());
			}
			set
			{
				base.dtModel.Rows[0]["ADDITIONAL_EXPENSE"] = value;
			}
		}
