...
namespace Policies.Aspx
{
	...
	/// </summary>
	public class PolicyLossReport : Cms.Policies.policiesbase//Cms.CmsWeb.cmsbase
	{
...
//				((BaseDataGrid)c1).WhereClause=" PL.CUSTOMER_ID=" + gIntCUSTOMER_ID + " AND PL.DRIVER_NAME LIKE '" + gIntCUSTOMER_ID.ToString() + "^" + gIntPOL_ID.ToString() + "^" + gIntPOL_VERSION_ID.ToString() + "%'";
...
//			#endregion
		...
		private void ShowDrivers()
		{
			...
			try
			{
				...
				objWebGrid.SelectClause = "CUSTOMER_ID,POLICY_ID,POLICY_VERSION_ID,DRIVER_ID, ISNULL(DRIVER_FNAME,'')+ ' ' + ISNULL(DRIVER_MNAME,'')+ ' ' + ISNULL(DRIVER_LNAME,'') AS DRIVER_NAME";
		...
		private void SetDriverAutoLoss_NEW()
		{
			...
			try
			{
				...
				gIntPOL_ID 			=	int.Parse(Request["POL_ID"] == null || Request["POL_ID"] =="" ? FetchValueFromXML("POL_ID","") : Request["POL_ID"].ToString());				
				gIntPOL_VERSION_ID 	=	int.Parse(Request["POLICY_VERSION_ID"] == null || Request["POLICY_VERSION_ID"] ==""  ?  FetchValueFromXML("POLICY_VERSION_ID","") : Request["POLICY_VERSION_ID"].ToString());
				//getting info about polcy and poilcy ver
				objDSPoilcy = objGenInfo.GetPoilcyInfo(gIntCUSTOMER_ID,gIntPOL_ID,gIntPOL_VERSION_ID);
				...
				if(gstrLobID == "1")
					objDSDriver = objGenInfo.GetPolicyDriverList(gIntCUSTOMER_ID,gIntPOL_ID,gIntPOL_VERSION_ID,4);
				else
					objDSDriver = objGenInfo.GetPolicyDriverList(gIntCUSTOMER_ID,gIntPOL_ID,gIntPOL_VERSION_ID,int.Parse(gstrLobID));
				/////////////
				...
				Cms.Model.Policy.ClsPolicyDriverInfo objDriverInfo = new Cms.Model.Policy.ClsPolicyDriverInfo(); 
				Cms.Model.Application.ClsDriverDetailsInfo objAppDriverInfo=new Cms.Model.Application.ClsDriverDetailsInfo();
				if(objDSDriver.Tables[0].Rows.Count>0)
				{
					...
					objGenInfo.WriteTransactionLog(gIntCUSTOMER_ID, gIntPOL_ID, gIntPOL_VERSION_ID, trans_desc, int.Parse(GetUserId()),trans_custom, "Policy");
					...
					objAppDriverInfo.CUSTOMER_ID = objDriverInfo.CUSTOMER_ID = gIntCUSTOMER_ID;
					objDriverInfo.POLICY_ID = gIntPOL_ID;
					objDriverInfo.POLICY_VERSION_ID = gIntPOL_VERSION_ID;
					objAppDriverInfo.DRIVER_SSN  = objDriverInfo.DRIVER_SSN = objDSDriver.Tables[0].Rows[index]["DRIVER_SSN"]==null?"":objDSDriver.Tables[0].Rows[index]["DRIVER_SSN"].ToString();
					objAppDriverInfo.DRIVER_CITY = objDriverInfo.DRIVER_CITY = objDSDriver.Tables[0].Rows[index]["DRIVER_CITY"]==null?"":objDSDriver.Tables[0].Rows[index]["DRIVER_CITY"].ToString();
					objAppDriverInfo.DRIVER_ZIP  = objDriverInfo.DRIVER_ZIP = objDSDriver.Tables[0].Rows[index]["DRIVER_ZIP"]==null?"":objDSDriver.Tables[0].Rows[index]["DRIVER_ZIP"].ToString();
					objAppDriverInfo.DRIVER_ADD1 = objDriverInfo.DRIVER_ADD1 = objDSDriver.Tables[0].Rows[index]["DRIVER_ADD1"]==null?"":objDSDriver.Tables[0].Rows[index]["DRIVER_ADD1"].ToString();
				...
				for (nCount =0; nCount <= objDSDriver.Tables[0].Rows.Count -1 ; nCount++)
				{
					...
					objGenInfo.WriteTransactionLog(gIntCUSTOMER_ID, gIntPOL_ID, gIntPOL_VERSION_ID, trans_desc1, int.Parse(GetUserId()),trans_custom1, "Policy");
					string strSSN="";
				...
				retAutoLoss	= objMVRUtil.GetAutoLoss(objDSDriver.Tables[0].Rows[index]["STATE_CODE"]==null?"":objDSDriver.Tables[0].Rows[index]["STATE_CODE"].ToString(),
					...
					strDateOfBirth ,
					objDSDriver.Tables[0].Rows[index]["DRIVER_SEX"]== null?"":objDSDriver.Tables[0].Rows[index]["DRIVER_SEX"].ToString() ,"",objAppDriverInfo, ref strWebResponse,strAdditionalDrivers);
				...
				{
					string strLblText=objPriorLoss.InsertToPriorLossTable(objDSDriver,retAutoLoss.Substring(944, retAutoLoss.Length-944), objDSDriver.Tables[0].Rows[index]["DRIVER_ID"].ToString(),  strDriverName.Trim() ,userID,"POL",intNameMatch);//Done for Itrack Issue 6723 on 7 Dec 09
					lblMessage.Text += strLblText;
					lblMessage.Text += "Prior Loss Report fetched for: " + strDriverName.Trim() + "<br>";
				}
				else
				{
				...
				for(int indx=0; indx < Seleteddriv_ids.Length; indx++)
				{
					if(Seleteddriv_ids[indx].Trim()!="")
						objPriorLoss.SetLossOrdered(gIntCUSTOMER_ID,gIntPOL_ID,gIntPOL_VERSION_ID,int.Parse(Seleteddriv_ids[indx].Trim()),"PPA-POL",ReportStatus);
				}
			}
			catch(Exception ex)
			{
		...
		private void SetDriverAutoLoss()
		{
			...
			for (nCount =0; nCount <= objDSDriver.Tables[0].Rows.Count -1 ; nCount++)
			{
				...
				//string strDriverXml = Cms.BusinessLayer.BlApplication.ClsDriverDetail.GetAppDriverDetailsXML(gIntCUSTOMER_ID, gIntPOL_ID, gIntPOL_VERSION_ID, Convert.ToInt32(objDSDriver.Tables[0].Rows[nCount]["DRIVER_ID"]) );
				Cms.Model.Application.ClsDriverDetailsInfo objDriverInfo = new Cms.Model.Application.ClsDriverDetailsInfo();
				//base.PopulateModelObject(objDriverInfo, strDriverXml);
				objDriverInfo.CUSTOMER_ID = gIntCUSTOMER_ID;
//				objDriverInfo.POL_ID = gIntPOL_ID;
//				objDriverInfo.POL_VERSION_ID = gIntPOL_VERSION_ID;
			...
			//	lblMessage.Text = "The record has been added to Prior Loss Table. Please use Prior Information menu to see its updated rows.";
				...
				ClsPriorLoss objPriorLoss = new  ClsPriorLoss(); 
				objPriorLoss.SetLossOrdered(gIntCUSTOMER_ID,gIntPOL_ID,gIntPOL_VERSION_ID,int.Parse(objDSDriver.Tables[0].Rows[nCount]["DRIVER_ID"].ToString()),"PPA-POL",ReportStatus);
			}
			if(nCount == 0)
			{
		...
		//Modified for ITrack 2852 1-Nov-2007
			...
			try
			{
				...
				foreach(XmlNode lossnode in lossnodes)
				{
					...
					*/
					...
					objPriorLossInfo.DRIVER_NAME = gIntCUSTOMER_ID.ToString() + "^" + gIntPOL_ID.ToString() + "^" + gIntPOL_VERSION_ID.ToString() + "^" + driver_id + "^" + "POL";
					...
					*/
					...
					string sqlSelPriorLoss = "SELECT * FROM APP_PRIOR_LOSS_INFO WHERE OCCURENCE_DATE = '" + oc_Date + "' AND LOB = '" + objPriorLossInfo.LOB + "' AND CUSTOMER_ID = " + (Request["CUSTOMER_ID"] == null ? "0" : Request["CUSTOMER_ID"].ToString()) + " AND DRIVER_ID = " + driver_id;
