...
dated					: 16 Jul 2007
purpose					: for Policy Home/Rental Loss Report   
<Modified Date			: - > 
<Modified By			: - > 
<Purpose				: - > 
******************************************************************************************/
...
namespace Policies.Aspx.Homeowner
{
	...
	/// </summary>
	public class PolHomeLossReport : Cms.CmsWeb.cmsbase
	{
...

		{
			...
			try
			{
				...
				((BaseDataGrid)c1).WhereClause=" LOB=" + GetLOBID().ToString()  + " AND PL.CUSTOMER_ID=" + GetCustomerID().ToString()  + " AND PL.DRIVER_NAME LIKE '" + GetCustomerID().ToString() + "^" + GetPolicyID().ToString() + "^" + GetPolicyVersionID().ToString() + "%'";
		...
		public  string GetHomeLoss()
		{
			...
			string strPOL_ID= GetPolicyID().ToString();
			string strPOL_VERSION_ID= GetPolicyVersionID().ToString();
			...
			try
			{
				...
				int intPolicyID = Convert.ToInt32(GetPolicyID());
				int intPolicyVersionID = Convert.ToInt32(GetPolicyVersionID());
				...
				if(strCalledFrom.ToUpper() != "WAT")
					dtLocations = ClsLocation.GetLocationsWithCustomer(intCustomerID,intPolicyID,intPolicyVersionID,"POLICY");
				else
					dtLocations = ClsLocation.GetLocationsWithCustomer(intCustomerID,intPolicyID,intPolicyVersionID,"POLICY-WAT");
				...
				for(int i=0;i< dtLocations.Rows.Count;i++)
				{
					...
					try
					{
						...
						trans_custom += dtLocations.Rows[i]["LAST_NAME"].ToString();
						*/
						objGenInfo.WriteTransactionLog(intCustomerID, intPolicyID, intPolicyVersionID, trans_desc, int.Parse(GetUserId()),trans_custom, "Policy");
					}
					catch//(Exception ex)
					{
					}
					...
					string sqlSelAppl = "Proc_GetPDFApplicantDetails " + intCustomerID.ToString() + "," + intPolicyID.ToString() + "," + intPolicyVersionID.ToString() + ",'policy'"; // AND DRIVER_ID = " + driver_id;
					DataSet Applds = objPriorLoss.GetExistingPriorLoss(sqlSelAppl);
					#endregion
					...
					objGenInfo.WriteTransactionLog(intCustomerID, intPolicyID, intPolicyVersionID, "Property Loss Ordered on " + DateTime.Now.ToString("MM/dd/yyyy") + " for", int.Parse(GetUserId()),fetch_custom, "Policy");

//					try
//					{
						if(strCalledFrom.ToUpper() == "WAT" && 	LossFound==true)
							objPriorLoss.SetLossOrdered(intCustomerID,intPolicyID,intPolicyVersionID,int.Parse(dtLocations.Rows[i]["LOCATION_ID"].ToString()),"WAT-POL",ReportStatus);
						else if (LossFound)
							objPriorLoss.SetLossOrdered(intCustomerID,intPolicyID,intPolicyVersionID,int.Parse(dtLocations.Rows[i]["LOCATION_ID"].ToString()),"HOME-POL",ReportStatus);
//					}
//					catch(Exception)
//					{}
		...
		private void InsertToPriorLossTable(string strXMLHomeLoss,string LOC_ID)
		{
			...
			try
			{
				...
				foreach(XmlNode lossnode in lossnodes)
				{
					...
					//
					objPriorLossInfo.DRIVER_NAME = objPriorLossInfo.CUSTOMER_ID.ToString() + "^" + GetPolicyID().ToString() + "^" + GetPolicyVersionID().ToString() + "^" + LOC_ID + "^" + "POL";
