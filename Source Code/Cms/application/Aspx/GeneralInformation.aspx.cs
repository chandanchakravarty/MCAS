...
<Modified Date			: Nidhi
<Modified By			: 5/9/2005 11:54:37 AM 
<Purpose				: Output parameter for app_id.

<Modified Date			: Gaurav
<Modified By			: 5/13/2005 
<Purpose				: change cmsbutton from Execute to Write

<Modified Date			: Nidhi
<Modified By			: 06/02/2005 
<Purpose				: label for LOB
<						: 
<Modified Date			: Nidhi
<Modified By			: 06/14/2005 
<Purpose				: Delete button added but set to visible false for the moment
<						: 
<Modified Date			: Nidhi
<Modified By			: 06/23/2005 
<Purpose				: Added a combo for state which is populated when the user selects the customer.
<						: Checked the incident where the sub-lob was getting blank on changing the agency.
<						: Added an underwriter field.
<Modified Date			: Nidhi
<Modified By			: 06/25/2005 
<Purpose				: Added all the fields on the billing info screen. 
<						: Removed the billing info tab
<Modified Date			: Gaurav
<Modified By			: 06/29/2005 
<Purpose				: Added a button - Back to Search
<Modified Date			: Nidhi
<Modified By			: 06/29/2005 
<Purpose				: Quote button made functional
<Modified Date			: Nidhi & Shrikant
<Modified By			: 07/08/2005 
<Purpose				: Quote button made functional

<Modified Date			: Anurag Verma
<Modified By			: 07/12/2005 
<Purpose				: Fixing Issues

<Modified Date			: Anurag Verma
<Modified By			: 07/13/2005 
<Purpose				: Adding two controls for Policy type

<Modified Date			: Anurag Verma
<Modified By			: 07/13/2005 
<Purpose				: Making Policy_type dropdownlist as mandatory
...
<Modified By			: - Nidhi 
<Purpose				: - Added a button "Verify Application". 

<Modified Date			: - 28/09/2005
<Modified By			: - Mohit Gupta 
<Purpose				: - Agency can not create 15 days old application but wolverine can do. 

<Modified Date			: - 29/09/2005
<Modified By			: - Mohit Gupta 
<Purpose				: - Policy Type dropdown shold be displayed for Homeowners & Rental Dwelling lob with different values. 

<Modified Date			: - 06-10-2005
<Modified By			: - Vijay Arora
<Purpose				: - To Get the values of Customer Id, Application ID, Application Version ID.

<Modified Date			: - 11-10-2005
<Modified By			: - Mohit Gupta
<Purpose				: - Inception date must default to Effective date.

<Modified Date			: - 25-10-2005
<Modified By			: - Vijay Aora
<Purpose				: - Define button named btnConvertAppToPolicy and Handler of the Button.

<Modified Date			: - 07-02-2006
<Modified By			: - Shafi
<Purpose				: - Change The caption Of Delete Button.

<Modified Date			: - 13 Feb. 2006
<Modified By			: - Ashwani,  marked as <001>,<002>
<Purpose				: -	 To get the Pol_version_id

<Modified Date			: - 6 Sep. 2007
<Modified By			: - praveen kasana,  
<Purpose				: -	CheckIsactiveApplication() : Check application status
...
{	
	public class GeneralInformation  : Cms.Application.appbase 
	{
		...
		//protected System.Web.UI.WebControls.DropDownList cmbAPP_AGENCY_ID;//Commented by Charles on 21-Aug-09 for APP/POL OPTIMISATION
		...
		//protected System.Web.UI.HtmlControls.HtmlInputHidden hidAGENCY_ID; //Commented by Charles on 21-Aug-09 for APP/POL Optimization
		protected System.Web.UI.HtmlControls.HtmlInputHidden hidIsTerminated; //Added by Charles on 21-Aug-09 for APP/POL Optimization
		...
		//protected System.Web.UI.WebControls.RequiredFieldValidator rfvAPP_AGENCY_ID; //Commented by Charles on 21-Aug-09 for APP/POL Optimization
		...
		//private const string TEMP_UNDERWRITER ="To be assigned";
		...
		//protected System.Web.UI.WebControls.TextBox txtCHARGE_OFF_PRMIUM;
		...
		//protected System.Web.UI.WebControls.Label lblNA;
		
		//Commenting the IIX button
		...
		private void Page_Load(object sender, System.EventArgs e)
		{
			...
			//cmbAPP_TERMS.Attributes.Add("onBlur","javascript:fillBillingPlan();return false;"); //Commented by Charles on 14-Sep-09 for APP/POL Optimization 
			...
			txtAPP_EFFECTIVE_DATE.Attributes.Add("onblur","javascript:setTimeout('ChangeDefaultDate();',100);FormatDate();DisplaySubLobonEffDatChg();");
			...
			//btnSubmitInProgress.Attributes.Add("onclick","javascript:RetunrType();");

			//btnSubmitAnywayInProgress.Attributes.Add("onclick","javascript:return RetunrType();");
			...
			objResourceMgr = new System.Resources.ResourceManager("Cms.Application.Aspx.GeneralInformation" ,System.Reflection.Assembly.GetExecutingAssembly());
			...
			if(!Page.IsPostBack)
			{
				...
				SetErrorMessages();
				SetCaptions();
				//CheckBillingPlan();	//Moved to Line# 663 by Charles on 14-Sep-09 for APP/POL Optimization			
				FillControls();
				//SetAgencyBillType();
				#endregion
				//SUBLOBS
					...
					if (Request.QueryString["CALLEDFROM"]!= null &&(Request.QueryString["CALLEDFROM"].ToString().Equals(CALLED_FROM_CLIENT)||Request.QueryString["CALLEDFROM"].ToString().Equals(CALLED_FROM_INNER_CLIENT)))
					{
						...
						if (dsCustomer!=null && dsCustomer.Tables[0].Rows.Count>0)
						{
							if (dsCustomer.Tables[0].Rows[0]["IS_CUSTOMER_ACTIVE"].ToString().Trim()=="" || dsCustomer.Tables[0].Rows[0]["IS_CUSTOMER_ACTIVE"].ToString().Trim()=="Y")
							{
								...
								if (strSystemID.Trim().ToUpper() !=strCarrierSystemID.ToUpper().Trim())
									strAgency_ID = cmbAPP_AGENCY_ID.Items[cmbAPP_AGENCY_ID.SelectedIndex].Value; */
						...
						/*Commented by Charles on 21-Aug-09 for APP/POL Optimization
						cmbAPP_AGENCY_ID.SelectedIndex=cmbAPP_AGENCY_ID.Items.IndexOf(cmbAPP_AGENCY_ID.Items.FindByValue(strAgency_ID));
						//make Agency Read only as per Itrack 5004 -- Code added Pravesh on 27 Nov 08
						cmbAPP_AGENCY_ID_SelectedIndexChanged(null,null);
						cmbAPP_AGENCY_ID.Enabled=false;
						// end here Itrack 5004
						*/
						cmbPOLICY_TYPE.SelectedIndex =cmbPOLICY_TYPE.Items.IndexOf(cmbPOLICY_TYPE.Items.FindByValue(hidPOLICY_TYPE.Value));
			...
			else
			{
				...
				/*Commented by Charles on 21-Aug-09 for APP/POL Optimizations
				DataSet dsCustomer = ClsCustomer.GetCustomerDetails(int.Parse(GetCustomerID()));
				if (dsCustomer!=null && dsCustomer.Tables[0].Rows.Count>0)
				{
					strAgency_ID=dsCustomer.Tables[0].Rows[0]["CUSTOMER_AGENCY_ID"]==null?"":dsCustomer.Tables[0].Rows[0]["CUSTOMER_AGENCY_ID"].ToString() ;
					if (GetSystemId() !=strCarrierSystemID.ToUpper().Trim())
						strAgency_ID = cmbAPP_AGENCY_ID.Items[cmbAPP_AGENCY_ID.SelectedIndex].Value; */
			...
			{
				btnQuote.Attributes.Add("Style","display:none");
				btnConvertAppToPolicy.Attributes.Add("Style","display:none");
			}
			#endregion
			...
			//hidCSR.Value = cmbCSR.Value;//Commented for Itrack Issue 5454(Attachment 2)
		...
		{ 
			...
			//Commented by Sibin for Itrack Issue 5166 on 8 Jan 09
			//cmbSTATE_ID.Items.Insert(0,new ListItem("","0"));
			...
			/*Commented by Charles on 24-Aug-09 for APP/POL Optimization
			string  strSystemID			 = GetSystemId();
			string  strCarrierSystemID = System.Configuration.ConfigurationSettings.AppSettings.Get("CarrierSystemID").ToString();
			if ( strSystemID.Trim().ToUpper() != strCarrierSystemID.Trim().ToUpper())
			{
				DataSet objDataSet = ClsAgency.GetAgencyIDAndNameFromCode(strSystemID);

				if (objDataSet.Tables[0].Rows.Count > 0 )
				{
					
					cmbAPP_AGENCY_ID.Items.Clear();
					cmbAPP_AGENCY_ID.Items.Add(new ListItem(objDataSet.Tables[0].Rows[0]["AGENCY_DISP_NAME"].ToString(),objDataSet.Tables[0].Rows[0]["AGENCY_ID"].ToString()));
					cmbAPP_AGENCY_ID.Items[0].Selected = true; 
					lblAGENCY_DISPLAY_NAME.Text=objDataSet.Tables[0].Rows[0]["AGENCY_DISP_NAME"].ToString();
					hidAPP_AGENCY_ID.Value=objDataSet.Tables[0].Rows[0]["AGENCY_ID"].ToString();
				}
				cmbAPP_AGENCY_ID.Enabled = false;				
			}
			else
			...
			*/
				...
				/* Commented by Charles on 19-Aug-09 for APP/POL OPTIMIZATIONS
				//Added Manoj Rathore Getting AgencyIds to Change Color by Terminal Date
				DataSet ds = Cms.BusinessLayer.BlCommon.ClsAgency.GetInactiveAgencyDataset(EffectiveDate);
				string strAgnID = "";
				string StrIds = "";
				...
				hidAGENCY_ID.Value=StrIds;
				cmbAPP_AGENCY_ID.Enabled = false;
				//end 
		  } */
			//installment plans Dead code : to be vrfd
			/* Commented by Charles on 14-Sep-09 for APP/POL Optimization
		...
		private void setCarrierInsuredBill()
		{
			...
			//if(strAGENCY_ID.IndexOf(strCarrierSystemID.ToUpper()) == 0) //Commented by Charles on 24-Aug-09 for APP/POL Optimization
		...
		private void SetCarrierInsureBillAtPageLoad()
		{
			if (hidOldData.Value != null && hidOldData.Value !="" && hidOldData.Value !="0")
			{
				...
				//cmbAPP_AGENCY_ID.SelectedIndex=cmbAPP_AGENCY_ID.Items.IndexOf(cmbAPP_AGENCY_ID.Items.FindByValue(strAgency)); //Commented by Charles on 21-Aug-09 for APP/POL Optimization
		...
		private ClsGeneralInfo GetFormValue()
		{
			...
			if(hidAppID.Value.ToString().Trim()!="" && hidAppID.Value.ToString().Equals ("NEW")) // Request.QueryString["CUSTOMER_ID"]==null || Request.QueryString["CUSTOMER_ID"].ToString()=="")
			{
				if(txtAPP_NUMBER.Text.Trim()== TEMP_APP_NUMBER ) //|| txtUNDERWRITERNAME.Text.Trim()== TEMP_UNDERWRITER
				{
			...
			else
			{
				...
				//				{
				//					objGeneralInfo.UNDERWRITER =int.Parse(cmbUNDERWRITER.SelectedValue);//int.Parse (ClsUser.GetUnderwriterForApplication());//UNDERWRITER
				//				}
				//				else
				//				{
				//					objGeneralInfo.UNDERWRITER=0;
				//				}
			}
			...
			//objGeneralInfo.APP_TERMS=	cmbAPP_TERMS.SelectedItem.Value==null?"":cmbAPP_TERMS.SelectedItem.Value;
			...
			objGeneralInfo.APP_SUBLOB=	hidSUB_LOB.Value=="" ? "0" : hidSUB_LOB.Value;//cmbAPP_SUBLOB.SelectedItem.Value==null?"0":cmbAPP_SUBLOB.SelectedValue;
			
			objGeneralInfo.CSR = int.Parse(hidCSR.Value=="" ? "-1" : hidCSR.Value);
			//add by kranti
			//			if (cmbCSR.Value.Trim()!=null)
			//			{
			//				objGeneralInfo.CSR = int.Parse(cmbCSR.Value=="" ? "0" : cmbCSR.Value);
			//				
			//									
			//			}			
			...
			/* Commented by Charles on 21-Aug-09 for APP/POL Optimization
			if (cmbAPP_AGENCY_ID.SelectedItem!=null)
			{
				objGeneralInfo.APP_AGENCY_ID=int.Parse(cmbAPP_AGENCY_ID.SelectedItem.Value=="" ? "0" : cmbAPP_AGENCY_ID.SelectedItem.Value);
			}
			else
			{
				objGeneralInfo.APP_AGENCY_ID =0;
			} */
			...
			//billing information
			//			if(cmbDIV_ID.SelectedItem !=null  )
			//			{
			//				if(cmbDIV_ID.SelectedValue.ToString()!="Select Division")
			//objGeneralInfo.DIV_ID=	int.Parse(cmbDIV_ID.SelectedValue.ToString());
			//			}
			...
			if(cmbBILL_TYPE_ID.SelectedItem != null)
			{
...

				{
					...
					{
						objGeneralInfo.INSTALL_PLAN_ID = int.Parse (cmbINSTALL_PLAN_ID.SelectedItem.Value);
					}
					else
					{
		...
		{
			rfvCUSTOMER_ID.ErrorMessage			=  Cms.CmsWeb.ClsMessages.GetMessage(base.ScreenId,"100");
			rfvAPP_TERMS.ErrorMessage			=  Cms.CmsWeb.ClsMessages.GetMessage(base.ScreenId,"93");
			rfvAPP_EFFECTIVE_DATE.ErrorMessage	=  Cms.CmsWeb.ClsMessages.GetMessage(base.ScreenId,"95");
			rfvAPP_EXPIRATION_DATE.ErrorMessage	=  Cms.CmsWeb.ClsMessages.GetMessage(base.ScreenId,"96");
			rfvAPP_LOB.ErrorMessage				=  Cms.CmsWeb.ClsMessages.GetMessage(base.ScreenId,"97");
			revAPP_EFFECTIVE_DATE.ErrorMessage  = Cms.CmsWeb.ClsMessages.GetMessage("502","22");
			//rfvAPP_AGENCY_ID.ErrorMessage		=  Cms.CmsWeb.ClsMessages.GetMessage(base.ScreenId,"99"); //Commented by Charles on 21-Aug-09 for APP/POL Optimization
			...
			rfvSTATE_ID.ErrorMessage = Cms.CmsWeb.ClsMessages.GetMessage(base.ScreenId,"35");
			revAPP_INCEPTION_DATE.ValidationExpression=aRegExpDate;
			revAPP_INCEPTION_DATE.ErrorMessage = Cms.CmsWeb.ClsMessages.GetMessage("502","22");
		...
		private void InitializeComponent()
		{
			...
			//this.cmbAPP_AGENCY_ID.SelectedIndexChanged += new System.EventHandler(this.cmbAPP_AGENCY_ID_SelectedIndexChanged); //Commented by Charles on 21-Aug-09 for APP/POL Optimization
			...
			//this.btnSubmitInProgress.Click += new System.EventHandler(this.btnSubmitInProgress_Click);
			//this.btnSubmitAnywayInProgress.Click += new System.EventHandler(this.btnSubmitAnywayInProgress_Click);
		...
		/* Commented by Charles on 21-Aug-09 for APP/POL Optimization
		private void cmbAPP_AGENCY_ID_SelectedIndexChanged(object sender, System.EventArgs e)
		{
			
			try
			{		
				//hidFormSaved = 4 when   the control is coming from the customer section.
				if (hidFormSaved.Value !="4")
				{
					hidFormSaved.Value ="3";
				}
				if (cmbAPP_AGENCY_ID.SelectedItem !=null && cmbAPP_AGENCY_ID.SelectedItem.Value !="")
				{
					//fill CSR dropdown
					FillCSRDropDown(int.Parse(cmbAPP_AGENCY_ID.SelectedItem.Value));
					FillProducerDropDown(int.Parse(cmbAPP_AGENCY_ID.SelectedItem.Value));
				}
				else
				{
					cmbCSR.Items.Clear();
				...
				cmbAPP_TERMS.ClearSelection(); 
				ListItem li=new ListItem();  
				li=cmbAPP_TERMS.Items.FindByValue(appTerms);  				
				if(li!=null)
					li.Selected=true; 	
				
				hidFocusFlag.Value="2"; 

				//SetAgencyBillType() function will be called only Onchange of Agency Dropdown,not on SAVE
				//Hence set the flag:
				if(hidBILL_TYPE_FLAG.Value!="1")
				{
					SetAgencyBillType();
				}
		...
		private void SetAgencyBillType()
		{
			...
			if(dsAgency!= null && dsAgency.Tables[0].Rows.Count > 0)
			{
				...
				//				switch(bill_type.Trim())
				//				{
				//					case "0": bill_type = "8460"; break;
				//					case "1": bill_type = "8459"; break;
				//					case "2": bill_type = "11191"; break;
				//					default: bill_type = "8460"; break;
				//				}
				for(int index =0;index < cmbBILL_TYPE_ID.Items.Count;index++)
				{	
		...
		private void btnCopy_Click(object sender, System.EventArgs e)
		{
			...
			{ 
				...
				//SetAppVersionID(strVersionID.ToString());
				//hidAppVersionID = strVersionID.ToString();
				...
				try
				{
					...
					switch(Convert.ToInt32(cmbAPP_LOB.SelectedValue))
					{
						...
						case 2:
							cmbAPP_TERMS.SelectedIndex =1;
							break;
						case 3://Change selected index to 2 from 12,on 31-Jul-09,for Itrack 6205,by Charles
		...
		{			
			try
			{				
				//string[] strTerminatedAgency = hidAGENCY_ID.Value.Split('^'); //Commented by Charles on 21-Aug-09 for APP/POL Optimization
				//for(int i=0; i< strTerminatedAgency.Length-1; i++) //Commented by Charles on 21-Aug-09 for APP/POL Optimization
				//{
					//if(cmbAPP_AGENCY_ID.SelectedValue == strTerminatedAgency[i].ToString()) //Commented by Charles on 21-Aug-09 for APP/POL Optimization
				...
				string strCSSNo=GetColorScheme(),strCalled="ANYWAY",AppStatus="",strMessage="",strPolNumber="",strShowMessage="";
				PrintingError=false;
				//check for 
				string isActive="";
				//string isPolActive="";
		...
		{		
			try
			{
				...
				//{
				//	if(cmbAPP_AGENCY_ID.SelectedValue == strTerminatedAgency[i].ToString())
					if(hidIsTerminated.Value=="Y")
					{
						lblMessage.Text = ClsMessages.FetchGeneralMessage("1014");
						lblMessage.Visible=true;
				...
				string strCalled="SUBMIT",strMessage="",AppStatus="";
...
//		private void btnSubmitInProgress_Click(object sender, System.EventArgs e)
//		{
//		
//		}
//
//		private void btnSubmitAnywayInProgress_Click(object sender, System.EventArgs e)
//		{
//		
//		}
