...
<%@ Page language="c#" Codebehind="GeneralInformation.aspx.cs" AutoEventWireup="false" validateRequest="false" Inherits="Cms.Application.Aspx.GeneralInformation" %>
...
<HTML>
	<HEAD>
		...
		lobMenuArr[5]	= 5;
		lobMenuArr[6]	= 3;
		//Added By Gaurav for General Liability
		lobMenuArr[7]	= 6;
...
function setTab()
{
	...
	{ 
		if ((document.getElementById('hidFormSaved').value == '1') || (document.getElementById('hidOldData').value != ""))
		{
			...
			URL = "GeneralInformation.aspx?CUSTOMER_ID=" + CustomerID  
				+ "&APP_ID= " + AppID  
				+ "&APP_VERSION_ID=" + AppVersionID
				+ "&CALLEDFROM=" + document.getElementById("hidCalledFrom").value
				+ "&transferdata="
			RemoveTab(1,top.frames[1]);
			DrawTab(1,top.frames[1],"Application Information",URL);
			
			
			
			if (document.getElementById('hidLOBID')!=null)
			{
			//alert(document.getElementById('hidLOBID').value);
				SelectedLOBID=document.getElementById('hidLOBID').value;	
				if(SelectedLOBID =="5")
				{	
					//Url="AddBillingInfo.aspx?Lob=" + SelectedLOBID + "&CUST_ID="+CustomerID+"&AP_ID="+AppID+"&APP_VER_ID="+AppVersionID+"&";
					//DrawTab(2,top.frames[1],'Billing Information',Url);			
					//Url="PkgLobDetailsIndex.aspx?CUSTOMER_ID="+CustomerID+"&APP_ID="+AppID+"&APP_VERSION_ID="+AppVersionID+"&"
					//Changes Made By Ravindra(02/10/2006) Start:
					//Title Package LOB Details Changed To Umbrella LOB Details as per 
					//Change Request in UmbrellaRiskInformationSereenFlow.doc
					//DrawTab(2,top.frames[1],'Package LOB Details',Url);		
					//DrawTab(2,top.frames[1],'Umbrella LOB Details',Url);	
					//Changes Made By Ravindra(02/10/2006) End
					...
					DrawTab(2,top.frames[1],'Co-Applicant Details',Url);		
						...
						DrawTab(4,top.frames[1],'Billing Info',Url);
		...
		//	alert(document.getElementById('hidOldData').value);
			...
			{ 
				...
				{	
					...
					{		
						...
						if(tempXML!="" && tempXML!=0)
						{
							...
							//document.getElementById('txtCHARGE_OFF_PRMIUM').value=formatCurrency(document.getElementById('txtCHARGE_OFF_PRMIUM').value);
						...
						//	
							...
							//FillDepartment();
							//SelectComboOption('cmbDEPT_ID',document.getElementById('hidDEPT_ID').value)
							//FillProfitCenter();  
							//SelectComboOption('cmbPC_ID',document.getElementById('hidPC_ID').value)
							
							//if(document.getElementById('txtCHARGE_OFF_PRMIUM').value=="0.00")
							//	document.getElementById('txtCHARGE_OFF_PRMIUM').value="";
				...
				if(document.getElementById("hidAPP_AGENCY_ID")!=null)
				{ 
					if(document.getElementById('hidFormSaved')!=null &&( document.getElementById('hidFormSaved').value == '0'||document.getElementById('hidFormSaved').value == '1'))
					{
						var AgencyId = document.getElementById("hidAPP_AGENCY_ID").value;
						SelectComboOption("cmbAPP_AGENCY_ID",AgencyId);		
	...
	{	
		 var varAgnID = document.getElementById('hidAGENCY_ID').value;
		 array = varAgnID.split("^");
		 for(i=0;i<array.length;i++)
		 {
			var arr	= array[i];			
			for (cntr=0;cntr<document.getElementById('cmbAPP_AGENCY_ID').length;cntr++)
			{
			
				if(document.getElementById('cmbAPP_AGENCY_ID').options[cntr].value==arr)
				{
					document.getElementById('cmbAPP_AGENCY_ID').options[cntr].className ="DeactivatedInstallmentPlan";
				}
		...
		{ 
			...
			else
			{
				...
				if(sEffDate != "")
				{
					...
					//{
					//	sEffDate = BuildDateForCnty(document.APP_LIST.txtAPP_EFFECTIVE_DATE,'U');
					//}
					//else								//Date in US format
					{
		...
		function FillDepartment()
			{
				...
				if (document.getElementById('trDETAILS')!=null  && (document.getElementById('trDETAILS').style.display == "inline" || document.getElementById('trDETAILS').style.display == "") )
				{
 					document.getElementById('cmbDEPT_ID').innerHTML='';
					var str1 = "";
					
					var deptXml = document.getElementById('hidDepartmentXml').value;
				
					var divId=document.getElementById('cmbDIV_ID').options[document.getElementById('cmbDIV_ID').selectedIndex].value ;
					...
					document.getElementById('cmbDEPT_ID').add(oOption);
					
					for(i=0;i<tree.childNodes.length;i++)
					{
						if(!tree.childNodes[i].firstChild) continue;
						
							var nodeName = tree.childNodes[i]; 
			...
			*/
			...
			function FillProfitCenter()
			{
				if (document.getElementById('trDETAILS')!=null  && (document.getElementById('trDETAILS').style.display == "inline" || document.getElementById('trDETAILS').style.display == "") )
				{
 				document.getElementById('cmbPC_ID').innerHTML='';
				var pcXml = document.getElementById('hidProfitCenterXml').value ;
				
				var Deptid=document.getElementById('cmbDEPT_ID').options[document.getElementById('cmbDEPT_ID').selectedIndex].value;
				document.getElementById('hidDEPT_ID').value = Deptid;
				...
				oOption=document.createElement("option");
								...
								document.getElementById('cmbPC_ID').add(oOption);
				if(document.getElementById('cmbDEPT_ID').selectedIndex ==	0)
				{
			...
			*/
			...
			function FillProfitCenter()
			{
				...
				var Deptid;
				if(document.getElementById('cmbDEPT_ID').options.length>1)
				{
					Deptid=document.getElementById('cmbDEPT_ID').options[document.getElementById('cmbDEPT_ID').selectedIndex].value;
					document.getElementById('hidDEPT_ID').value = Deptid;
			...
			*/
			...
			{
				var pcid=document.getElementById('cmbPC_ID').options[document.getElementById('cmbPC_ID').selectedIndex].value;
				document.getElementById('hidPC_ID').value = pcid;
			...
			*/
			...
			{			
				if (document.getElementById('cmbDEPT_ID').options.selectedIndex==0)
				{
			...
			*/
			...
			{
				if (document.getElementById('cmbPC_ID').options.selectedIndex==0)
				{
					arguments.IsValid = false;
					return;				
				}			
			...
			*/
			...
			{
				if (document.getElementById('cmbDIV_ID').options.selectedIndex==0)
				{
					arguments.IsValid = false;
					return;				
				}			
			...
			*/
			...
			function ResetForm1()
			{
				...
				document.location.href="GeneralInformation.aspx?CUSTOMER_ID="+custID+"&APP_ID="+appID+"&APP_VERSION_ID="+appVersionID+"&CALLEDFROM="+ calledFrom + "&transferdata=";
				return false; 
			}
			...
			{
				//return;
				//alert(document.getElementById('hidBILL_TYPE_ID').value);
				//.options[document.getElementById('cmbBILL_TYPE_ID').selectedIndex].value)
			  ...
			  if(document.getElementById('trDETAILS')==null)
			  {
			   ...
			   return false;
				...
				if(document.getElementById("hidFocusFlag")!=null)
				{
					...
					/*Commented by Charles on 21-Aug-09 for APP/POL Optimization
					if(document.getElementById("hidFocusFlag").value=="2")
					{
						if(document.getElementById("cmbAPP_AGENCY_ID"))
						{
							if(document.getElementById("cmbAPP_AGENCY_ID").style.display!='none')
							{
								//document.getElementById("cmbAPP_AGENCY_ID").focus();
								document.getElementById("hidFocusFlag").value="0"
							}
						}
			...
			function DisplaySubLobonEffDatChg()
			{	
				
				var CalledFrom = document.getElementById('hidCallefroms').value;
				var trailexpiryDate = '01/01/2010';
				var diff = DateDiffernce(trailexpiryDate,document.getElementById('txtAPP_EFFECTIVE_DATE').value);
				...
				if((diff<0 && State=="14" && (CalledFrom == "AUTO" || LOB == "2")))
					{ 
						document.getElementById('capAPP_SUBLOB').style.display='inline';
						document.getElementById('cmbAPP_SUBLOB').style.display='inline';
					}
				else if((diff>=0 && State=="14" && (CalledFrom == "AUTO" || LOB == "2")))
					{ 
			...
			function DisplaySubLOB()
			{
				...
				if( LOB == "3" || LOB == "4" || LOB == "5" || LOB == "1" || LOB == "6" || LOB == "7" || CalledFrom == "MOT" || CalledFrom == "WAT" || CalledFrom == "UMB" || CalledFrom == "HOME" || CalledFrom == "REDW" || CalledFrom == "GEN" || (State=="22" && (CalledFrom == "AUTO" || LOB == "2")) || diff>=0)
				 { 
					document.getElementById('capAPP_SUBLOB').style.display='none';
					document.getElementById('cmbAPP_SUBLOB').style.display='none';
				 }
			...
			{ 
				objArgs.IsValid=true;
					return;
				/*//get the value of Policy term
				var policyTermValue = document.getElementById('cmbAPP_TERMS').options[document.getElementById('cmbAPP_TERMS').selectedIndex].value;
				
				if (document.getElementById('cmbINSTALL_PLAN_ID').selectedIndex == -1)
					return;				
				...
				//alert('in chk');
				// No need to check for policyterm in case of full pay plan
				var FullPayPlanID = '<%=Cms.BusinessLayer.BlCommon.Accounting.ClsInstallmentPlan.FullPayPlanID()%>';
				
				if (PlanId == FullPayPlanID)
				{
					objArgs.IsValid=true;
					return;
				}
				...
				{
					var objXmlHandler = new XMLHandler();
					var tree = (objXmlHandler.quickParseXML(strXML).getElementsByTagName('INSTALL_PLAN_ID'));			
					
					for (ctr=0; ctr < tree.length; ctr++)
					{
						if (tree[ctr].firstChild.text == PlanId)
						{
							var MonthsBetween = parseInt(tree[ctr].parentNode.childNodes[5].firstChild.text); //.childNodes.length);
							var NoOfPaymt = parseInt(tree[ctr].parentNode.childNodes[4].firstChild.text);
							if (policyTermValue >= MonthsBetween * NoOfPaymt)
							{
								objArgs.IsValid=true;
								return;
							}
			...
			if(document.getElementById('cmbAPP_LOB').value == '1' || document.getElementById('cmbAPP_LOB').value == '6')
			{
				...
				*/
				...
				/*else if(document.getElementById('cmbAPP_LOB').value == '6')
				{
					//document.getElementById("rfvPROPRTY_INSP_CREDIT").setAttribute('enabled',false); 
					//document.getElementById("rfvPROPRTY_INSP_CREDIT").setAttribute('isValid',false); 
					//document.getElementById("rfvPROPRTY_INSP_CREDIT").style.display = "none";
					...
					//document.getElementById("cmbPIC_OF_LOC").className = "none";
					//alert('rental');
					//alert(document.getElementById('cmbPROPRTY_INSP_CREDIT').options.selectedIndex);
					//alert(document.getElementById('cmbPIC_OF_LOC').options.selectedIndex);
					if(document.getElementById("cmbPROPRTY_INSP_CREDIT").value!=10964 && document.getElementById("cmbPROPRTY_INSP_CREDIT").value!=10963)
					{
						document.getElementById("rfvPROPRTY_INSP_CREDIT").setAttribute('enabled',true); 
						document.getElementById("rfvPROPRTY_INSP_CREDIT").setAttribute('isValid',true); 
						document.getElementById("rfvPROPRTY_INSP_CREDIT").style.display = "inline";
						document.getElementById("spnPROPRTY_INSP_CREDIT").style.display = "inline";
						document.getElementById("cmbPROPRTY_INSP_CREDIT").className = "inline";
					}
					if(document.getElementById("cmbPIC_OF_LOC").value!=10964 && document.getElementById("cmbPIC_OF_LOC").value!=10963)
					{
						document.getElementById("rfvPIC_OF_LOC").setAttribute('enabled',true); 
						document.getElementById("rfvPIC_OF_LOC").setAttribute('isValid',true); 
						document.getElementById("rfvPIC_OF_LOC").style.display = "inline";
						document.getElementById("spnPIC_OF_LOC").style.display = "inline";
						document.getElementById("cmbPIC_OF_LOC").className = "inline";
					}
   ...
   function RefreshClientTop()
	...
	<BODY oncontextmenu = "return false;" leftMargin="0" topMargin="0" onload="Init();ApplyColor(); RefreshClientTop();" onkeydown="ProcessKeypress();">
					...
					<TD>
						<TABLE id="Table2" width="100%" align="center" border="0">
							<TBODY>
								...
								<tr>
									<TD class="headerEffectSystemParams" colSpan="4">Application</TD>
								</tr>
								<tr>
								...
								<tr>
									<TD class="headerEffectSystemParams" colSpan="4">Term</TD>
								</tr>
								<tr>
									...
									<TD class="midcolora"><asp:dropdownlist id="cmbAPP_TERMS" onfocus="SelectComboIndex('cmbAPP_TERMS')" runat="server" 
										 AutoPostBack="true"></asp:dropdownlist><!-- OnBlur="fillBillingPlan() onchange="ShowExpirationDate();" removed by Charles on 14-Sep-09 for APP/POL Optimization" --><BR>
									...
									<TD class="midcolora" vAlign="middle"><asp:textbox id="txtAPP_EFFECTIVE_DATE" runat="server" maxlength="10" size="12" Display="Dynamic" onblur="DisplaySubLobonEffDatChg();"></asp:textbox><asp:hyperlink id="hlkCalandarDate" runat="server" CssClass="HotSpot" Display="Dynamic">
								...
								<tr>
									<TD class="headerEffectSystemParams" colSpan="4">Agency</TD>
								</tr>
								<tr>
									...
									<!-- Commented by Charles on 21-Aug-09 for APP/POL OPTIMISATION --><%--<asp:dropdownlist id="cmbAPP_AGENCY_ID" onfocus="SelectComboIndex('cmbAPP_AGENCY_ID')" runat="server"
											AutoPostBack="True"></asp:dropdownlist><BR>
										<asp:requiredfieldvalidator id="rfvAPP_AGENCY_ID" runat="server" Display="Dynamic" ControlToValidate="cmbAPP_AGENCY_ID"
											ErrorMessage="APP_AGENCY_ID can't be blank."></asp:requiredfieldvalidator>--%></TD>	
									</tr>
								<tr>
									...
									<TD class="midcolora"><SELECT id="cmbCSR" onfocus="SelectComboIndex('cmbCSR')" runat="server" onchange="sethidCSR();"></SELECT></TD>
								
								</tr>
								<tr>
									<TD class="headerEffectSystemParams" colSpan="4">Billing Information</TD>
								</tr>
								<tr>
									...
									<TD class="midcolora" width="34%">
										...
										<asp:dropdownlist id="cmbINSTALL_PLAN_ID" onfocus="SelectComboIndex('cmbINSTALL_PLAN_ID')" runat="server"
											onchange="fillDownPayMode();"></asp:dropdownlist><%--asp:label id="lblNA" style="DISPLAY: none" runat="server" CssClass="LabelFont">Not Applicable</asp:label--%><br>
								...
								<tr>
									...
									<TD class="midcolora" width="32%"><%--<asp:dropdownlist id="cmbUNDERWRITER" onfocus="SelectComboIndex('cmbUNDERWRITER')" Runat="server"></asp:dropdownlist>--%><asp:label id="lblUNDERWRITER" runat="server"></asp:label></TD>
								...
								<tr>
									<TD class="headerEffectSystemParams" colSpan="4">Any other policy with Wolverine</TD>
								</tr>
								<tr>
								...
								<tr id="trbutton" runat="server">
									...
									<td class="midcolorr" align="right" colSpan="2">
										...
										<cmsb:cmsbutton class="clsButton" id="btnSubmitAnywayInProgress" runat="server" Text="Submit Anyway InProgress" CausesValidation="False"></cmsb:cmsbutton>&nbsp;
								...
								<%--<input id="hidAGENCY_ID" type="hidden" name="hidAGENCY_ID" runat="server">--%><!-- Commented by Charles on 21-Aug-09 for APP/POL Optimization -->
							...
							<TBODY>
								<tr>
									...
									<td class="midcolorr" colSpan="3"><cmsb:cmsbutton class="clsButton" id="btnCustomerAssistant" runat="server" Text="Back To Customer Assistant"></cmsb:cmsbutton></ASP:BUTTON>
										<%--<input type="button" id="btnFetcgIIX" style="display:none" value="Call IIX" runat="server" class="clsbutton">--%>
									</td>
								</tr>
							</TBODY>
						</TABLE>
		...
		{
			//window.open("ShowInputVerificationXml.aspx");
			 
			//parent.refSubmitWin=window.open('ShowInputVerificationXml.aspx','BRICS','resizable=yes,scrollbars=yes,left=150,top=50,width=650,height=620');
			...
			//wBody=parent.refSubmitWin.document.body;			
			//parent.refSubmitWin.document.write(document.getElementById("hidHTML").value);
			//parent.refSubmitWin.document.title = "Input XML Verification";
