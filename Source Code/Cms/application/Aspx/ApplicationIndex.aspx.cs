...
{
	/// <summary>
	/// Summary description for ApplicationIndex.
	/// </summary>
	public class ApplicationIndex : Cms.Application.appbase
	{
		...
		private void Page_Load(object sender, System.EventArgs e)
		{
			...
			string colors="";
            objResourceMgr = new ResourceManager("Cms.Application.Aspx.ApplicationIndex", Assembly.GetExecutingAssembly());
			...
			try
			{
                ...
                objWebGrid.SelectClause = @" case when pcl.APP_STATUS ='QAPP' then pcl.APP_NUMBER end as QQ_NUMBER,
case when pcl.APP_STATUS <> 'QAPP' then pcl.APP_NUMBER + ' ' +pcl.POLICY_DISP_VERSION +'('+(ISNULL(PPSMV.POLICY_DESCRIPTION,PPSM.POLICY_DESCRIPTION)) + ')' end as APP_Number,
Convert(varchar(10),PCL.APP_EFFECTIVE_DATE,case " + GetLanguageID() + @" when 1 then 101 else 103 end) as EffectiveDate ,
Convert(varchar(10),PCL.APP_EXPIRATION_DATE,case " + GetLanguageID() + @" when 1 then 101 else 103 end) as ExpirationDate,
pcl.AGENCY_ID, USER_LIST.USER_FNAME +' ' + USER_LIST.USER_LNAME as CSR,
pcl.POLICY_LOB as LOB_id,ISNULL(LVM.LOB_DESC,lob.LOB_DESC)LOB_DESC,pcl.POLICY_STATUS,
isnull(CLT.CUSTOMER_FIRST_NAME,'') +' '+isnull(CLT.CUSTOMER_MIDDLE_NAME,'') +' '+ isnull(CLT.CUSTOMER_LAST_NAME,'') as CustomerName,
isnull(MSCL.STATE_CODE,case " + GetLanguageID() + @" when 2 then 'Todos' else 'All' end) as STATE_CODE,
case when LVM.LANG_ID=2 then case when pcl.BILL_TYPE='DB'then 'DIR' else 'AB'end else pcl.BILL_TYPE end  as BILL_TYPE,
pcl.CUSTOMER_ID,pcl.APP_VERSION_ID,pcl.APP_ID,pcl.POLICY_ID,pcl.POLICY_VERSION_ID,
isnull(pcl.IS_ACTIVE,'N') as IS_ACTIVE,isnull(AGENCY_LIST.AGENCY_DISPLAY_NAME,'') as Agency,
case when qcql.quote_xml is not null then '<img src=/cms//cmsweb/images/quote.gif style=''Border-Width:0'' ImageAlign=absMiddle>'  else '' end  as quote_xml,
'POLICY_ID='+isnull(cast(PCL.POLICY_ID as varchar(8000)),0)+'&CUSTOMER_ID='+isnull(cast(PCL.CUSTOMER_ID as varchar(8000)),0)+'&POLICY_VERSION_ID='+isnull(cast(PCL.POLICY_VERSION_ID as varchar(8000)),0)+'&pcl.POLICY_LOB='+isnull(cast(LOB.LOB_id as varchar(8000)),0) as UniqueGrdId ";
...
LEFT OUTER  JOIN MNT_AGENCY_LIST AGENCY_LIST WITH(NOLOCK) ON PCL.AGENCY_ID = AGENCY_LIST.AGENCY_ID  
LEFT OUTER JOIN MNT_USER_LIST USER_LIST WITH(NOLOCK) ON PCL.CSR = USER_LIST.[USER_ID]
LEFT OUTER JOIN MNT_COUNTRY_STATE_LIST MSCL  WITH(NOLOCK) ON MSCL.STATE_ID=PCL.STATE_ID
LEFT OUTER JOIN POL_POLICY_STATUS_MASTER PPSM  WITH(NOLOCK) ON PPSM.POLICY_STATUS_CODE=PCL.APP_STATUS
LEFT OUTER JOIN POL_POLICY_STATUS_MASTER_MULTILINGUAL PPSMV  WITH(NOLOCK) ON PPSMV.POLICY_STATUS_CODE=PCL.APP_STATUS and PPSMV.LANG_ID = " + GetLanguageID() + @"
left OUTER join MNT_LOB_MASTER_MULTILINGUAL LVM with(nolock) on PCL.POLICY_LOB=LVM.LOB_id and LVM.LANG_ID = " + GetLanguageID() + @" ";

                //objWebGrid.WhereClause = sWHERECLAUSE + " AND PCL.APP_STATUS IN ('APPLICATION','REJECT') ";
                objWebGrid.WhereClause = sWHERECLAUSE + " " + objResourceMgr.GetString("ExtraWhereClauses") + " ";
                objWebGrid.SearchColumnHeadings = objResourceMgr.GetString("SearchColumnHeadings");//"App #^Eff. Date^Exp. Date^Agency^CSR^LOB^State^Bill Type"; //Policy #^Quick Quote #^
               ...
               /* objWebGrid.SearchColumnNames = "APP_Number^APP_EFFECTIVE_DATE^APP_EXPIRATION_DATE^AGENCY_LIST.AGENCY_DISPLAY_NAME^CSR^pcl.POLICY_LOB^MSCL.STATE_CODE^pcl.BILL_TYPE";
			
                objWebGrid.SearchColumnType = "T^D^D^T^T^L^T^T";//T^T^
				
                objWebGrid.DropDownColumns = "^^^^^LOB^^";//^^
			
				objWebGrid.OrderByClause = "APP_NUMBER ASC";
			
                objWebGrid.DisplayColumnNumbers = "4^10^7^8^9^3^17^16^20";//5^6^

                objWebGrid.DisplayColumnNames = "APP_NUMBER^EffectiveDate^ExpirationDate^Agency^CSR^quote_xml^LOB_DESC^STATE_CODE^BILL_TYPE";//^POLICY_NUMBER^QQ_NUMBER

                objWebGrid.DisplayColumnHeadings = objResourceMgr.GetString("DisplayColumnHeadings");//"App #^Eff. Date^Exp. Date^Agency^CSR^Rate^LOB^State^Bill Type";//^Policy #^Quick Quote #

                objWebGrid.DisplayTextLength = "40^20^20^20^20^20^20^20^20";//20^20^

                objWebGrid.DisplayColumnPercent = "20^7^7^10^8^8^12^5^5";//15^10^
                objWebGrid.ColumnTypes = "LBL^B^B^B^B^IMGLNK^B^B^B";//B^B^
                ...
                */
                ...
                objWebGrid.DropDownColumns = objResourceMgr.GetString("DropDownColumns");
                ...
                objWebGrid.ColumnsLink = objResourceMgr.GetString("ColumnsLink");

				objWebGrid.PrimaryColumns = "1";
                objWebGrid.PrimaryColumnsName = "pcl.APP_ID^pcl.CUSTOMER_ID^pcl.APP_VERSION_ID^pcl.POLICY_LOB";
                ...
                objWebGrid.ExtraButtons = objResourceMgr.GetString("ExtraButtons");//"1^Add New^0^addRecord";//"2^Add New~Delete App^0~1^addRecord~deleteApp"; //
				objWebGrid.PageSize = int.Parse (GetPageSize());
				objWebGrid.CacheSize = int.Parse (GetCacheSize());
				objWebGrid.ImagePath = System.Configuration.ConfigurationManager.AppSettings.Get("RootURL").Trim()+ "/images/collapse_icon.gif";
                objWebGrid.HImagePath = System.Configuration.ConfigurationManager.AppSettings.Get("RootURL").Trim() + "/images/expand_icon.gif";
                objWebGrid.HeaderString = objResourceMgr.GetString("HeaderString");//"Application Information" ;
