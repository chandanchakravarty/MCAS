<%@ Page language="c#" Codebehind="DivisionIndex.aspx.cs" AutoEventWireup="false" Inherits="Cms.CmsWeb.Maintenance.DivisionIndex" %>
  ...
  <HEAD>
		...
		function SetTabs(pretab,loadPage)
		{
			if (strSelectedRecordXML != "")
			{
				...
				var tree = objXmlHandler.quickParseXML(strSelectedRecordXML).getElementsByTagName('DIV_ID');
				var nodeName = objXmlHandler.quickParseXML(strSelectedRecordXML).getElementsByTagName('DivisionName');
				var nodeCode = objXmlHandler.quickParseXML(strSelectedRecordXML).getElementsByTagName('DivisionCode');
				var entityTyp = '';
				if(nodeName != null)
					entityTyp += nodeName[0].firstChild.text;
				if(nodeCode != null)
					entityTyp += '~' + nodeCode[0].firstChild.text;
					
//Changed By Amit K mishra for tfs bug #836
				if (tree != null)
				{
					//Url="AttachmentIndex.aspx?calledfrom=division&EntityType=Division&EntityId="+tree[0].firstChild.text + "&";
					Url="AttachmentIndex.aspx?calledfrom=division&EntityType=Division&EntityName=" + entityTyp +"&EntityId="+tree[0].firstChild.text + "&";
					//DrawTab(2,this,'',Url,null,pretab,! loadPage);
				}
				else
				{
					RemoveTab(2,this);
			...
			<table class="tableWidth" border="0" cellpadding="0" cellspacing="0">
				<tr>
					<td>
						...
						<input id="hidEntityDetail" type="hidden" name="hidEntityDetail" value="">
						</FORM>
					</td>
				</tr>
			</table>
