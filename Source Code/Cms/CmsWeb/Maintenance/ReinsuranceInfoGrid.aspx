<%@ Page language="c#" Codebehind="ReinsuranceInfoGrid.aspx.cs" AutoEventWireup="false" Inherits="Cms.CmsWeb.Maintenance.ReinsuranceInfoGrid" %>
...
<HTML>
	<HEAD>
		...
		var value;
			...
			{
			contr = document.getElementById("hidlocQueryStr").value;
			arrKeyValues = contr.split("=");
			value = arrKeyValues[1];
			window.open('CopyReinsuranceContractsInfo.aspx?CONTRACT_ID='+value,'Copy',600,300,'Yes','Yes','No','No','No');	
			}
			
		}
		
		//Called when user clicks on revise
		function onRevise(num,msDg)
		{
			if (document.getElementById("hidlocQueryStr").value == "")
			{
				alert("Please select the record.");
