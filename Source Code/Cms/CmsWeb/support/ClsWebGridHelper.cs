/******************************************************************************************
	...
	<Start Date				: -	> 25 Macrh 09
	<End Date				: - > 26 March 09
	<Description			: - > webgrid Class to to fetch Grid data
	<Review Date			: - >
	<Reviewed By			: - >
*/	
using System;
...
{
	/// <summary>
	/// Summary description for ClsWebGridHelper.
	/// </summary>
	public class ClsWebGridHelper : ClsCommon
	{
		public ClsWebGridHelper()
		{
            ClsMessages.SetCustomizedXml(ClsCommon.BL_LANG_CULTURE);
            gstrLblSearchBtn = ClsMessages.FetchGeneralMessage("1073"); //"Search";
            gstrLblAdvSearchBtn = ClsMessages.FetchGeneralMessage("1074"); //"Advanced Search";
            gstrLblBasicSearchBtn = ClsMessages.FetchGeneralMessage("1075"); //"Basic Search";
            ...
            gstrSearchOptionTxt = ClsMessages.FetchGeneralMessage("1076"); //"Search Option";
            sSearchCritTxt = ClsMessages.FetchGeneralMessage("1077"); //"Search Criteria";
            sErrorTxt = ClsMessages.FetchGeneralMessage("1078"); //"Some Error found";
            sNoMatchesTxt = ClsMessages.FetchGeneralMessage("1079"); //"No Matches found";
			sItemAnd = ClsMessages.FetchGeneralMessage("1371");//"And";
            sItemOr = ClsMessages.FetchGeneralMessage("1372");//"Or";
            sPage = ClsMessages.FetchGeneralMessage("1399"); //page
            sOf = ClsMessages.FetchGeneralMessage("1400"); //of
		}        
		...
		private void SetSession(string srchCrit,string pageID)
		{
			...
			if (HttpContext.Current.Session["SrchCrit"] == null) 
			{
				if(srchCrit!="")
				{
					if(!ht.Contains(pageID))  
						ht.Add(pageID,srchCrit); 
					HttpContext.Current.Session["SrchCrit"] = ht;
				}				
				else
					HttpContext.Current.Session["SrchCrit"] = ht;
			}
		...
		{
			ht =(Hashtable) HttpContext.Current.Session["SrchCrit"];
			string schcrit="";
        ...
        /// <summary>
        /// Creates Empty Dataset from search query columns
        /// </summary>
        /// <param name="sQueryGroup"></param>
        /// <returns>Empty Dataset</returns>
        /// Created by Charles on 23-Feb-2010 for Default Search Implementation
        private DataSet CreateDefaultDataset(string sQueryGroup)
        {
            DataSet lds = new DataSet();

            try
            {
                string[] sArrSelectClauses = HttpUtility.UrlDecode(sQueryGroup.Replace("+", "%2B")).Split(new char[] { SEPARATOR });
                string sSelectClauses = sArrSelectClauses[0];

                DataTable dt = new DataTable();

                foreach (string s in sSelectClauses.Split(','))
                {
		...
		{
			//decripting All Values	
            string message = Cms.CmsWeb.ClsMessages.FetchGeneralMessage("1192"); ;
            string message1 = Cms.CmsWeb.ClsMessages.FetchGeneralMessage("1193");
			sOrderClause=DecryptData(sOrderClause); 
			sDispColGroup=DecryptData(sDispColGroup);
			sPrimaryCols = DecryptData(sPrimaryCols) ;
			// end here
			...
			strLookUpDetails = HttpUtility.UrlDecode(lookUpDetails).ToString();

			string sSysDefined = "";
			DataTable lobj_GridTable=null;
			DataSet lds=null;
			
			//Added by Charles on 12-Jun-09 for Itrack 5968
			DataSet lds1=null;
			...
			try
			{
				...
				sdispTextLen=(string [])HttpUtility.UrlDecode(dispTextLen).Split(new char []{INTERNALSEPARATOR});
				strDisplayTextLen=(string [])HttpUtility.UrlDecode(dispTextLen).Split(new char []{INTERNALSEPARATOR});
				...
				string[] arrDropdown=HttpUtility.UrlDecode(DropdownCols).Split(new char[]{INTERNALSEPARATOR}); //split the display col group string
				int iDrp=0;
				...
				string[] sArrColGroup = HttpUtility.UrlDecode(sDispColGroup).Split(new char[]{SEPARATOR}); //split the display col group string
				...
				string[] sArrColType=HttpUtility.UrlDecode(sColumnType).Split(new Char[]{INTERNALSEPARATOR}); //Split the Column Type;

				strArrayColTypes = sArrColType;
				
				string[] sArrColsLink=HttpUtility.UrlDecode(sColsLink).Split(new Char[]{INTERNALSEPARATOR});//Split the Columns Link
				
				string[] sArrDispColsName=HttpUtility.UrlDecode(sDisplayColsName).Split(new Char[]{INTERNALSEPARATOR}); //Split the Column which will bind to the grid

				string[] sAddSpace=HttpUtility.UrlDecode(colHeader).Split(new char[]{INTERNALSEPARATOR});  
				...
				string[] sArrSearchGroup = HttpUtility.UrlDecode(sSearchColGroup).Split(new Char[]{SEPARATOR}); //split the search column group string
				...
				string[] sArrPrimaryCols = HttpUtility.UrlDecode(sPrimaryCols).Split(new Char[]{INTERNALSEPARATOR}); //split the primary column string
				sPrimCol=sPrimaryColsName;		
				//Get the values of the PK columns
				primaryKeyColumns = HttpUtility.UrlDecode(sPrimaryColsName).Split(new Char[]{INTERNALSEPARATOR});;
                ...
                int resultFunc = -1; //Added by Charles on 23-Feb-2010 for Default Search Implementation
                ...
                {
                    //Added by Charles on 23-Feb-2010 for Default Search Implementation
                    int dsindex = sSearchCriteria.IndexOf("*#)(");
                    if (dsindex > 0 || (sSearchCriteria == null || sSearchCriteria == ""))
                    {
                        lds = CreateDefaultDataset(sQueryGroup);
                        if (lds != null)
                        {
                            resultFunc = 0;
                            //Added by Charles on 10-May-2010 for Itrack 7304
                            try
                            {
                                string[] sArrFilterClauses = HttpUtility.UrlDecode(sFilterGroup.Replace("+", "%2B")).Split(new char[] { SEPARATOR });
                                sFilterCol = sArrFilterClauses[0].Trim();
                    ...
                    if (resultFunc == -1) //If condition added by Charles on 23-Feb-2010 for Default Search Implementation
                    {
                ...
                if (resultFunc == -1) //If condition added by Charles on 23-Feb-2010 for Default Search Implementation
                {
                    ...
                    {
                        SqlConnection lobjCon = new SqlConnection(ClsCommon.ConnGridStr);
                        SqlCommand lobjCmd = new SqlCommand();
                        SqlDataAdapter lobjDA = new SqlDataAdapter();

                        //Changed from lds to lds1 by Charles on 12-Jun-09 for Itrack 5968
                        lds1 = new DataSet();
                        ...
                        try
                        {
                            ...
                            //Changed by Charles on 17-Jun-09 for Itrack 5968. Selecting all records as searching by Encrypted Column.
                            lobjCmd.CommandText = sqlQuery.Replace("top " + Convert.ToString(int.Parse(totRecords) * iPageSize), "");

                            lobjCmd.CommandType = CommandType.Text;
                            lobjDA.SelectCommand = lobjCmd;

                            //Changed lds to lds1 & temp1 to temp by Charles on 12-Jun-09 for Itrack 5968
                            lobjDA.Fill(lds1, "temp");

                            //Changed from lds to lds1 by Charles on 12-Jun-09 for Itrack 5968
                            totalRec = lds1.Tables[0].Rows.Count;

                            //Changed from lds to lds1 by Charles on 12-Jun-09 for Itrack 5968
                            for (int index = lds1.Tables[0].Rows.Count - 1; index >= 0; index--)
                            {
                                //Changed from lds to lds1 by Charles on 12-Jun-09 for Itrack 5968
                                DataRow ldr = lds1.Tables[0].Rows[index];
                                ...
                                try
                                {
                                    ...
                                    {
                                        //Changed from lds to lds1 by Charles on 12-Jun-09 for Itrack 5968
                                        lds1.Tables[0].Rows[index].Delete();
                            ...
                            //Added by Charles for Itrack 5968 on 12-Jun-2009
                            if (lIntStart > 0)
                            {
                                pageNum = (lIntStart / iPageSize) + 1;
                            }
                            else
                                pageNum = 1;
                            ...
                            int upToRecords = 0;
                            int pageRowCount = lIntStart; //To display 'iPageSize' No. of Records

                            upToRecords = lIntStart + iPageSize;

                            if (upToRecords > totalRec) //Checking whether Total Records is less than upToRecords
                                upToRecords = totalRec;
                            int i = 0;
                            //Loop to select rows from lIntStart to upToRecords into dataset lds from lds1
                            for (int rowCount = 0; rowCount < lds1.Tables[0].Rows.Count; rowCount++)
                            {
                                if (lds1.Tables[0].Rows[rowCount].RowState == DataRowState.Deleted) continue; //Ignoring Deleted Rows
                                //Added for Itrack Issue 6925 on 13 Jan 
                                if (i != lIntStart)
                                {
                                    i++;
                                    continue;
                                ...
                                if (pageRowCount >= upToRecords) break;
                                if (rowCount >= lIntStart && pageRowCount < upToRecords)
                                {
                        ...
                        finally
                        {
                            ...
                            //Added by Charles for Itrack 5968 on 17-Jun-2009
                            if (lds1 != null) lds1.Dispose();
                        }
                    }
                }
				...
				if(decrypt_select != "")
				{
					foreach(DataRow ldr in lds.Tables[0].Rows)
					{
						try
						{
							//Added by Charles for Itrack 5968 on 16-Jun-2009 to ignore Deleted Rows.
							if(ldr.RowState== DataRowState.Deleted ) continue;
				...
				for (lIntMainCount=0; lIntMainCount < sArrColActualNames.GetLength(0); lIntMainCount++)
				{
					...
					switch(lstrColsType)
					{
						...
						case "HL":
							...
							lObjGrdColHL.NavigateUrl=HttpUtility.UrlEncode(lObjGrdColHL.DataNavigateUrlFormatString);
				...
				//Added by Charles on 18-Jun-09 for Itrack 5968
				if(iTotalPages>((int.Parse(totRecords) * iPageSize)/iPageSize)) 
					iTotalPages=(int.Parse(totRecords) * iPageSize)/iPageSize;

				if (iPageSize > 0 && iTotalPages > 0) 
				{
					lStrTotalPages=sPage+" " + iPageNo.ToString()+" " + sOf +" "+ iTotalPages.ToString()+""+"&nbsp;";
				...
				if(sSearch)
				{
					...
					if(HttpUtility.UrlDecode(sSearchCriteria).IndexOf(SEPARATOR)>0 )
					{
						string[] safterDecode =HttpUtility.UrlDecode(sSearchCriteria).Split(new char[]{SEPARATOR}); 
						if(safterDecode[1]=="*#)(")
					...
					if (!bAdvSearch) 
					{
						...
						if(HttpUtility.HtmlEncode(sSearchVal)=="*#)(")
							sSearchVal="";

						tmpTree.Append("&nbsp;&nbsp;<b>" + sSearchCritTxt + ":&nbsp;</b><span id=_bdgSearchDiv><input type=text onBlur=\"ChangeDate();FormatAmountSearchGrid()\" id=SearchVal size=20  value=\"" + HttpUtility.HtmlEncode(sSearchVal) + "\"></span>&nbsp;");
				...
				else
				{
					...
					if (!bAdvSearch) 
					{
						...
						tmpTree.Append("</select>");
						if (!bSearchColSelected) sSearchVal = "";
						tmpTree.Append("&nbsp;&nbsp;<b>" + sSearchCritTxt + ":&nbsp;</b><span id=_bdgSearchDiv><input type=text id=SearchVal size=20 class=SearchText onBlur=\"ChangeDate();FormatAmountSearchGrid()\" value=\"" + HttpUtility.HtmlEncode(sSearchVal) + "\"></span>&nbsp;<input type=\"button\" class=\"clsButton\" value=\""+gstrLblSearchBtn+"\" id=\"SearchButton\" onclick=\"initSearch(1, false);\">");
						tmpTree.Append("&nbsp;<input type=button class=clsButton value=\""+gstrLblAdvSearchBtn+"\"  onclick=\"getDefaultPage(1, true);\" size=3>");
					}
				...
				string []srhCriteria=HttpUtility.UrlDecode(sSearchCriteria.Replace("+", "%2B")).Split(new char[]{SEPARATOR});
				...
				else
				{
					if(srhCriteria.Length>1)
					{
						if(srhCriteria[1]=="*#)(")
						{
							if(lds.Tables[0].Rows.Count==0)
							{
                                lStrTotalPages = "<table id=\"pageTable\" cellspacing=0 cellpadding=0 border=0 width=100% align=center><tr class=" + strDataRowClass + "><td width=100% align=center colspan=5>" + message + "</td></tr></table>";			//Please click search button to perform search	
								tmpTree.Append(lStrTotalPages);	
							}
						}
						else
						{
							if(lds.Tables[0].Rows.Count==0)
							{
                                lStrTotalPages = "<table id=\"pageTable\" cellspacing=0 cellpadding=0 border=0 width=100% align=center><tr class=" + strDataRowClass + "><td width=100% align=center colspan=5>" + message1 + "</td></tr></table>";				
								tmpTree.Append(lStrTotalPages);	
							}                            
						}
					}
...
//								string secXML=Cms.CmsWeb.cmsbase.gstrSecurityXML;
							...
							{*/// for itrack no 1161
                          //  if (strArrayButtonText[buttonCounter] == strSave || strArrayButtonText[buttonCounter] == strAdd || strArrayButtonText[buttonCounter] == strAddNew || strArrayButtonText[buttonCounter] == strAddCustomer) 	
                            if ((strArrayButtonId[buttonCounter] == "0" || strArrayButtonId[buttonCounter] == "1" || strArrayButtonId[buttonCounter] == "2" || strArrayButtonId[buttonCounter] == "3" || strArrayButtonId[buttonCounter] == "4" || strArrayButtonId[buttonCounter] == "5" || strArrayButtonId[buttonCounter] == "6" || strArrayButtonId[buttonCounter] == "7" || strArrayButtonId[buttonCounter] == "8" || strArrayButtonId[buttonCounter] == "9") && strArrayButtonText[buttonCounter] != strDelete)
							{
								if(write=="Y")
								{
									tmpTree.Append("&nbsp;<input type=\"button\"  class=\"clsButton\" value=\"" + strArrayButtonText[buttonCounter] + "\" id=\"ExtraButton" +  buttonCounter + "\"  onclick=\"");
                            ...
                            else if (strArrayButtonText[buttonCounter] == strDelete)	                         
							{
								if(delete=="Y")
								{
									tmpTree.Append("&nbsp;<input type=\"button\"  class=\"clsButton\" value=\"" + strArrayButtonText[buttonCounter] + "\" id=\"ExtraButton" +  buttonCounter + "\"  onclick=\"");
		...
		private void GridBinding(object sender,DataGridItemEventArgs e)
		{
			...
			if (e.Item.ItemType  == ListItemType.Item || e.Item.ItemType  == ListItemType.AlternatingItem)
			{
				...
				if(grouping=="Y" || grouping == "Z")
				{
					...
					if(colGrps!="")
					{
						string [] arrMajor= HttpUtility.UrlDecode(colGrps).Split(new char[]{INTERNALSEPARATOR});
                ...
                //by praveer change ddl_9 to ddl_8 as a  column state is deleted in Cuastomer Asst. Policy Tab(itrack no 702)
				DropDownList ddl=(DropDownList) e.Item.Cells[0].FindControl("ddl_8");   

				if(ddl!=null)
				{
					Label lbl = (Label) e.Item.Cells[0].FindControl("lbl_8");   
					...
					if (((DataRowView)e.Item.DataItem).Row.Table.Columns.Contains("policy_status") == true )
					{
						if (((DataRowView)e.Item.DataItem).Row.Table.Columns.Contains("process_value") == true )
						{
							...
							else
							{
								...
								if (e.Item.Cells[0].FindControl("but_8") != null)
									((Button) e.Item.Cells[0].FindControl("but_8")).Visible = false; //by pravesh change but_8 to but_9 as a new column (Change Eff. date) is added in Cuastomer Asst. Policy Tab
                                //by praveer change ddl_9 to ddl_8 as a  column state is deleted in Cuastomer Asst. Policy Tab(itrack no 702)
				...
				{
					foreach (string str in HttpUtility.UrlDecode(strRightAlignCols).Split('^'))
					{
				...
				//if(isUnq=="")
				string[] lArrPrimaryColsName=HttpUtility.UrlDecode(sPrimCol).Split(new Char[]{INTERNALSEPARATOR});
				string[] pkValues=HttpUtility.UrlDecode(insertRowId).Split(new Char[]{INTERNALSEPARATOR});
		...
		private void UpdateCtrlHyperlink(Control ctrl,string queryString,string oldText, DataGridItemEventArgs e,int iMaj)
		{
			...
			if(hlk != null)
			{
                ...
                hlk.NavigateUrl = hlk.NavigateUrl.Replace("?","") + QueryStringModule.EncriptQueryString(queryString);
		...
		private string CreateQuery(string sQueryGroup, string sFilterGroup, bool bShowExcluded, string sSearchCriteria, string sDefaultSortCol, out string sFilterCol, out string sFilterLabel, out string sColName, out bool bAsc, out string sSearchCol, out string sSearchVal,out string sSQLQuery,string sSearchName,int iPageNo,string sPrimaryColsName,string sCacheSize,string uniqId,string lst,string strFilterValue,string searchType,string [] sSearchArrCol) 
		{
			...
			try 
			{
				...
				if (sFilterGroup != null && HttpUtility.UrlDecode(sFilterGroup.Replace("+", "%2B")).IndexOf(SEPARATOR) > 0)
				{
					string[] sArrFilterClauses = HttpUtility.UrlDecode(sFilterGroup.Replace("+", "%2B")).Split(new char[]{SEPARATOR});
				...
				string[] sArrSearchType = HttpUtility.UrlDecode(searchType).Split(new char[]{INTERNALSEPARATOR}); //split the display col group string
				...
				if (sSearchCriteria != null && HttpUtility.UrlDecode(sSearchCriteria.Replace("+", "%2B")).IndexOf(SEPARATOR) > 0)
				{
					//code added by Krishan to store the data in a table.
					sDefaultSearchString=HttpUtility.UrlDecode(sSearchCriteria.Replace("+", "%2B"));
					
					string[] sArrSearchCriteria = HttpUtility.UrlDecode(sSearchCriteria.Replace("+", "%2B")).Split(new char[]{SEPARATOR});
				...
				string[] lArrPrimaryColsName=HttpUtility.UrlDecode(sPrimaryColsName).Split(new Char[]{INTERNALSEPARATOR});
				...
				sQueryGroup = DecryptData(sQueryGroup.Trim());
				string[] sArrQueryClauses = HttpUtility.UrlDecode(sQueryGroup.Replace("+", "%2B")).Split(new char[]{SEPARATOR});
		...
		private string CreateAdvSearch(string[] sArrSearchCols, string[] sArrSearchHeads, string sSearchCriteria, int iRowNos) 
		{
			...
			string[] sTmpArray = HttpUtility.UrlDecode(sSearchCriteria.Replace("+", "%2B")).Split(new char[]{SEPARATOR});
			...
			for (int i=1; i<=iRowNos; i++) 
			{
				if(i==1)
				{
					...
					if(HttpUtility.HtmlEncode(sArrSearchCriteria[(i-1)*3 + 1])!="*#)(")
						tmpSearch.Append("<TD NOWRAP><span id=\"_bdgAdvSearchDiv\">&nbsp;<input type=\"text\" id=\"AdvSearchVals\" size=\"15\" onBlur=\"ChangeDate()\" class=\"SearchText\" value=\"" + HttpUtility.HtmlEncode(sArrSearchCriteria[(i-1)*3 + 1]).Trim() + "\"></span></TD>\n");
					else
		...
		public DataTable ExecuteInlineQuery(string pStrSQL)
		{
			...
			SqlConnection lobjCon=new SqlConnection(ClsCommon.ConnGridStr);//System.Configuration.ConfigurationManager.AppSettings["DB_CON_STRING"].ToString());
			SqlCommand lobjCmd=new SqlCommand();
			SqlDataAdapter lobjDA=new SqlDataAdapter();
			try
			{
		...
		public DataSet ExecuteInlineGridRecords(string pStrSQL,int pIntStartRecord,int pIntPageSize,out int totRec,out int pageNum,string sPrimaryCols)
		{
			...
			//SqlConnection lobjCon=new SqlConnection(System.Configuration.ConfigurationManager.AppSettings["DB_CON_STRING"].ToString());
			//added by Pravesh use user with only select rights in DB
			SqlConnection lobjCon=new SqlConnection(ClsCommon.ConnGridStr);//System.Configuration.ConfigurationManager.AppSettings["DB_WEBGRID_CON_STRING"].ToString());
			SqlCommand lobjCmd=new SqlCommand();
			SqlDataAdapter lobjDA=new SqlDataAdapter();
			try
			{
				...
				string[] lArrPrimaryColsName=HttpUtility.UrlDecode(sPrimaryCols).Split(new Char[]{INTERNALSEPARATOR});
				string[] pkValues=HttpUtility.UrlDecode(insertRowId).Split(new Char[]{INTERNALSEPARATOR});
				...
				for (int rowCount = 0;rowCount < lds1.Tables[0].Rows.Count; rowCount++)
				{
					//Added by Charles on 17-Jun-09 for Itrack 5968
					if(rowCount>upToRecords)break;
					...
					if (sArrSearchCriteria.Length == 2) //only a basic search
					{
						...
						if(sArrSearchType.Length>0)
						{
							if(sArrSearchType[iSrchCol]=="T" || sArrSearchType[iSrchCol]=="P")
								...
								//Done for Itrack Issue 6853 on 14 Dec 09
								...
								sSearchClause = "((REPLACE(" + sArrSearchCriteria[0].Replace("!","+").Trim() + ",' ','') LIKE ('%" + sArrSearchCriteria[1].Replace("!","+").Trim().Replace("'", "''").Replace(" ","") + "%')) OR (REPLACE(" + sArrSearchCriteria[0].Replace("!","+").Trim() + ",' ','') LIKE ('%" + sArrSearchCriteria[1].Replace("!","+").Trim().Replace("'", "''").Replace(" ","%") + "%')) OR (REPLACE(" + sArrSearchCriteria[0].Replace("!","+").Trim() + ",' ','') LIKE REPLACE('%" + sArrSearchCriteria[1].Replace("!","+").Trim().Replace("'", "''").Replace(" ","") + "%',',','')))";
							else if (sArrSearchType[iSrchCol]=="D")
							{
								string [] twodates = sArrSearchCriteria[1].Trim().Replace("'", "''").Split('-');
                                //Added By Pradeep Kushwaha for Multilingual on 28-Feb-2011
                                String toDate = String.Empty;
                                String toDate1 = String.Empty;
								if(twodates.Length > 1)
								{
                                        try {
                                            if (ClsCommon.BL_LANG_ID == 2 || ClsCommon.BL_LANG_ID == 3)
                                        {
                                            if (twodates[0].ToString() != "")
                                                toDate = FormatDateInNeutralculture(twodates[0].ToString(),false);
                                            if (twodates[1].ToString() != "")
                                                toDate1 = FormatDateInNeutralculture(twodates[1].ToString(),true);
                                        }
                                        else
                                        {
                                            twodates[1] = Convert.ToDateTime(twodates[1]).AddDays(1).AddMilliseconds(-1).ToString();
									...
									//sSearchClause = "((" + sArrSearchCriteria[0].Trim() + " >= '" + twodates[0]  + "' AND " + sArrSearchCriteria[0].Trim() + " <= '" + twodates[1]  + "'))";
                                    sSearchClause = "((" + sArrSearchCriteria[0].Trim() + " >= '" + toDate + "' AND " + sArrSearchCriteria[0].Trim() + " <= '" + toDate1 + "'))";
                                    //Added till here 
								}
							}
							...
							if(sArrSearchType.Length>0)
								...
								else if(sArrSearchType[iSrchCol]=="D")
								{
									...
									if(twodates.Length > 1)
									{
                                        ...
                                        //sSearchClause += "((" + sArrSearchCriteria[i].Trim() + " >= '" + twodates[0]  + "' AND " + sArrSearchCriteria[i].Trim() + " <= '" + twodates[1]  + "'))" + sArrSearchCriteria[i+2].Trim();
                                        sSearchClause += "((" + sArrSearchCriteria[i].Trim() + " >= '" + toDate + "' AND " + sArrSearchCriteria[i].Trim() + " <= '" + toDate1 + "'))" + sArrSearchCriteria[i + 2].Trim();
                                        //Added till here 
        ...
        /// <returns></returns>
        // Added By Pradeep Kushwaha on 28-Feb-2011
        private String FormatDateInNeutralculture(String datetime, Boolean RequireTime)
        {
            ...
            try
            {
                if (ClsCommon.BL_LANG_ID == 2)
                {
                    IFormatProvider provider = new System.Globalization.CultureInfo("pt-BR", true);
                    ...
                    if (datetime != "")
                    {
                        ...
                        RetDate = dt.ToShortDateString();
                        if(RequireTime)
                        RetDate = Convert.ToDateTime(RetDate).AddDays(1).AddMilliseconds(-1).ToString();

                    }
                }
                else if (ClsCommon.BL_LANG_ID == 3)
                {
                    IFormatProvider provider = new System.Globalization.CultureInfo("zh-SG", true);
	...
	public class CreateItemTemplateBUT : ITemplate
	{
		...
		public void InstantiateIn(Control objContainer)
		{
			...
			but.ID=strControlId;
            but.Text = ClsCommon.BL_LANG_ID == 2 ? "Ir" : "GO"; //"GO"; 
