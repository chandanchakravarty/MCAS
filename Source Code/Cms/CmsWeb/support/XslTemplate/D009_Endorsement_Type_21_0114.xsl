...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform"  xmlns:msxsl="urn:schemas-microsoft-com:xslt"  xmlns:user="urn:user-namespace-here"    xmlns:clitype="clitype" xmlns:fn="http://www.w3.org/2005/xpath-functions" xmlns:iso4217="http://www.xbrl.org/2003/iso4217" xmlns:ix="http://www.xbrl.org/2008/inlineXBRL" xmlns:java="java" xmlns:link="http://www.xbrl.org/2003/linkbase" xmlns:xbrldi="http://xbrl.org/2006/xbrldi" xmlns:xbrli="http://www.xbrl.org/2003/instance" xmlns:xlink="http://www.w3.org/1999/xlink" xmlns:xs="http://www.w3.org/2001/XMLSchema"    xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xmlns:altova="http://www.altova.com" exclude-result-prefixes="clitype fn iso4217 ix java link xbrldi xbrli xlink xs xsi altova">
  ...
  <xsl:template match="/">
    <html >
		<!--There is no modification , file is edited  to change status of tfs id-->
      <body>
            ...
            <tbody>
              <xsl:for-each select="POLICY_DOCUMENTS">
                ...
                <xsl:for-each select="RISKINFO">
                    ...
                    <tr style="height:0.18in; ">
                       ...
                       <td style="text-align:left; ">
                        ...
                        <span style="font-family:Arial; font-size:12pt ">
                          <xsl:for-each select="../../RISK_LOCATION">
                            <xsl:for-each select="PREV_VERSION">
										<xsl:choose>											
											<xsl:when test="RISK_ID = user:GetRiskID()">
                        ...
                        <xsl:choose>
                          ...
                          <xsl:otherwise>
							  <xsl:choose>
								  <xsl:when test="PRE_CLASS_FIELD != CURRENT_CLASS_FIELD">
									  <br></br>
									  <span style="font-family:Arial; font-size:12pt ">
                        ...
                        <!--<table border="0"  width="100%" cellpadding="0" cellspacing="0">
								...
								<xsl:for-each select="DISCOUNT">
									...
									<xsl:choose>
										<xsl:when test="user:GetRiskID() =  RISK_ID and IS_PRINT = '1'">
											<xsl:value-of select="user:SetIsPrintD(IS_PRINT)"/>
										</xsl:when >
										<xsl:otherwise>
											--><!--<xsl:value-of select="user:GetIsPrintD()"/>--><!--
										</xsl:otherwise>
									</xsl:choose >
								</xsl:for-each>
							</xsl:for-each>
							--><!--<xsl:value-of select="user:GetIsPrintD()"/>--><!--
                          <xsl:choose>
                            <xsl:when test ="user:GetIsPrintD() &gt; 0">
                              <tr>
                                <td>
                                  ...
                                  <xsl:when test="user:GetRiskID() =  RISK_ID and IS_PRINT = '1'">
                                    <tr>
                                      <td style="width:33%;vertical-align:top" align="left" >
                                        <span style="font-family:Arial; font-size:12pt">
											<xsl:if test ="DISCOUNT_DESCRIPTION != ''">
												<br></br>
                        ...
                        <BR></BR>
						  <xsl:choose>
							  <xsl:when test="string(PRE_REMARKS) != string(CURRENT_REMARKS) or (string(PRE_REMARKS) = '' and string(CURRENT_REMARKS) != '')">
                        <span  id="relaco"  style="font-family:Arial; font-size:12pt ">
                          RELAÇÃO DE OBJETOS:&#160;<br></br><br></br>
				  ...
				  <!--This is applied to remove blank age issue.modified by naveen-->
				  <xsl:if test="POLICY_DOCUMENTS/MULTIBROKER/POLR/IS_PRINT='1'">
					  <xsl:for-each select="$XML">
