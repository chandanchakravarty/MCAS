...
{
	/// <summary>
	/// Summary description for SubmitQuation.
	/// </summary>
	public class SubmitQuestion : Cms.CmsWeb.cmsbase  
	{
        ...
        private void Page_Load(object sender, System.EventArgs e)
		{
            ...
            try
            {
                ...
                ((BaseDataGrid)c1).SelectClause ="QM.QID,QM.QDESC,QM.QUESTIONTYPEID,QM.ISACTIVE,CONVERT(VARCHAR(15),QM.LASTMODIFIEDDATE,101) LASTMODIFIEDDATE,UL.USER_FNAME + ' ' + UL.USER_LNAME NAME,QM.GROUPID,QM.SCREENID,QM.TABID";
                //specifying tables for from clause
                if(lStrGroupID.Trim()=="")
                {
                    ((BaseDataGrid)c1).FromClause=" USERQUESTIONS QM LEFT OUTER JOIN MNT_USER_LIST UL ON UL.USER_ID = QM.LASTMODIFIEDBY INNER JOIN QUESTIONTABMASTER QT ON QT.TABID=QM.TABID AND QT.SCREENID=QM.SCREENID  ";							
                }
                else
                {
                     ((BaseDataGrid)c1).FromClause=" QUESTIONGROUPMASTER QGM RIGHT OUTER JOIN  USERQUESTIONS QM ON QGM.GROUPID=QM.GROUPID AND QGM.TABID=QM.TABID AND QGM.SCREENID=QM.SCREENID LEFT OUTER JOIN MNT_USER_LIST UL ON UL.USER_ID = QM.LASTMODIFIEDBY ";
                }

                if(lStrGroupID.Trim()=="")
                {
                    ((BaseDataGrid)c1).WhereClause="QM.TABID=" + hidTabID.Value + " AND QM.SCREENID=" + hidTemplateID.Value;  
                }
                else
                {
                     ((BaseDataGrid)c1).WhereClause="QM.TABID=" + hidTabID.Value + " AND QM.GROUPID=" + hidGroupID.Value + " AND QM.SCREENID=" + hidTemplateID.Value;
                }

                
                
                //specifying Text to be shown in combo box
                ((BaseDataGrid)c1).SearchColumnHeadings="Question Description^Amended On^Amended By";
                //specifying column to be used for combo box
                ((BaseDataGrid)c1).SearchColumnNames="QM.QDESC^QM.LASTMODIFIEDDATE^NAME";
                ...
                ((BaseDataGrid)c1).OrderByClause="QDESC asc";
                ...
                ((BaseDataGrid)c1).DisplayColumnNames="QDESC^LASTMODIFIEDDATE^NAME";
                //specifying text to be shown as column headings
                ((BaseDataGrid)c1).DisplayColumnHeadings="Question Description^Amended On^Amended By";
                ...
                ((BaseDataGrid)c1).PrimaryColumnsName="QM.QID^QM.GROUPID^QM.SCREENID^QM.TABID";
                ...
                if(lStrGroupID=="")
                    ((BaseDataGrid)c1).ExtraButtons="3^Add New~Preview Questions~Back To Tab^0~1~2^addRecord~fnPreviewQuestion~fnMovePrev";
                else
                    ((BaseDataGrid)c1).ExtraButtons="3^Add New~Preview Questions~Back To Group^0~1~2^addRecord~fnPreviewQuestion~fnMovePrev";
                ...
                //specifying heading
                ((BaseDataGrid)c1).HeaderString = "Search Questions";
                ...
                if(lStrGroupID=="")
                    ((BaseDataGrid)c1).QueryStringColumns ="QID^GROUPID";
                else
                    ((BaseDataGrid)c1).QueryStringColumns ="QID";
                ((BaseDataGrid)c1).DefaultSearch="Y";  
 
				
				((BaseDataGrid)c1).FilterLabel="Include Inactive";
				((BaseDataGrid)c1).FilterColumnName="QM.ISACTIVE";
				((BaseDataGrid)c1).FilterValue="Y";
            ...
            if(lStrGroupID!="")
                TabCtl.TabURLs = @"GridQuestion.aspx?ScreenID=" +  lStrTemplateID + "&TabID=" + lStrTabID + "&GroupID=" + lStrGroupID + "&CalledFrom=" + lStrCalledFrom + "&";
            else
                TabCtl.TabURLs = @"GridQuestion.aspx?ScreenID=" +  lStrTemplateID + "&TabID=" + lStrTabID + "&CalledFrom=" + lStrCalledFrom + "&";
