...
{
	/// <summary>
	/// Summary description for SubmitTab.
	/// </summary>
	public class SubmitTab : Cms.CmsWeb.cmsbase  
	{
		...
		{
			base.ScreenId="128_1";
            objResourceMgr = new ResourceManager("Cms.CmsWeb.User_Defined.SubmitTab", Assembly.GetExecutingAssembly());
            ...
            try
            {
                ...
                ((BaseDataGrid)c1).SelectClause ="TM.TABID,TM.TABNAME,TM.ISACTIVE,CONVERT(VARCHAR(15),TM.LASTMODIFIEDDATE,101) LASTMODIFIEDDATE,UL.USER_FNAME + '' + UL.USER_LNAME NAME,TM.SCREENID";
                //specifying tables for from clause
                ((BaseDataGrid)c1).FromClause="QUESTIONTABMASTER TM LEFT OUTER JOIN MNT_USER_LIST UL ON UL.USER_ID = TM.LASTMODIFIEDBY ";
                //specifying conditions for where clause
                ((BaseDataGrid)c1).WhereClause=" TM.SCREENID =" +  lStrTemplateID ;
                //specifying Text to be shown in combo box
                ((BaseDataGrid)c1).SearchColumnHeadings = objResourceMgr.GetString("SearchColumnHeadings");//"Tab Name^Amended On^Amended By";
                //specifying column to be used for combo box
                ((BaseDataGrid)c1).SearchColumnNames="TM.TABNAME^TM.LASTMODIFIEDDATE^NAME";
                ...
                ((BaseDataGrid)c1).OrderByClause="TABNAME asc";
                //specifying column numbers of the query to be displyed in grid
                ((BaseDataGrid)c1).DisplayColumnNumbers="2^4^5";
                //specifying column names from the query
                ((BaseDataGrid)c1).DisplayColumnNames="TABNAME^LASTMODIFIEDDATE^NAME";
                //specifying text to be shown as column headings
                ((BaseDataGrid)c1).DisplayColumnHeadings = objResourceMgr.GetString("DisplayColumnHeadings");//"Tab Name^Amended On^Amended By";
                ...
                ((BaseDataGrid)c1).PrimaryColumnsName="TM.TABID";
                ...
                ((BaseDataGrid)c1).ExtraButtons = objResourceMgr.GetString("ExtraButtons");//"6^Add New~Groups~Questions~Change Question Order~Preview Questions~Back To Screen^0~1~2~3~4~5^addRecord~fnSubmitTab~fnSubmitQues~fnChangeQuesOrder~fnPreviewQuestion~fnMovePrev";//objResourceMgr.GetString("ExtraButtons");//
                ...
                //specifying heading
                ((BaseDataGrid)c1).HeaderString = objResourceMgr.GetString("HeaderString");//"Search Tab";
                ...
                ((BaseDataGrid)c1).QueryStringColumns ="TABID";
            ...
            TabCtl.TabURLs = @"TabDetails.aspx?ScreenID=" +  lStrTemplateID + "&";
