...
{
	/// <summary>
	/// Summary description for UserQuestion.
	/// </summary>
	public class UserQuestion : Cms.CmsWeb.cmsbase 
	{
		...
		private string lstrCarrier;
		private string lstrAppId;
		private string lstrAppVerId;
		private string lstrClient;


		private void Page_Load(object sender, System.EventArgs e)
		{
			base.ScreenId = "197_0_0";
			//Checking whether policy for the application exist or not
            ...
            if (ds.Tables[0].Rows[0]["POLICY_NUMBER"].ToString() != "")
			{
            ...
            */
			...
			try
			{
				...
				lstrCarrier = "1";
				lstrClient = GetCustomerID();
                lstrAppId = GetPolicyID();// GetAppID();
                lstrAppVerId = GetPolicyVersionID();// GetAppVersionID();
				
					

				lStrResourceName = "Cms.CmsWeb.UserDefined.PreviewUserQuestion";		//The fully qualified class name.
				...
				lstrSaveMessage= aobjResMang.GetString("msgsave");
				btnSave.Text= "Save";
				gstrScreenID = Request["ScreenID"];
				gstrTabID = Request["TabID"];

				//adding the back button logic  to go back wards in case of multiple screens
				if (Request.QueryString["backbutton"] != null && Request.QueryString["backbutton"].Trim().ToLower() ==  "true")
				{
					btnBack.Text = "Back";
				...
				if(!IsPostBack)
				{
					...
					lblhidScreenID.Text = gstrScreenID;
				}
				...
				fnGenerateHtml(lstrCarrier,lstrClient,lstrAppId,lstrAppVerId);
			}
			catch(Exception ex)
			{
				throw(ex);
		...
		public string fnGenerateHtml(string psCarrierId,string lstrClient,string lstrAppId,string lstrAppVerId)
		{
			...
			dsUserQuestion = objUserQuestion.GetUserQuestion(lstrClient,lblTabID.Text,lblhidScreenID.Text,lstrAppId,lstrAppVerId);

			if (dsUserQuestion.Tables.Count > 0  && dsUserQuestion.Tables[0].Rows.Count > 0)
			{
				gStrInsertUpdateFlag="update";
				...
				//****************************Case of Update*****************************
				//use of dsUserQuestion dataset to fetch the records

				lArrayQues=new string[dsUserQuestion.Tables[0].Rows.Count];
				lArrayAns =new string[dsUserQuestion.Tables[0].Rows.Count];
				...
				foreach(DataRow ldrRow in dsUserQuestion.Tables[0].Rows)
				{
					...
					lsQuesAnswer			= ldrRow["ANSWER"].ToString();
					lsOptAnswer				= ldrRow["OPTIONVALUE"].ToString();
					lsOptionAnswerID		= ldrRow["OPTIONID"].ToString();
					lsOthAnswerDesc			= ldrRow["DESCANSWER"].ToString();
					...
					//for transaction log 
					lArrayQues[iQuestionCtr] = lsQuestionDesc;
					
					if(lsQuestionType=="9")
					{
						if(lblPrevQID.Text!=liQuestionID.ToString())
						{
							lblPrevQID.Text = liQuestionID.ToString();
			...
			else
			{
				...
				dsUserQuestion = objUserQuestion.GetTabQuestion(lblTabID.Text,lblhidScreenID.Text,psCarrierId);
				if(dsUserQuestion.Tables[0].Rows.Count == 0 )
				{
		...
		public void PrepareHTML(string psQuestionType,string piQuestionID,string psReq,string psQuestionDesc,string psSuffix,string psPrefix,string psgroupid,string psQNotes,string psListID,string psflgOtherQuess,string psOthQuesDesc,string psOthQuesCompareVal,string psGroupType,string psListType,string psListTable,string psControlStyle,string psControlValidationType,string psControlMaxLength,string psControlSpan,string psQuesAnswer,string psOptAnswer,string psOptionAnswerID,string psOthAnswerDesc,string lsDepQuestionText,string lsDepQuesType,string lsDepAnsType,string lsDepQuesList)
		{
			...
			if(psgroupid!="" && psGroupType=="G" && GroupAdded != psgroupid)
			{
				...
				trBlankRow.CssClass="headerEffectSystemParams";
			...
			tcBlankCell.Width =Unit.Percentage(1);
			...
			else
			{
				...
				tcQuestion.Width =Unit.Percentage(100/((MaxCols+1)/2));
			...
			switch (Question_Type)
			{
				case 1:
					...
					if(lsListId.Trim()!="")
					{
						...
						//in case of update .Updating the answer
						if (psOptionAnswerID != "")
						{
							ListItem liOptionAnswer =  cboUserList.Items.FindByValue(lsListId + "#" + psOptionAnswerID);
							if (liOptionAnswer != null)
							{
								cboUserList.SelectedIndex=-1;
								liOptionAnswer.Selected=true;
								lArrayAns[iQuestionCtr] = liOptionAnswer.Value;
							}
						}
						//***************************************
					...
					if(lsOtherFlag == "Y" && lsDepQuesType.Trim() != "")
					{
						...
						lblOthDesc.Text="<br>" + lsDepQuestionText + "&nbsp;";
						...
						switch(liDepQuesType)
						{
							case 1:
								...
								if (psOthAnswerDesc.Trim() != "" && ((DropDownList)baseControl).Items.FindByValue(psOthAnswerDesc) != null)
								{
									((DropDownList)baseControl).SelectedIndex=-1;
									((DropDownList)baseControl).Items.FindByValue(psOthAnswerDesc).Selected=true;
								}
								break;
							case 2:
								...
								if (psOthAnswerDesc.Trim() != "")
								{
									((ListBox)baseControl).SelectedIndex=-1;
									string[] lsAnswers = psOthAnswerDesc.Split(',');
									foreach(string lsSelectedAnswer in lsAnswers)
									{
										if (((ListBox)baseControl).Items.FindByValue(psOthAnswerDesc) != null)
										{
											((ListBox)baseControl).Items.FindByValue(psOthAnswerDesc).Selected=true;
										}
									}
								}
							...
							case 4:
								...
								if (psOthAnswerDesc.Trim() != "")
								{
									((TextBox)baseControl).Text= psOthAnswerDesc;
								}
								break;
							case 5:
								baseControl = new TextBox();
							...
							case 13:
								...
								if (psOthAnswerDesc.Trim() != "" && ((RadioButtonList)baseControl).Items.FindByValue(psOthAnswerDesc) != null)
								{
									((RadioButtonList)baseControl).SelectedIndex=-1;
									((RadioButtonList)baseControl).Items.FindByValue(psOthAnswerDesc).Selected=true;
								}
							...
							case 14:
								...
								if (psOthAnswerDesc.Trim() != "")
								{
									((CheckBoxList)baseControl).SelectedIndex=-1;
									string[] lsAnswers = psOthAnswerDesc.Split(',');
									foreach(string lsSelectedAnswer in lsAnswers)
									{
										if (((CheckBoxList)baseControl).Items.FindByValue(psOthAnswerDesc) != null)
										{
											((CheckBoxList)baseControl).Items.FindByValue(psOthAnswerDesc).Selected=true;
										}
									}
								}
								break;
						...
						if (psOthAnswerDesc.Trim() != "")
						{
							lArrayAns[iQuestionCtr] = psOthAnswerDesc;
						}
						else
						{
				...
				case 2:
					...
					//adding the answer
					if(psOptionAnswerID.Trim()!= "")						
					{
						ListControl.SelectedIndex=-1;
						lArrayAns[iQuestionCtr] = psOptionAnswerID;

						string[] laAnswers = psOptionAnswerID.Split(new char[]{','});
						foreach(string lsAnswer in laAnswers)
						{
							ListItem liAnswer =  ListControl.Items.FindByValue(lsListId + "#" +  lsAnswer);
							if (liAnswer != null)
							{
								liAnswer.Selected=true;
				...
				case 4:			
					...
					if (psQuesAnswer != "")
					{
						lArrayAns[iQuestionCtr] = psQuesAnswer;
						txtControl.Text = psQuesAnswer;
					}
				...
				case 9:
					...
					{	
						...
						{	
							...
							lstrGridAnswer  = objUserQuestion.GetUserHorizontalAnswerClient(int.Parse(QuestionID),liGridOptId,iHGCtr,lstrClient,lblTabID.Text,lstrAppId,lstrAppVerId,lsCarrierID,lblhidScreenID.Text);
							...
							switch(lintGridOptTypeID)
							{
								case 1:		
									...
									if (lstrGridAnswer != "")
									{
										ListItem liOptionAnswer =  cboUserList.Items.FindByValue(lsListId + "#" + lstrGridAnswer);
										if (liOptionAnswer != null)
										{
											cboUserList.SelectedIndex=-1;
											liOptionAnswer.Selected=true;
								...
								case 2:
									...
									//adding the answer
									if(lstrGridAnswer.Trim()!= "")						
									{
										ListControl.SelectedIndex=-1;
										lArrayAns[iQuestionCtr] = lstrGridAnswer;

										string[] laAnswers = lstrGridAnswer.Split(new char[]{','});
										foreach(string lsAnswer in laAnswers)
										{
											ListItem liAnswer =  ListControl.Items.FindByValue(lsListId + "#" +  lsAnswer);
											if (liAnswer != null)
								...
								case 4:
									...
									{
										lArrayAns[iQuestionCtr] = lstrGridAnswer;
										txtControl.Text = lstrGridAnswer;
									}
				...
				case 8:
					...
					foreach(DataRow ldrVGridQuestion in VerticalArray)
					{
						...
						{	
							...
							lsVAnswer = objUserQuestion.GetUserHorizontalAnswerClient(int.Parse(QuestionID),liVGridOptId,liVHGridOptId,lstrClient,lblTabID.Text,lstrAppId,lstrAppVerId,lsCarrierID,lblhidScreenID.Text);
							...
							switch(liVGridOptTypeId)
							{
								case 1:
									...
									if (lsVAnswer != "")
									{
										ListItem liOptionAnswer =  cboUserList.Items.FindByValue(lsListId + "#" + lsVAnswer);
										if (liOptionAnswer != null)
										{
											cboUserList.SelectedIndex=-1;
											liOptionAnswer.Selected=true;
								...
								case 2:
									...
									{
										ListControl.SelectedIndex=-1;
										lArrayAns[iQuestionCtr] = lsVAnswer;

										string[] laAnswers = lsVAnswer.Split(new char[]{','});
										foreach(string lsAnswer in laAnswers)
										{
											ListItem liAnswer =  ListControl.Items.FindByValue(lsListId + "#" +  lsAnswer);
											if (liAnswer != null)
				...
				case 13:
					...
					if (psOptionAnswerID != "")
					{
						ListItem liOptionAnswer =  rbList.Items.FindByValue(lsListId + "#" + psOptionAnswerID);
						if (liOptionAnswer != null)
						{
				...
				case 14:
					...
					if(psOptionAnswerID.Trim()!= "")						
					{
						...
						foreach(string lsAnswer in laAnswers)
						{
							ListItem liAnswer =  chkBoxList.Items.FindByValue(lsListId + "#" +  lsAnswer);
							if (liAnswer != null)
							{
								liAnswer.Selected=true;
		...
		{
			Response.Write("<script language='javascript'>parent.location.href='questiontab.aspx'</script>");
			Response.End();
		}
		
		private void btnSave_Click(object sender, System.EventArgs e)
		{
			string lstrQuestionText="";
			string lsrtUserListID="";
			string lsrtUserOptionListID="";
			string lstrUserQuestionID="";
			string lstrUserHOptionID="";
			string lstrUserVOptionID="";
			string lstrUserAnswerType="";
			string lintClientID ="";			
			string lstrOthAnswerDescription="";
			string lstrOthAnswerControlID="";
			int ReturnValue=0;			
			string gStrPolicyID="";
			string lstrPolicyVersion = "";
			//string lstrProposalVersion="";
			string strCarrier="";
			string[] lArrayQuestions=null;
			string[] lArrayAnswers=null;
			//string strBrokerID="";
			string lStrControlName="";
			try
			{
				 
				lintClientID=GetCustomerID();
                gStrPolicyID = GetPolicyID(); //GetAppID();							
                lstrPolicyVersion = GetPolicyVersionID(); //GetAppVersionID();							
				strCarrier="1";
				
				
				//deleting old records
				gstrCombineQID = lblComIds.Text;
				string[] strarrystring = gstrCombineQID.Split('#');

				int ReturnStatus = objUserQuestion.DeleteData(lintClientID,lblTabID.Text,gStrPolicyID,lstrPolicyVersion,lblhidScreenID.Text);

				lArrayQuestions =new string[strarrystring.Length];
				lArrayAnswers =new string[strarrystring.Length];
					
				for(int i=0; i<=strarrystring.Length-1;i++)
				{
					lstrQuestionText = Request.Form[strarrystring[i]];
					lStrControlName  = strarrystring[i];
					lsrtUserListID	 = "";
					lsrtUserOptionListID = "";
					if(strarrystring[i]!="")
					{
						//Array to store answer desc and question ID tom implement Transaction log.
						string [] lsrtArrayIds = strarrystring[i].Split(new char[]{'_'});
						lstrUserQuestionID = lsrtArrayIds[1];
						lstrUserHOptionID = lsrtArrayIds[2];
						lstrUserVOptionID = lsrtArrayIds[3];
						lstrUserAnswerType = lsrtArrayIds[4];
						if (lstrUserAnswerType.IndexOf("~") != -1)
						{
							lstrUserAnswerType = lstrUserAnswerType.Split('~')[0];
						}

						lstrOthAnswerControlID = "AnswDesc_"+lstrUserQuestionID+"_0_0_"+lstrUserAnswerType;
						lstrOthAnswerDescription = Request[lstrOthAnswerControlID];
						lArrayQuestions[i]	=lstrUserQuestionID+ "$" + lstrUserHOptionID+ "$" +lstrUserVOptionID;
						
						if(lstrUserAnswerType == "1")	
						{
							///splitting the List ID and Option List ID.
							
							if(lstrQuestionText.Trim()!="")
							{
								string [] lsrtListArrayIds = lstrQuestionText.Split(new char[]{'#'});
								lsrtUserListID = lsrtListArrayIds[0];
								lsrtUserOptionListID = lsrtListArrayIds[1];
							}
							else
							{
								lsrtUserListID="";
							...
							if(lstrUserHOptionID!="0" || lstrUserVOptionID !="0")
							{
								//lArrayAnswers[i]=lsrtUserOptionListID;
								DropDownList Temp=new DropDownList();
								Temp=(DropDownList)FindControl(lStrControlName);

								if(lstrOthAnswerDescription!=null)
								{
									if(lstrOthAnswerDescription.Length>0)
									{
										lArrayAnswers[i]=Temp.SelectedItem.Text + " " + lstrOthAnswerDescription;
									}
									else
									{
										lArrayAnswers[i]=Temp.SelectedItem.Text;
									}
								}
								else
								{
								...
								ReturnValue = objUserQuestion.InsertData(lstrUserQuestionID,lstrUserHOptionID,lstrUserVOptionID,"",lsrtUserOptionListID,lintClientID.ToString(),lsrtUserListID,"",lstrOthAnswerDescription,lblTabID.Text,gStrPolicyID,lstrPolicyVersion,strCarrier,lblhidScreenID.Text);
							}
							else
							{
								//lArrayAnswers[i]=lsrtUserListID;
								DropDownList Temp=new DropDownList();
								Temp=(DropDownList)FindControl(lStrControlName);
								...
								//int ReturnValue = objUserQuestion.InsertData(lstrUserQuestionID,lstrUserHOptionID,lstrUserVOptionID,lsrtUserOptionListID,"","1",lsrtUserListID);
								ReturnValue = objUserQuestion.InsertData(lstrUserQuestionID,lstrUserHOptionID,lstrUserVOptionID,"","",lintClientID.ToString(),lsrtUserListID,lsrtUserOptionListID,lstrOthAnswerDescription,lblTabID.Text,gStrPolicyID,lstrPolicyVersion,strCarrier,lblhidScreenID.Text);
							}
						}
						else if(lstrUserAnswerType == "13")	
						{
							///splitting the List ID and Option List ID.
							...
							{
								//lArrayAnswers[i]=lsrtUserOptionListID;
								RadioButtonList Temp=new RadioButtonList();
								Temp=(RadioButtonList)FindControl(lStrControlName);
							...
							/////If The Answer is of Multi Select List Box 
							//else if(lstrUserAnswerType == "2" && lstrQuestionText !="")
						else if(lstrUserAnswerType == "2" || lstrUserAnswerType == "14")
						{
							///Splitting the List ID And Preparing the comma seprated string of Option List ID							
							if(lstrQuestionText.Trim()!="")
							{
								string [] lsrtListArrayIds = lstrQuestionText.Split(new char[]{','});
								lsrtUserOptionListID="";
								for(int MultiIds=0; MultiIds<=lsrtListArrayIds.Length-1;MultiIds++)
								{
									if (lsrtListArrayIds[MultiIds] != "")
									{
										string [] lsrtListIds = lsrtListArrayIds[MultiIds].Split(new char[]{'#'});
										lsrtUserListID = lsrtListIds[0];
										if(lsrtUserOptionListID=="")
										{
											lsrtUserOptionListID = lsrtListIds[1];
										}
										else
										{
											lsrtUserOptionListID = lsrtUserOptionListID + "," + lsrtListIds[1];
										}
									}
								}
							}
						...
						else if(lstrUserHOptionID!="0" || lstrUserVOptionID !="0")
						{
							lArrayAnswers[i]=lstrQuestionText;
							ReturnValue = objUserQuestion.InsertData(lstrUserQuestionID,lstrUserHOptionID,lstrUserVOptionID,"",lstrQuestionText,lintClientID.ToString(),lsrtUserListID,"",lstrOthAnswerDescription,lblTabID.Text,gStrPolicyID,lstrPolicyVersion,strCarrier,lblhidScreenID.Text);
						}
						else
						{
							lArrayAnswers[i]=lstrQuestionText;
			...
			{
				lblErroMsg.Visible=true;
				lblErroMsg.Text=lstrSaveMessage;
			}
