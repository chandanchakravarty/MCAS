		...
		<xsl:when test ="$AMOUNT !=''">
			<xsl:choose>	
				<xsl:when test ="$AMOUNT &gt;= 0">
					<xsl:choose>
						<xsl:when test="$AMOUNT &lt; 1000">Violation: <xsl:value-of select ="VIOLATION/VIODATE"/><xsl:value-of select ="'   '"/>,$<xsl:value-of select ="VIOLATION/AMOUNTPAID"/> Damage, <xsl:value-of select ="VIOLATION/VIOLATIONTYPE"/></xsl:when>
						<xsl:otherwise>Accident: <xsl:value-of select ="VIOLATION/VIODATE"/><xsl:value-of select ="'   '"/>,$<xsl:value-of select ="VIOLATION/AMOUNTPAID"/> Damage, <xsl:value-of select ="VIOLATION/VIOLATIONTYPE"/></xsl:otherwise>
					</xsl:choose>
				</xsl:when>
				<xsl:otherwise>		
				</xsl:otherwise>
