...
<![CDATA[ 
	...
	<xsl:template name="CSLBIPD_PERSONAL_INDIANA">
		<xsl:choose>
			...
			-->
				...
				<xsl:variable name="VAR7"> <!-- for limit relativity equivalent to limit template -->
					<xsl:call-template name="CSLRELATIVITY" />
				</xsl:variable>
				<xsl:variable name="VAR8">
					<xsl:call-template name="MULTIVEHICLE">
				...
				<xsl:variable name="VARSEC2" select="$SECOND_STEP" />
				<xsl:variable name="THIRD_STEP" select="$VARSEC2 * $VAR3" />
				<xsl:variable name="VARTHI3" select="round(format-number($THIRD_STEP,'##.0000'))" />
				<xsl:variable name="FORTH_STEP" select="$VARTHI3 * $VAR4" />
				<xsl:variable name="VARFOR4" select="round(format-number($FORTH_STEP,'##.0000'))" />
				...
				<xsl:variable name="FOURTEEN_STEP" select="$VARTWE12 * $VAR14" />
	...
	<xsl:template name="BI_PERSONAL_INDIANA">
		<xsl:choose>
			...
			<xsl:when test="VEHICLES/VEHICLE/CSL = 0 or VEHICLES/VEHICLE/CSL='' or normalize-space(VEHICLES/VEHICLE/CSL)='NO COVERAGE' ">
				...
				<xsl:variable name="VARFIS1" select="$FIRST_STEP" />
				...
				<xsl:variable name="VARTWE13" select="round(format-number($THRTEEN_STEP,'##.0000'))" />
	...
	<xsl:template name="MEDPAY">
		<xsl:choose>
			<xsl:when test="POLICY/STATENAME = 'INDIANA' and VEHICLES/VEHICLE/MPLIMIT !='0'  and VEHICLES/VEHICLE/MPLIMIT !='' and VEHICLES/VEHICLE/MPLIMIT !='NO COVERAGE' and VEHICLES/VEHICLE/VEHICLETYPE !=$VT_SUSPENDED_COMP and VEHICLES/VEHICLE/VEHICLETYPE !=$VT_UTILITY_TRAILER and VEHICLES/VEHICLE/VEHICLETYPE !=$VT_CAMPER_TRAVEL_TRAILER and VEHICLES/VEHICLE/VEHICLETYPE_SCO != 'SCO'">
				...
				<xsl:variable name="FOURTEEN_STEP" select="$VARTWE13 * $VAR14" />
				<xsl:variable name="VARTWE14" select="round(format-number($FOURTEEN_STEP,'##.0000'))" />
				<xsl:value-of select="round($VARTWE14)" />
			</xsl:when>
			<xsl:otherwise>
			0.00
		</xsl:otherwise>
	...
	<xsl:template name="UM_PERSONAL_INDIANA">
		...
		<xsl:variable name="VAR_UWTIER">
			<xsl:call-template name="UWTIER">
				<xsl:with-param name="FACTORELEMENT" select="'UM'"></xsl:with-param>
			</xsl:call-template>
		</xsl:variable>
		<xsl:value-of select="round(round($VAR1*$VAR_TYPEOFVEHICLE)*$VAR_UWTIER)" />
	</xsl:template>
	...
	<xsl:template name="UIM">
		<xsl:choose>
			...
			<xsl:otherwise>
				...
				<xsl:choose><xsl:when test="POLICY/STATENAME ='INDIANA'"><xsl:value-of select="round(round(round($VAR1*$VAR2)*$VAR_TYPEOFVEHICLE)*$VAR_UWTIER)" /></xsl:when><xsl:otherwise><xsl:value-of select="round(round($VAR1*$VAR2)*$VAR_TYPEOFVEHICLE)" /></xsl:otherwise></xsl:choose>				
			</xsl:otherwise>
		</xsl:choose>
	</xsl:template>
		...
		<xsl:variable name="COMP_PREMIUM">
			<xsl:choose>
				...
				<xsl:otherwise>
					...
					<xsl:variable name="VARFORT15" select="round(format-number($FIFTEEN_STEP,'##.0000'))" />
					<xsl:value-of select="round($VARFORT15)" />
				</xsl:otherwise>
			</xsl:choose>
		</xsl:variable>
		<xsl:variable name="VAR_MIN_PREMIUM_VALUE" select="$HOCoveragesDoc/PRODUCTMASTER/PRODUCT/FACTOR[@ID='MINIMUM_VEHICLE_PREMIUM_COMP_ONLY']/NODE[@ID='MINIMUM_PREMIUM']/ATTRIBUTES/@MIN_PREMIUM" />
	...
	<xsl:template name="COLL_PERSONAL_INDIANA">
		...
		<xsl:choose>
			...
			<xsl:otherwise>
				...
				<xsl:variable name="SIXTEEN_STEP" select="$VARFIF15 * $VAR16" />
				<xsl:variable name="VARFIF16" select="round(format-number($SIXTEEN_STEP,'##.0000'))" />
				<xsl:value-of select="round($VARFIF16)" />
			</xsl:otherwise>
		</xsl:choose>
	</xsl:template>
	<!-- End of Collision  -->
	...
	<!-- Underwriting Tier -->
	<xsl:template name="UWTIER">
		<xsl:param name="FACTORELEMENT" /> <!-- Depending on the factor element BI/PD..etc -->
		<xsl:variable name="VAR_UWTIER" select="POLICY/UNDRWRTINGTIER" /> <!--POLICY/TERRITORY-->
		<xsl:choose>
			<xsl:when test="$VAR_UWTIER =''">
			1.00
		</xsl:when>
			<xsl:otherwise>
				<xsl:choose>
					<xsl:when test="$FACTORELEMENT = 'BI'">
						<xsl:value-of select="$HOCoveragesDoc/PRODUCTMASTER/PRODUCT/FACTOR[@ID='UNDERWRITINGTIER']/NODE[@ID='UWTIR']/ATTRIBUTES[@UWTIER= $VAR_UWTIER ]/@BIREL" />
					</xsl:when>
					<xsl:when test="$FACTORELEMENT = 'PD'">
						<xsl:value-of select="$HOCoveragesDoc/PRODUCTMASTER/PRODUCT/FACTOR[@ID='UNDERWRITINGTIER']/NODE[@ID='UWTIR']/ATTRIBUTES[@UWTIER= $VAR_UWTIER ]/@PDREL" />
					</xsl:when>
					<xsl:when test="$FACTORELEMENT = 'MEDPAY'">
						<xsl:value-of select="$HOCoveragesDoc/PRODUCTMASTER/PRODUCT/FACTOR[@ID='UNDERWRITINGTIER']/NODE[@ID='UWTIR']/ATTRIBUTES[@UWTIER= $VAR_UWTIER ]/@MEDPAYREL" />
					</xsl:when>
					<xsl:when test="$FACTORELEMENT = 'UM'">
						<xsl:value-of select="$HOCoveragesDoc/PRODUCTMASTER/PRODUCT/FACTOR[@ID='UNDERWRITINGTIER']/NODE[@ID='UWTIR']/ATTRIBUTES[@UWTIER= $VAR_UWTIER ]/@UMREL" />
					</xsl:when>
					<xsl:when test="$FACTORELEMENT = 'UIM'">
						<xsl:value-of select="$HOCoveragesDoc/PRODUCTMASTER/PRODUCT/FACTOR[@ID='UNDERWRITINGTIER']/NODE[@ID='UWTIR']/ATTRIBUTES[@UWTIER= $VAR_UWTIER ]/@UIMREL" />
					</xsl:when>
					<xsl:when test="$FACTORELEMENT = 'COMP'">
						<xsl:value-of select="$HOCoveragesDoc/PRODUCTMASTER/PRODUCT/FACTOR[@ID='UNDERWRITINGTIER']/NODE[@ID='UWTIR']/ATTRIBUTES[@UWTIER= $VAR_UWTIER ]/@COMPREL" />
					</xsl:when>
					<xsl:when test="$FACTORELEMENT = 'COLL'">
						<xsl:value-of select="$HOCoveragesDoc/PRODUCTMASTER/PRODUCT/FACTOR[@ID='UNDERWRITINGTIER']/NODE[@ID='UWTIR']/ATTRIBUTES[@UWTIER= $VAR_UWTIER ]/@COLLREL" />
					</xsl:when>
					<xsl:otherwise>1.00</xsl:otherwise>
				</xsl:choose>
			</xsl:otherwise>
