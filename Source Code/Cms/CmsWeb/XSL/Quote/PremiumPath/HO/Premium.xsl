...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:msxsl="urn:schemas-microsoft-com:xslt"
	...
	<msxsl:script language="c#" implements-prefix="user">
		<![CDATA[ 
				...
				int ZONE_ID=2;
				long  SlabPrm=0;
				public long GetTotalSum(long  SlabsSum)
				{
					long  val=0;
					if(SlabsSum < 0)
					{
			...
			<GETPATH>
				<PRODUCT PRODUCTID="0" DESC="Policy Form:">
					...
					<GROUP GROUPID="5">
						...
						<SUBGROUP>
							<STEP STEPID="9"> <!-- ACV Loss Settlement Windstorm or Hail Losses to Roof Surfacing (HO 0493)  -->
								<PATH>
									<xsl:call-template name="ACV_LOSS_SETL_WIND_HAIL"></xsl:call-template>
								</PATH>
							</STEP>
						</SUBGROUP>
						<SUBGROUP>
							<STEP STEPID="10"> <!-- Dwellng  under construction -->
								<PATH>
						...
						<SUBGROUP>
							<STEP STEPID="11"> <!-- Experience Factor -->
								<PATH>
						...
						<SUBGROUP>
							<IF>
								<STEP STEPID="13"> <!-- Protective Device -->
									<PATH>
								{
						...
						<SUBGROUP>
							<STEP STEPID="14"> <!-- Multi-policy display -->
								<PATH>
						...
						<SUBGROUP>
							<STEP STEPID="15"> <!-- Valued customer -->
								<PATH>
						...
						<SUBGROUP>
							<STEP STEPID="16"> <!-- Replacement Cost -->
								<PATH>
									<xsl:choose>
						...
						<SUBGROUP>
							<STEP STEPID="18"> <!--  Suburban Protection Class Rating Discount -->
								<PATH>
									<xsl:call-template name="SUBURBANPROTECTIONDISCOUNTDISPLAY"></xsl:call-template>
								</PATH>
							</STEP>
						</SUBGROUP>
						<SUBGROUP>
							<STEP STEPID="19"> <!-- Loss Free Renewal Discount and Loss Surcharge -->
								<PATH>
									<xsl:call-template name="LOSSFREECREDITDISPLAY"></xsl:call-template>
								</PATH>
							</STEP>
						</SUBGROUP>
						<SUBGROUP>
							<IF>
								<STEP STEPID="20"> <!-- Wood Stove -->
									<PATH>
						...
						<SUBGROUP>
							<IF>
								<STEP STEPID="21"> <!-- Breed of Dog -->
									<PATH>
								{	<xsl:variable name="VAR_BREEDDOG">
											<xsl:call-template name="BREEDDOG" />
										</xsl:variable>
									<xsl:choose>
											<xsl:when test="$VAR_BREEDDOG = 1.00">0</xsl:when>
											<xsl:otherwise>
						...
						<SUBGROUP>
							<IF>
								<STEP STEPID="22"> <!-- Seasonal Secondary -->
									<PATH>
					...
					<GROUP GROUPID="6">
						...
						<SUBGROUP>
							<STEP STEPID="24"> <!-- HO-64 Renters Deluxe Endorsement -->
								<PATH>
						...
						<SUBGROUP>
							<IF>
								<STEP STEPID="25"> <!-- HO-66 Condominium Deluxe Endorsement -->
									<PATH>
						...
						<SUBGROUP>
							<IF>
								<STEP STEPID="27"> <!-- HO-11 Expanded Replacement Coverage for Building -->
									<PATH>
								{
						...
						<SUBGROUP>
							<IF>
								<STEP STEPID="28"> <!-- HO-96 Increased Fire Dept. Service Charge -->
									<PATH>
								{
						...
						<SUBGROUP>
							<IF>
								<STEP STEPID="29"> <!-- HO-312 Business Property Increased Limits -->
									<PATH>
						...
						<SUBGROUP>
							<IF>
								<STEP STEPID="30"> <!--HO-48 Other Structures - Increased Limits -->
									<PATH>
								{
						...
						<SUBGROUP>
							<IF>
								<STEP STEPID="31"> <!-- HO-40 Other Structures - Rented to Others -->
									<PATH>
								{
						...
						<SUBGROUP>
							<IF>
								<STEP STEPID="32"> <!-- HO-42 Other Structure With Incidental --> <!--This step is no more used-->
									<PATH>
								{
						...
						<SUBGROUP>
							<IF>
								<STEP STEPID="33"> <!-- HO-50 Personal Property Away From Premises  -->
									<PATH>
						...
						<SUBGROUP>
							<IF>
								<STEP STEPID="34"> <!-- Building Additions and Alterations (HO-51) in case of ho4 -->
									<PATH>
								{
								<xsl:choose>
						...
						<SUBGROUP>
							<IF>
								<STEP STEPID="35"> <!-- HO-53 Increased Credit Card -->
									<PATH>
								{
						...
						<SUBGROUP>
							<IF>
								<STEP STEPID="37"> <!-- Unscheduled Jewelry, Watches & Fur -->
									<PATH>
								{
						...
						<SUBGROUP>
							<IF>
								<STEP STEPID="38"> <!-- Unscheduled Jewelry, Watches & Fur -->
									<PATH>
								{
						...
						<SUBGROUP>
							<IF>
								<STEP STEPID="40"> <!-- Silverware, Goldware, Etc. -->
									<PATH>
								{
						...
						<SUBGROUP>
							<IF>
								<STEP STEPID="42"> <!-- HO-35  loss assessment in case of ho6 -->
									<PATH>
								{
									<xsl:choose>
						...
						<SUBGROUP>
							<IF>
								<STEP STEPID="43"> <!-- HO-277 Ordinance or Law coverage Forms -->
									<PATH>
								{
						...
						<SUBGROUP>
							<IF>
								<STEP STEPID="44"> <!-- HO-32 Unit Owners Coverage A Special Coverage -->
									<PATH>
						...
						<SUBGROUP>
							<IF>
								<STEP STEPID="45"> <!-- HO-455 Identity Fraud Expense Coverage -->
									<PATH>
								{
						...
						<SUBGROUP>
							<IF>
								<STEP STEPID="46"> <!-- HO-315 Earthquake -->
									<PATH>
							{	
						...
						<SUBGROUP>
							<IF>
								<STEP STEPID="47"> <!-- HO-327 Water Back-Up and Sump Pump Overflow -->
									<PATH>
								{
						...
						<SUBGROUP>
							<IF>
								<STEP STEPID="48"> <!-- HO-33 Condo-Unit Owner Rental to Others. -->
									<PATH>
						...
						<SUBGROUP>
							<STEP STEPID="49"> <!-- HO 373 Indiana Contingent Workers  Compensation. -->
								<PATH>
									<xsl:choose>
						...
						<SUBGROUP>
							<IF>
								<STEP STEPID="50"> <!-- MINE SUBSIDENCE COVERAGE -->
									<PATH>
						...
						<SUBGROUP>
							<IF>
								<STEP STEPID="51"> <!-- HO-9 Collapse From Sub-Surface Water -->
									<PATH>
								{
						...
						<SUBGROUP>
							<IF>
								<STEP STEPID="52"> <!-- HO-490 Specific Structure(s) Away From Premises -->
									<PATH>
								{
						...
						<SUBGROUP>
							<IF>
								<STEP STEPID="53"> <!-- HO-489 Other Structures (Repair Cost Coverage) -->
									<PATH>
								{
						...
						<SUBGROUP>
							<IF>
								<STEP STEPID="54"> <!-- primary Residence (1 or 2 Family) -->
									<PATH>
								{
						...
						<SUBGROUP>
							<IF>
								<STEP STEPID="55"> <!-- Residence Employees  -->
									<PATH>
								{
						...
						<SUBGROUP>
							<IF>
								<STEP STEPID="56"> <!-- Additional Premises-Occupied By Insured -->
									<PATH>
								{
						...
						<SUBGROUP>
							<IF>
								<STEP STEPID="57"> <!-- HO-40 Additional Premises Rented to Others - Residence Premises -->
									<PATH>
								{
						...
						<SUBGROUP>
							<IF>
								<STEP STEPID="58"> <!--   HO-70 Additional Premises Rented - Other Location - 1 Family -->
									<PATH>
								{
						...
						<SUBGROUP>
							<IF>
								<STEP STEPID="59"> <!--   HO-70 Additional Premises Rented - Other Location - 2 Family -->
									<PATH>
								{
						...
						<SUBGROUP>
							<IF>
								<STEP STEPID="60"> <!--   HO-42 Incidental Office Private School or Studio On Premises -->
									<PATH>
								 {
						...
						<SUBGROUP>
							<IF>
								<STEP STEPID="61"> <!-- Business Located in an Other Structure -->
									<PATH>
								 {
						...
						<SUBGROUP>
							<IF>
								<STEP STEPID="62"> <!-- HO-42 Incidental Office Private School or Studio Instruction Only -->
									<PATH>
								{
						...
						<SUBGROUP>
							<IF>
								<STEP STEPID="63"> <!-- HO-200 Waterbed Liability -->
									<PATH>
								{
						...
						<SUBGROUP>
							<IF>
								<STEP STEPID="64"> <!-- HO-43 Incidental Office Private School or Studio Off Premises -->
									<PATH>
								{
						...
						<SUBGROUP>
							<IF>
								<STEP STEPID="66"> <!-- HO-71 Business Pursuits:Class A  -->
									<PATH>
								{
						...
						<SUBGROUP>
							<IF>
								<STEP STEPID="67"> <!-- HO-71 Business Pursuits:Class B -->
									<PATH>
								{
						...
						<SUBGROUP>
							<IF>
								<STEP STEPID="68"> <!-- HO-71 Business Pursuits:Class C -->
									<PATH>
								{
						...
						<SUBGROUP>
							<IF>
								<STEP STEPID="69"> <!-- HO-71 Business Pursuits:Class D  -->
									<PATH>
								{
						...
						<SUBGROUP>
							<IF>
								<STEP STEPID="70"> <!-- HO-72 Farm Liability - Incidental Farming Residence Premises -->
									<PATH>
								{
						...
						<SUBGROUP>
							<IF>
								<STEP STEPID="71"> <!-- HO-73 Farm Liability - Owned By Insured Rented to Others -->
									<PATH>
								{
						...
						<SUBGROUP>
							<IF>
								<STEP STEPID="72"> <!-- HO-73 Farm Liability - Owned/Operated By Insured/Insured Employee -->
									<PATH>
								{
						...
						<SUBGROUP>
							<IF>
								<STEP STEPID="73"> <!-- HO-82 Personal Injury -->
									<PATH>
								{
					...
					<GROUP GROUPID="7">
						...
						<SUBGROUP>
							<IF>
								<STEP STEPID="75"> <!-- Cameras (Non-Professional) -->
									<PATH>
								{
						...
						<SUBGROUP>
							<IF>
								<STEP STEPID="76"> <!-- Cellular Phones (HO-900) -->
									<PATH>
								{
						...
						<SUBGROUP>
							<IF>
								<STEP STEPID="81"> <!-- Musical (Non-Professional) -->
									<PATH>
								{
						...
						<SUBGROUP>
							<IF>
								<STEP STEPID="82"> <!-- Personal Computers (HO-214) -->
									<PATH>
								{
						...
						<SUBGROUP>
							<IF>
								<STEP STEPID="86"> <!-- Fine Arts - Without Breakage -->
									<PATH>
								{
						...
						<SUBGROUP>
							<IF>
								<STEP STEPID="87"> <!-- Fine Arts - With Breakage -->
									<PATH>
								{
						...
						<SUBGROUP>
							<IF>
								<STEP STEPID="88"> <!-- Handicap Electronics -->
									<PATH>
								{
						...
						<SUBGROUP>
							<IF>
								<STEP STEPID="92"> <!-- Personal Computers - Laptop -->
									<PATH>
								{
						...
						<SUBGROUP>
							<IF>
								<STEP STEPID="93"> <!-- Salesman Supplies -->
									<PATH>
								{
						...
						<SUBGROUP>
							<IF>
								<STEP STEPID="94"> <!-- Scuba Diving Equipment -->
									<PATH>
								{
						...
						<SUBGROUP>
							<IF>
								<STEP STEPID="96"> <!--Tack and Saddle   -->
									<PATH>
								{
						...
						<SUBGROUP>
							<IF>
								<STEP STEPID="97"> <!-- Tools (At Premises)  -->
									<PATH>
								{
						...
						<SUBGROUP>
							<IF>
								<STEP STEPID="98"> <!-- Tools (At Business Location) -->
									<PATH>
								{
						...
						<SUBGROUP>
							<IF>
								<STEP STEPID="100"> <!-- Train Collections -->
									<PATH>
								{
						...
						<SUBGROUP>
							<IF>
								<STEP STEPID="101"> <!-- Wheelchairs/Scooters -->
									<PATH>
								{
						...
						<SUBGROUP>
							<IF>
								<STEP STEPID="102"> <!-- Cameras (Professional Use) -->
									<PATH>
								{
									<xsl:call-template name="CAMERAS_PROF_PREMIUM"></xsl:call-template>
								}
							</PATH>
								</STEP>
							</IF>
						...
						<SUBGROUP>
							<IF>
								<STEP STEPID="103"> <!-- Musical Instruments (Used for Remuneration) -->
									<PATH>
								{
									<xsl:call-template name="MUSICAL_RENUMERATION_PREMIUM"></xsl:call-template>
								}
							</PATH>
								</STEP>
							</IF>
					...
					<GROUP GROUPID="8">
						...
						<SUBGROUP>
							<IF>
								<STEP STEPID="104"> <!-- Property Expense Fee -->
									<PATH>
								{
				...
				<PRODUCT PRODUCTID="0">
					...
					<GROUP GROUPID="5" CALC_ID="10005">
						...
						<STEP STEPID="6" CALC_ID="1006" PREFIX="" SUFIX="" ISCALCULATE="F">
							...
							<COMP_ACT_PRE></COMP_ACT_PRE>
							<COMP_REMARKS>
								<xsl:call-template name="REMARKINSURANCE" />
							</COMP_REMARKS>
							<COMP_EXT></COMP_EXT>
							<COM_EXT_AD>
						...
						<STEP STEPID="7" CALC_ID="1007" PREFIX="" SUFIX="" ISCALCULATE="F">
							...
							<COMP_ACT_PRE></COMP_ACT_PRE>
							<COMP_REMARKS>
								<xsl:call-template name="REMARKSMOKER" />
							</COMP_REMARKS>
							<COMP_EXT></COMP_EXT>
							<COM_EXT_AD>
								<xsl:call-template name="SMOKER_CREDIT_DISPLAY" />
								<xsl:text>%</xsl:text>
							</COM_EXT_AD>
						</STEP>
						...
						<!-- ACV Loss Settlement Windstorm or Hail Losses to Roof Surfacing (HO 0493)  -->
						<STEP STEPID="9" CALC_ID="1009" PREFIX="" SUFIX="" ISCALCULATE="F">
							...
							<L_PATH></L_PATH>
							<COMPONENT_TYPE>D</COMPONENT_TYPE>
							<COMPONENT_CODE>D_ACVLSWH</COMPONENT_CODE>
							<COMP_ACT_PRE></COMP_ACT_PRE>
							<COMP_REMARKS>
								<xsl:call-template name="REMARKHAILSTORM" />
							</COMP_REMARKS>
							<COMP_EXT></COMP_EXT>
							<COM_EXT_AD>
								<xsl:call-template name="ACV_LOSS_SETL_WIND_HALL_CREDIT" />
								<xsl:text>%</xsl:text>
							</COM_EXT_AD>
						</STEP>
						...
						<STEP STEPID="10" CALC_ID="10010" PREFIX="" SUFIX="" ISCALCULATE="F">
							...
							<COMP_ACT_PRE></COMP_ACT_PRE>
							<COMP_REMARKS>
								<xsl:call-template name="REMARKCONSTRUCTION" />
							</COMP_REMARKS>
							<COMP_EXT></COMP_EXT>
							<COM_EXT_AD>
								<xsl:call-template name="DWELLING_UNDER_CONSTRUCTION_CREDIT"></xsl:call-template>
								<xsl:text>%</xsl:text>
							</COM_EXT_AD>
						</STEP>
						<!-- Experience Factor -->
						<STEP STEPID="11" CALC_ID="1011" PREFIX="" SUFIX="" ISCALCULATE="F">
							...
							<COMP_ACT_PRE></COMP_ACT_PRE>
							<COMP_REMARKS>
								<xsl:call-template name="REMARKEXPER" />
							</COMP_REMARKS>
							<COMP_EXT></COMP_EXT>
							<COM_EXT_AD>
								<xsl:call-template name="EXPERIENCECREDIT"></xsl:call-template>
								<xsl:text>%</xsl:text>
							</COM_EXT_AD>
						</STEP>
						<!-- New Steps START-->
						...
						<STEP STEPID="12" CALC_ID="1012" PREFIX="" SUFIX="" ISCALCULATE="F">
							...
							<D_PATH></D_PATH>
							<L_PATH></L_PATH>
							<COMPONENT_TYPE><xsl:call-template name="AGE_OF_HOME_TYPE" /></COMPONENT_TYPE>
							<COMPONENT_CODE>D_AGE_HOME</COMPONENT_CODE>
							<COMP_ACT_PRE></COMP_ACT_PRE>
							<COMP_REMARKS>
								<xsl:call-template name="REMARKAGEHOME" />
							</COMP_REMARKS>
							<COMP_EXT></COMP_EXT>
							<COM_EXT_AD>
								<xsl:call-template name="AGEOFHOME_CREDIT" />
								<xsl:text>%</xsl:text>
							</COM_EXT_AD>
						</STEP>
						<!-- Protective Device -->
						<STEP STEPID="13" CALC_ID="1013" PREFIX="" SUFIX="" ISCALCULATE="F">
							...
							<COMP_ACT_PRE></COMP_ACT_PRE>
							<COMP_REMARKS>
								<xsl:call-template name="REMARKPROTDEVICE" />
							</COMP_REMARKS>
							<COMP_EXT></COMP_EXT>
							<COM_EXT_AD>
						...
						<STEP STEPID="15" CALC_ID="1015" PREFIX="" SUFIX="" ISCALCULATE="F">
							...
							<COMP_ACT_PRE></COMP_ACT_PRE>
							<COMP_REMARKS>
								<xsl:call-template name="REMARKVALUDCUST" />
							</COMP_REMARKS>
							<COMP_EXT></COMP_EXT>
							<COM_EXT_AD>
								<xsl:call-template name="VALUEDCUSTOMER_FACTOR"></xsl:call-template>
								<xsl:text>%</xsl:text>
							</COM_EXT_AD>
						</STEP>
						<!-- Replacement Cost -->
						<STEP STEPID="16" CALC_ID="1016" PREFIX="" SUFIX="" ISCALCULATE="F">
							<xsl:attribute name="DESC">
								<xsl:call-template name="STEPID16"></xsl:call-template>
							</xsl:attribute>
							<PATH>0</PATH>
							<D_PATH></D_PATH>
							<L_PATH></L_PATH>
						...
						<STEP STEPID="17" CALC_ID="1017" PREFIX="" SUFIX="" ISCALCULATE="F">
							...
							<COMP_ACT_PRE></COMP_ACT_PRE>
							<COMP_REMARKS>
								<xsl:call-template name="REMARKPRIORLOSS" />
							</COMP_REMARKS>
							<COMP_EXT>
								<xsl:call-template name="COMPONENT_CODE">
						...
						<!--  Suburban Protection Class Rating Discount -->
						<STEP STEPID="18" CALC_ID="1018" PREFIX="" SUFIX="" ISCALCULATE="F">
							...
							<L_PATH></L_PATH>
							<COMPONENT_TYPE>D</COMPONENT_TYPE>
							<COMPONENT_CODE>SPC_RD</COMPONENT_CODE>
							<COMP_ACT_PRE></COMP_ACT_PRE>
							<COMP_REMARKS>
								<xsl:call-template name="REMARKSUBURBAN" />
							</COMP_REMARKS>
							<COMP_EXT>
								<xsl:call-template name="COMPONENT_CODE">
									<xsl:with-param name="FACTORELEMENT" select="'SPC_RD'"></xsl:with-param>
								</xsl:call-template>
							</COMP_EXT>
							<COM_EXT_AD>
								<xsl:call-template name="SUBURBANPROTECTIONDISCOUNTPERCENT" />
								<xsl:text>%</xsl:text>
							</COM_EXT_AD>
						</STEP>
						<!-- Loss Free Renewal Discount and Loss Surcharge-->
						<STEP STEPID="19" CALC_ID="1019" PREFIX="" SUFIX="" ISCALCULATE="F">
							...
							<D_PATH></D_PATH>
							<L_PATH></L_PATH>
							<COMPONENT_TYPE><xsl:call-template name="COMP_TYPE_LOSS"/></COMPONENT_TYPE>
							<COMPONENT_CODE>LFRD_LS</COMPONENT_CODE>
							<COMP_ACT_PRE></COMP_ACT_PRE>
							<COMP_REMARKS>
								<xsl:call-template name="REMARKLOSSFREE" />
							</COMP_REMARKS>
							<COMP_EXT>
								<xsl:call-template name="COMPONENT_CODE">
									<xsl:with-param name="FACTORELEMENT" select="'LFRD_LS'"></xsl:with-param>
								</xsl:call-template>
							</COMP_EXT>
							<COM_EXT_AD>
								<xsl:call-template name="LOSSFREECREDITPERCENTAGE" />
							</COM_EXT_AD>
						</STEP>
						<!--Wood Stove -->
						<STEP STEPID="20" CALC_ID="1020" PREFIX="" SUFIX="" ISCALCULATE="F">
							...
							<COMP_ACT_PRE></COMP_ACT_PRE>
							<COMP_REMARKS>
								<xsl:call-template name="REMARKWOODSTOVE" />
							</COMP_REMARKS>
							<COMP_EXT></COMP_EXT>
							<COM_EXT_AD>
						...
						<STEP STEPID="21" CALC_ID="1021" PREFIX="" SUFIX="" ISCALCULATE="F">
							...
							<COMP_ACT_PRE></COMP_ACT_PRE>
							<COMP_REMARKS>
								<xsl:call-template name="REMRKBREEDDOG" />
							</COMP_REMARKS>
							<COMP_EXT></COMP_EXT>
							<COM_EXT_AD>
								<xsl:call-template name="DOGS_CHARGE_INPERCENT" />
								<xsl:text>%</xsl:text>
							</COM_EXT_AD>
						</STEP>
						<!-- Seasonal Secondary -->
						<STEP STEPID="22" CALC_ID="1022" PREFIX="" SUFIX="" ISCALCULATE="F">
							...
							<COMP_ACT_PRE></COMP_ACT_PRE>
							<COMP_REMARKS>
								<xsl:call-template name="REMARKSEASONALSECONDRY" />
							</COMP_REMARKS>
							<COMP_EXT></COMP_EXT>
							<COM_EXT_AD>
								<xsl:call-template name="EXADSEASONALSECONDRY" />
							</COM_EXT_AD>
						</STEP>
						<GROUPFORMULA>
					...
					<GROUP GROUPID="6" CALC_ID="10006" PREFIX="" SUFIX="" ISCALCULATE="F">
						...
						<STEP STEPID="41" CALC_ID="1041" PREFIX="" SUFIX="" ISCALCULATE="F">
							...
							<L_PATH>
								...
								<xsl:value-of select="$VAR_HO35INCLUDE + $VAR_HO35ADDITIONAL" /--></L_PATH>
							<COMPONENT_TYPE>P</COMPONENT_TYPE>
						...
						<STEP STEPID="46" CALC_ID="1046" PREFIX="" SUFIX="" ISCALCULATE="F">
							...
							<PATH>0</PATH>
							<D_PATH><xsl:choose><xsl:when test="STATENAME ='MICHIGAN'"><xsl:text>10% - 250</xsl:text></xsl:when></xsl:choose></D_PATH>
							<L_PATH></L_PATH>
							<COMPONENT_TYPE>P</COMPONENT_TYPE>
						...
						<STEP STEPID="50" CALC_ID="1050" PREFIX="" SUFIX="" ISCALCULATE="F">
							...
							<PATH>0</PATH>
							<D_PATH>
								<xsl:value-of select="$HOCoveragesDoc/PRODUCTMASTER/PRODUCT/FACTOR[@ID='OTHER']/NODE[@ID ='MINE_SUBSIDENCE_COVERAGE']/@DEDUCTIBLE_PERCENT" />
								<xsl:text>%</xsl:text>
							</D_PATH>
							<L_PATH></L_PATH>
							<COMPONENT_TYPE>P</COMPONENT_TYPE>
						...
						<STEP STEPID="60" CALC_ID="1060" PREFIX="" SUFIX="" ISCALCULATE="F"> <!-- SKB-->
							<xsl:attribute name="DESC">
						...
						<STEP STEPID="63" CALC_ID="1063" PREFIX="" SUFIX="" ISCALCULATE="F"> <!-- New-->
							<xsl:attribute name="DESC">
					...
					<GROUP GROUPID="7" CALC_ID="10007" PREFIX="" SUFIX="" ISCALCULATE="F">
						...
						<STEP STEPID="100" CALC_ID="1100" PREFIX="" SUFIX="" ISCALCULATE="F">
							<xsl:attribute name="DESC">
								<xsl:call-template name="STEPID100"></xsl:call-template>
							</xsl:attribute>
							<PATH>0</PATH>
							<D_PATH>
								<xsl:choose>
						...
						<!-- Cameras (Professional Use) -->
						<STEP STEPID="102" CALC_ID="1102" PREFIX="" SUFIX="" ISCALCULATE="F">
							<xsl:attribute name="DESC">
								<xsl:call-template name="STEPID102"></xsl:call-template>
							</xsl:attribute>
							<PATH>0</PATH>
							<D_PATH>
								<xsl:choose>
									<xsl:when test="SCH_CAMERA_PROF_DED !=0">
										<xsl:value-of select="SCH_CAMERA_PROF_DED" />
									</xsl:when>
									<xsl:otherwise></xsl:otherwise>
								</xsl:choose>
							</D_PATH>
							<L_PATH>
								<xsl:value-of select="format-number(SCH_CAMERA_PROF_AMOUNT, '###,###')" />
							</L_PATH>
							<COMPONENT_TYPE>P</COMPONENT_TYPE>
							<COMPONENT_CODE>CMRA_PROF</COMPONENT_CODE>
							<COMP_ACT_PRE>
								<xsl:call-template name="CAMERAS_PROF_PREMIUM"></xsl:call-template>
							</COMP_ACT_PRE>
							<COMP_REMARKS></COMP_REMARKS>
							<COMP_EXT>
								<xsl:call-template name="COMPONENT_CODE">
									<xsl:with-param name="FACTORELEMENT" select="'CMRA_PROF'"></xsl:with-param>
								</xsl:call-template>
							</COMP_EXT>
							<COM_EXT_AD></COM_EXT_AD>
						</STEP>
						<!-- Musical Instruments (Used for Remuneration) -->
						<STEP STEPID="103" CALC_ID="1103" PREFIX="" SUFIX="" ISCALCULATE="F">
							<xsl:attribute name="DESC">
								<xsl:call-template name="STEPID103"></xsl:call-template>
							</xsl:attribute>
							<PATH>0</PATH>
							<D_PATH>
								<xsl:choose>
									<xsl:when test="SCH_MUSICAL_REMUN_DED !=0">
										<xsl:value-of select="SCH_MUSICAL_REMUN_DED" />
									</xsl:when>
									<xsl:otherwise></xsl:otherwise>
								</xsl:choose>
							</D_PATH>
							<L_PATH>
								<xsl:value-of select="format-number(SCH_MUSICAL_REMUN_AMOUNT, '###,###')" />
							</L_PATH>
							<COMPONENT_TYPE>P</COMPONENT_TYPE>
							<COMPONENT_CODE>MSC_PRF</COMPONENT_CODE>
							<COMP_ACT_PRE>
								<xsl:call-template name="MUSICAL_RENUMERATION_PREMIUM"></xsl:call-template>
							</COMP_ACT_PRE>
							<COMP_REMARKS></COMP_REMARKS>
							<COMP_EXT>
								<xsl:call-template name="COMPONENT_CODE">
									<xsl:with-param name="FACTORELEMENT" select="'MSC_PRF'"></xsl:with-param>
								</xsl:call-template>
							</COMP_EXT>
							<COM_EXT_AD></COM_EXT_AD>
						</STEP>
						<GROUPFORMULA>
							...
							<OPERATOR>+</OPERATOR>
							<OPERAND TOREPLACE="Y">@[CALC_ID=1077]</OPERAND>
							<OPERATOR>+</OPERATOR>
							<OPERAND TOREPLACE="Y">@[CALC_ID=1078]</OPERAND>
							<OPERATOR>+</OPERATOR>
							<OPERAND TOREPLACE="Y">@[CALC_ID=1079]</OPERAND>
							<OPERATOR>+</OPERATOR>
							<OPERAND TOREPLACE="Y">@[CALC_ID=1080]</OPERAND>
							<OPERATOR>+</OPERATOR>
							<OPERAND TOREPLACE="Y">@[CALC_ID=1081]</OPERAND>
							<OPERATOR>+</OPERATOR>
							<OPERAND TOREPLACE="Y">@[CALC_ID=1082]</OPERAND>
							<OPERATOR>+</OPERATOR>
							<OPERAND TOREPLACE="Y">@[CALC_ID=1083]</OPERAND>
							<OPERATOR>+</OPERATOR>
							<OPERAND TOREPLACE="Y">@[CALC_ID=1084]</OPERAND>
							<OPERATOR>+</OPERATOR>
							<OPERAND TOREPLACE="Y">@[CALC_ID=1085]</OPERAND>
							<OPERATOR>+</OPERATOR>
							<OPERAND TOREPLACE="Y">@[CALC_ID=1086]</OPERAND>
							<OPERATOR>+</OPERATOR>
							<OPERAND TOREPLACE="Y">@[CALC_ID=1087]</OPERAND>
							<OPERATOR>+</OPERATOR>
							<OPERAND TOREPLACE="Y">@[CALC_ID=1088]</OPERAND>
							<OPERATOR>+</OPERATOR>
							<OPERAND TOREPLACE="Y">@[CALC_ID=1089]</OPERAND>
							<OPERATOR>+</OPERATOR>
							<OPERAND TOREPLACE="Y">@[CALC_ID=1090]</OPERAND>
							<OPERATOR>+</OPERATOR>
							<OPERAND TOREPLACE="Y">@[CALC_ID=1091]</OPERAND>
							<OPERATOR>+</OPERATOR>
							<OPERAND TOREPLACE="Y">@[CALC_ID=1092]</OPERAND>
							<OPERATOR>+</OPERATOR>
							<OPERAND TOREPLACE="Y">@[CALC_ID=1093]</OPERAND>
							<OPERATOR>+</OPERATOR>
							<OPERAND TOREPLACE="Y">@[CALC_ID=1094]</OPERAND>
							<OPERATOR>+</OPERATOR>
							<OPERAND TOREPLACE="Y">@[CALC_ID=1095]</OPERAND>
							<OPERATOR>+</OPERATOR>
							<OPERAND TOREPLACE="Y">@[CALC_ID=1096]</OPERAND>
							<OPERATOR>+</OPERATOR>
							<OPERAND TOREPLACE="Y">@[CALC_ID=1097]</OPERAND>
							<OPERATOR>+</OPERATOR>
							<OPERAND TOREPLACE="Y">@[CALC_ID=1098]</OPERAND>
							<OPERATOR>+</OPERATOR>
							<OPERAND TOREPLACE="Y">@[CALC_ID=1099]</OPERAND>
							<OPERATOR>+</OPERATOR>
							<OPERAND TOREPLACE="Y">@[CALC_ID=1100]</OPERAND>
							<OPERATOR>+</OPERATOR>
							<OPERAND TOREPLACE="Y">@[CALC_ID=1101]</OPERAND>
							<OPERATOR>+</OPERATOR>
							<OPERAND TOREPLACE="Y">@[CALC_ID=1102]</OPERAND>
							<OPERATOR>+</OPERATOR>
							<OPERAND TOREPLACE="Y">@[CALC_ID=1103]</OPERAND>
						</GROUPFORMULA>
					</GROUP>
					<GROUP GROUPID="8" CALC_ID="10008" PREFIX="" SUFIX="" ISCALCULATE="F">
						...
						<STEP STEPID="104" CALC_ID="1104" PREFIX="" SUFIX="" ISCALCULATE="F">
							<xsl:attribute name="DESC">
								<xsl:call-template name="STEPID104" />
							</xsl:attribute>
							<PATH>0</PATH>
							<D_PATH></D_PATH>
							<L_PATH></L_PATH>
					...
					<GROUP GROUPID="9" CALC_ID="10009" PREFIX="" SUFIX="" ISCALCULATE="F">
						...
						<GDESC></GDESC>
						<!--Minimum Premium -->
						<STEP STEPID="105" CALC_ID="1105" PREFIX="" SUFIX="" ISCALCULATE="F">
							<xsl:attribute name="DESC">
								<xsl:call-template name="STEPID105"></xsl:call-template>
							</xsl:attribute>
							<PATH>0</PATH>
							<D_PATH></D_PATH>
							<L_PATH></L_PATH>
						...
						<!-- Final Premium -->
						<STEP STEPID="106" CALC_ID="1106" PREFIX="" SUFIX="" ISCALCULATE="F">
							<xsl:attribute name="DESC">
								<xsl:call-template name="STEPID106" />
							</xsl:attribute>
							<PATH>0</PATH>
							<D_PATH></D_PATH>
							<L_PATH></L_PATH>
	...
	<xsl:template name="DEWLLING-MAIN">
		...
		<xsl:variable name="VAR_TERR_RELATIVITY" select="$HOCoveragesDoc/PRODUCTMASTER/PRODUCT/FACTOR[@ID='TERRITORY']/NODE[@ID='TERRITORYRELATIVITY']/ATTRIBUTES[@TID=$TERCODES]/@RELATIVITY" />
		<xsl:variable name="F_CODE" select="FORM_CODE" />
		<xsl:variable name="VAR_FORMRELATIVITY" select="$HOCoveragesDoc/PRODUCTMASTER/PRODUCT/FACTOR[@ID='FORMGROUP']/NODE[@ID='FORMPROTCLASSRELATIVITY']/ATTRIBUTES[@FORMID=$F_CODE]/@RELATIVITY" />
		...
		<xsl:variable name="VAR_BASE_PRM" select="$HOCoveragesDoc/PRODUCTMASTER/PRODUCT[@ID='HO-P']/FACTOR[@ID='PREMIUM']/NODE[@ID='PREMIUMTERRITORY']/ATTRIBUTES[@Form_Code = $PCODE ]/@Base_premium" />
		<xsl:value-of select="round(round($VAR_BASE_PRM * $VAR_FORMRELATIVITY)*$VAR_TERR_RELATIVITY)" />
	</xsl:template>
	<!-- For INDINA -->
	...
	<xsl:template name="INSURANCESCORE_PERCENT"> <!-- INSURANCE SCORE Credit in percent for display -->
		...
		<xsl:choose> <!-- Fetch the insurance score credit depending on Insurance Score -->
			<xsl:when test="$INS='NOHITNOSCORE'">
				<xsl:value-of select="$HOCoveragesDoc/PRODUCTMASTER/PRODUCT/FACTOR[@ID='INSURANCESCORECREDIT']/NODE[@ID ='CREDIT'  ]/ATTRIBUTES[@MINSCORE = 'N']/@CREDIT" />
				<xsl:text>%</xsl:text>
			</xsl:when>
			<xsl:when test="$INS &gt;= 0">
	...
	<xsl:template name="AGEOFHOME_FACTOR">
		...
		<xsl:choose>
			...
			<xsl:otherwise>
				...
				<xsl:choose>
					<xsl:when test="STATENAME ='INDIANA'">
						<xsl:variable name="VAR_MINAGEOFHOME" select="$HOCoveragesDoc/PRODUCTMASTER/PRODUCT/FACTOR[@ID='CREDITS']/NODE[@ID ='AOHCREDIT']/@MINAGEOFHOMESURCHARGE"/>
						<xsl:variable name="VAR_AGE"><xsl:choose><xsl:when test="$AGE_OF_HOME &gt;= $VAR_MINAGEOFHOME"><xsl:value-of select="$VAR_MINAGEOFHOME"/></xsl:when><xsl:otherwise><xsl:value-of select="$AGE_OF_HOME"/></xsl:otherwise></xsl:choose></xsl:variable>
						<xsl:choose>
							<xsl:when test="AGEOFHOME &gt; $VAR_MAXAGEOFHOME and AGEOFHOME &lt; $VAR_MINAGEOFHOME">1.00</xsl:when>
							<xsl:when test="AGEOFHOME &lt;= $VAR_MAXAGEOFHOME and CONSTRUCTIONCREDIT = 'N'">
								<xsl:value-of select="$HOCoveragesDoc/PRODUCTMASTER/PRODUCT/FACTOR[@ID='CREDITS']/NODE[@ID ='AOHCREDIT']/ATTRIBUTES[@AGE = $VAR_AGE]/@FACTOR" />
							</xsl:when>
							<xsl:when test="AGEOFHOME &gt;= $VAR_MINAGEOFHOME and CONSTRUCTIONCREDIT = 'N'">
								<xsl:value-of select="$HOCoveragesDoc/PRODUCTMASTER/PRODUCT/FACTOR[@ID='CREDITS']/NODE[@ID ='AOHCREDIT']/ATTRIBUTES[@AGE = $VAR_AGE]/@FACTOR"/>
							</xsl:when>
							<xsl:otherwise>1.00</xsl:otherwise>
						</xsl:choose>
					</xsl:when>
					<xsl:when test="AGEOFHOME &gt; $VAR_MAXAGEOFHOME and STATENAME ='MICHIGAN'">1.00</xsl:when>
					<xsl:otherwise>
						<xsl:choose>
							<xsl:when test="CONSTRUCTIONCREDIT = 'N' and STATENAME ='MICHIGAN'">								
				...
				<xsl:choose>
					<xsl:when test="STATENAME ='INDIANA'">
						...
						<xsl:choose>
							<xsl:when test="AGEOFHOME &gt; $VAR_MAXAGEOFHOME and AGEOFHOME &lt; $VAR_MINAGEOFHOME">1.00</xsl:when>
							<xsl:when test="AGEOFHOME &lt;= $VAR_MAXAGEOFHOME and CONSTRUCTIONCREDIT = 'N'">
								<xsl:value-of select="$HOCoveragesDoc/PRODUCTMASTER/PRODUCT/FACTOR[@ID='CREDITS']/NODE[@ID ='AOHCREDIT']/ATTRIBUTES[@AGE = $VAR_AGE]/@CREDIT" />
							</xsl:when>
							<xsl:when test="AGEOFHOME &gt;= $VAR_MINAGEOFHOME and CONSTRUCTIONCREDIT = 'N'">
								<xsl:variable name="VAR_CREDIT"><xsl:value-of select="$HOCoveragesDoc/PRODUCTMASTER/PRODUCT/FACTOR[@ID='CREDITS']/NODE[@ID ='AOHCREDIT']/ATTRIBUTES[@AGE = $VAR_AGE]/@CREDIT"/></xsl:variable>
								<xsl:value-of select="$VAR_CREDIT - 100"/>
							</xsl:when>
							<xsl:otherwise>0.00</xsl:otherwise>
						</xsl:choose>
					</xsl:when>
	...
	<xsl:template name="AGEOFHOME_DISPLAY">
		...
		<xsl:choose>
			...
			<xsl:otherwise>
				...
				<xsl:variable name="VAR_MINAGEOFHOME" select="$HOCoveragesDoc/PRODUCTMASTER/PRODUCT/FACTOR[@ID='CREDITS']/NODE[@ID ='AOHCREDIT']/@MINAGEOFHOMESURCHARGE"/>
				<xsl:choose>
					<xsl:when test="AGEOFHOME &gt; $VAR_MAXAGEOFHOME and AGEOFHOME &lt; $VAR_MINAGEOFHOME and STATENAME ='INDIANA'">0.00</xsl:when>
					<xsl:when test="AGEOFHOME &gt; $VAR_MAXAGEOFHOME and STATENAME ='MICHIGAN' ">0.00</xsl:when>
					<xsl:otherwise>
						<xsl:choose>
	...
	<xsl:template name="VALUEDCUSTOMER_DISPLAY">
		<xsl:choose>
			<xsl:when test="normalize-space(VALUEDCUSTOMER)='Y' and PRODUCTNAME = $POLICYTYPE_HO5 and STATENAME ='MICHIGAN' and PRODUCT_PREMIER = $POLICYDESC_REPLACEMENT">0</xsl:when>
			<xsl:otherwise>0</xsl:otherwise>
		</xsl:choose>
	</xsl:template>
	<!-- ============================================================================================ -->
	...
	<xsl:template name="PRIOR_LOSS_DISPLAY">
		<xsl:choose>
			<xsl:when test="PRIOR_LOSS_SURCHARGE = 'Y'">0</xsl:when>
			<xsl:otherwise>0</xsl:otherwise>
		</xsl:choose>
	</xsl:template>
	...
	<!-- ============================================================================================ -->
	<!-- ============================================================================================ -->
	<!--				Templates for Suburban Protection Discount [Factor,Credit,Display](START)  			  -->
	<!-- ============================================================================================ -->
	<xsl:template name="SUBURBANPROTECTIONDISCOUNT">
		<xsl:variable name="PROTECTIONCLASS" select="FIREPROTECTIONCLASS" />
		<xsl:choose>
			<xsl:when test="normalize-space(SUBPROTDIS) = 'Y' and PRODUCT_PREMIER != $POLICYDESC_REPAIR">
				<xsl:value-of select="$HOCoveragesDoc/PRODUCTMASTER/PRODUCT/FACTOR[@ID='PROT_DISCOUNT']/NODE[@ID='SUB_PROT_DISCOUNT']/ATTRIBUTES/@FACTOR" />
			</xsl:when>
			<xsl:otherwise>1.00</xsl:otherwise>
		</xsl:choose>
	</xsl:template>
	<xsl:template name="SUBURBANPROTECTIONDISCOUNTPERCENT">
		<xsl:variable name="PROTECTIONCLASS" select="FIREPROTECTIONCLASS" />
		<xsl:choose>
			<xsl:when test="normalize-space(SUBPROTDIS) = 'Y' and PRODUCT_PREMIER != $POLICYDESC_REPAIR">
				<xsl:value-of select="$HOCoveragesDoc/PRODUCTMASTER/PRODUCT/FACTOR[@ID='PROT_DISCOUNT']/NODE[@ID='SUB_PROT_DISCOUNT']/ATTRIBUTES/@CREDIT" />
			</xsl:when>
			<xsl:otherwise>0</xsl:otherwise>
		</xsl:choose>
	</xsl:template>
	<xsl:template name="SUBURBANPROTECTIONDISCOUNTDISPLAY">
		<xsl:variable name="PROTECTIONCLASS" select="FIREPROTECTIONCLASS" />
		<xsl:variable name="VARCREDIT">
			<xsl:choose>
				<xsl:when test="normalize-space(SUBPROTDIS) = 'Y' and PRODUCT_PREMIER != $POLICYDESC_REPAIR">
					<xsl:value-of select="$HOCoveragesDoc/PRODUCTMASTER/PRODUCT/FACTOR[@ID='PROT_DISCOUNT']/NODE[@ID='SUB_PROT_DISCOUNT']/ATTRIBUTES/@CREDIT" />
				</xsl:when>
		...
		<xsl:choose>
			<xsl:when test="$VARCREDIT!=''">Applied</xsl:when>
			<xsl:otherwise>0</xsl:otherwise>
		</xsl:choose>
	</xsl:template>
	<!-- ============================================================================================ -->
	<!--				Templates for Suburban Protection Discount [Factor,Credit,Display](END)  			  -->
	<!-- ============================================================================================ -->
	<!-- ============================================================================================ -->
	<!--				Templates for Loss Free Renewal Discount and Loss Surcharge[Factor,Credit,Display](START)  			  -->
	<!-- ============================================================================================ -->
	<xsl:template name="LOSSFREECREDIT">
		<xsl:variable name="VAR_LOSS" select="TOTALLOSS" />
		<xsl:variable name="VAR_WITHWOLVERINE" select="YEARSCONTINSUREDWITHWOLVERINE" />
		<xsl:variable name="VAR_LOSSCREDIT" select="$HOCoveragesDoc/PRODUCTMASTER/PRODUCT/FACTOR[@ID='LOSS_DISCOUNT_SURCHARGE']/NODE[@ID='LFRDLS']/ATTRIBUTES[@YEARFROM &lt;= $VAR_WITHWOLVERINE and @YEARTO &gt;=$VAR_WITHWOLVERINE and @LOSSLO &lt;= $VAR_LOSS and @LOSSUP &gt;= $VAR_LOSS]/@FACTOR" />
		<xsl:choose>
			<xsl:when test="$VAR_LOSSCREDIT!=''">
				<xsl:value-of select="$VAR_LOSSCREDIT" />
			</xsl:when>
			<xsl:otherwise>1.00</xsl:otherwise>
		</xsl:choose>
	</xsl:template>
	<xsl:template name="LOSSFREECREDITPERCENTAGE">
		<xsl:variable name="VAR_LOSS" select="TOTALLOSS" />
		<xsl:variable name="VAR_WITHWOLVERINE" select="YEARSCONTINSUREDWITHWOLVERINE" />
		<xsl:variable name="VAR_LOSSCREDIT" select="$HOCoveragesDoc/PRODUCTMASTER/PRODUCT/FACTOR[@ID='LOSS_DISCOUNT_SURCHARGE']/NODE[@ID='LFRDLS']/ATTRIBUTES[@YEARFROM &lt;= $VAR_WITHWOLVERINE and @YEARTO &gt;=$VAR_WITHWOLVERINE and @LOSSLO &lt;= $VAR_LOSS and @LOSSUP &gt;= $VAR_LOSS]/@CREDIT" />
		<xsl:choose>
			<xsl:when test="$VAR_LOSSCREDIT!=''">
				<xsl:value-of select="$VAR_LOSSCREDIT" /><xsl:text>%</xsl:text>
			</xsl:when>
			<xsl:otherwise></xsl:otherwise>
		</xsl:choose>
	</xsl:template>
	<xsl:template name="LOSSFREECREDITDISPLAY">
		<xsl:variable name="VAR_LOSS" select="TOTALLOSS" />
		<xsl:variable name="VAR_WITHWOLVERINE" select="YEARSCONTINSUREDWITHWOLVERINE" />
		<xsl:variable name="VAR_LOSSCREDIT" select="$HOCoveragesDoc/PRODUCTMASTER/PRODUCT/FACTOR[@ID='LOSS_DISCOUNT_SURCHARGE']/NODE[@ID='LFRDLS']/ATTRIBUTES[@YEARFROM &lt;= $VAR_WITHWOLVERINE and @YEARTO &gt;=$VAR_WITHWOLVERINE and @LOSSLO &lt;= $VAR_LOSS and @LOSSUP &gt;= $VAR_LOSS]/@CREDIT" />
		<xsl:choose>
			<xsl:when test="$VAR_LOSSCREDIT!='' and $VAR_LOSSCREDIT!='0'">
				Applied
			</xsl:when>
			<xsl:otherwise>0</xsl:otherwise>
		</xsl:choose>
	...
	<!-- ============================================================================================ -->
	<!--				Templates for Loss Free Renewal Discount and Loss Surcharge[Factor,Credit,Display](END)  			  -->
	<!-- ============================================================================================ -->
	<!-- ============================================================================================ -->
	<!--				Templates for ACV Loss Settlement Windstorm or Hail Losses to Roof Surfacing (HO 04 93)(START)  			  -->
	<!-- ============================================================================================ -->
	<xsl:template name="ACV_LOSS_SETL_WIND_HALL_FACTOR">
		<xsl:variable name="VAR_ACV_SETL" select="HO493" />
		<xsl:variable name="VAR_PRODUCTNAME" select="PRODUCTNAME" />
		<xsl:choose>
			<xsl:when test="$VAR_ACV_SETL='Y'">
				<xsl:variable name="VAR_LOSSWINDSTORM" select="$HOCoveragesDoc/PRODUCTMASTER/PRODUCT/FACTOR[@ID='OPTIONALSECTIONCOVG']/NODE[@ID='ACV_LOSS_SETTLEMENT']/ATTRIBUTES[@FORM =$VAR_PRODUCTNAME]/@FACTOR" />
				<xsl:choose>
					<xsl:when test="$VAR_LOSSWINDSTORM!=''">
						<xsl:value-of select="$VAR_LOSSWINDSTORM" />
					</xsl:when>
					<xsl:otherwise>1.00</xsl:otherwise>
				</xsl:choose>
			</xsl:when>
	...
	<xsl:template name="ACV_LOSS_SETL_WIND_HALL_CREDIT">
		<xsl:variable name="VAR_ACV_SETL" select="HO493" />
		<xsl:variable name="VAR_PRODUCTNAME" select="PRODUCTNAME" />
		<xsl:choose>
			<xsl:when test="$VAR_ACV_SETL='Y'">
				<xsl:variable name="VAR_LOSSWINDSTORM" select="$HOCoveragesDoc/PRODUCTMASTER/PRODUCT/FACTOR[@ID='OPTIONALSECTIONCOVG']/NODE[@ID='ACV_LOSS_SETTLEMENT']/ATTRIBUTES[@FORM =$VAR_PRODUCTNAME]/@CREDIT" />
				<xsl:choose>
					<xsl:when test="$VAR_LOSSWINDSTORM!=''">
						<xsl:value-of select="$VAR_LOSSWINDSTORM" />
					</xsl:when>
	...
	<xsl:template name="ACV_LOSS_SETL_WIND_HAIL">
		<xsl:variable name="VAR_ACV_SETL" select="HO493" />
		<xsl:variable name="VAR_PRODUCTNAME" select="PRODUCTNAME" />
		<xsl:choose>
			<xsl:when test="$VAR_ACV_SETL='Y'">
				<xsl:variable name="VAR_LOSSWINDSTORM" select="$HOCoveragesDoc/PRODUCTMASTER/PRODUCT/FACTOR[@ID='OPTIONALSECTIONCOVG']/NODE[@ID='ACV_LOSS_SETTLEMENT']/ATTRIBUTES[@FORM =$VAR_PRODUCTNAME]/@FACTOR" />
				<xsl:choose>
					<xsl:when test="$VAR_LOSSWINDSTORM!=''">Applied</xsl:when>
					<xsl:otherwise>0</xsl:otherwise>
				</xsl:choose>
			</xsl:when>
			<xsl:otherwise>0</xsl:otherwise>
	...
	<!-- ============================================================================================ -->
	<!--				Templates for ACV Loss Settlement Windstorm or Hail Losses to Roof Surfacing (HO 04 93)(END)  			  -->
	<!-- ============================================================================================ -->
	<!-- ============================================================================================ -->
	...
	<xsl:template name="HEARING_AIDS">
		<xsl:choose>
			<xsl:when test="normalize-space(SCH_HEARING_AIDS_AMOUNT) != '' and SCH_HEARING_AIDS_AMOUNT &gt; 0"> 
				<xsl:variable name="VAR_RATE_PERVAL_IM" select="$HOCoveragesDoc/PRODUCTMASTER/PRODUCT/FACTOR[@ID='ENDORSEMENTS']/NODE[@ID='RATE_PERVAL_INLAND_MARINE']/ATTRIBUTES/@RATE_PERVAL_IM" />
				<xsl:variable name="VAR_TERMFACTOR">
					<xsl:call-template name="TERM_FACTOR" />
				</xsl:variable>
	...
	<xsl:template name="INSULIN_PUMPS">
		<xsl:choose>
			<xsl:when test="normalize-space(SCH_INSULIN_PUMPS_AMOUNT) != '' and SCH_INSULIN_PUMPS_AMOUNT &gt; 0"> 
				<xsl:variable name="VAR_RATE_PERVAL_IM" select="$HOCoveragesDoc/PRODUCTMASTER/PRODUCT/FACTOR[@ID='ENDORSEMENTS']/NODE[@ID='RATE_PERVAL_INLAND_MARINE']/ATTRIBUTES/@RATE_PERVAL_IM" />
				<xsl:variable name="VAR_TERMFACTOR">
					<xsl:call-template name="TERM_FACTOR" />
				</xsl:variable>
	...
	<!-- ============================================================================================ -->
	<!-- ============================================================================================ -->
	<!--					Template INLAND MARINE Camaras professional Use (start)						-->
	<!-- ============================================================================================ -->
	<xsl:template name="CAMERAS_PROF_PREMIUM">
		<xsl:variable name="VAR_SUMOFCAMERAPRM" select="user:GetTotalSum(-1)" />
		<xsl:choose>
			<xsl:when test="normalize-space(SCH_CAMERA_PROF_AMOUNT) != '' and SCH_CAMERA_PROF_AMOUNT &gt; 0"> 
				<xsl:variable name="VAR_RATE_PERVAL_IM" select="$HOCoveragesDoc/PRODUCTMASTER/PRODUCT/FACTOR[@ID='ENDORSEMENTS']/NODE[@ID='RATE_PERVAL_INLAND_MARINE']/ATTRIBUTES/@RATE_PERVAL_IM" />
				<xsl:variable name="VAR_TERMFACTOR">
					<xsl:call-template name="TERM_FACTOR" />
				</xsl:variable>
				<xsl:variable name="VAR_LIMIT" select="SCH_CAMERA_PROF_AMOUNT" />
				<xsl:variable name="VAR_DED_AMT" select="SCH_CAMERA_PROF_DED" />
				<xsl:variable name="VAR_RATE_FACTOR_FIRST_SLAB" select="$HOCoveragesDoc/PRODUCTMASTER/PRODUCT/FACTOR[@ID='ENDORSEMENTS']/NODE[@ID='CAMERA_PROF']/ATTRIBUTES[@SLAB = '1']/@RATE_PER_HUNDRED"/>
				<xsl:variable name="VAR_RATE_FACTOR_SECOND_SLAB" select="$HOCoveragesDoc/PRODUCTMASTER/PRODUCT/FACTOR[@ID='ENDORSEMENTS']/NODE[@ID='CAMERA_PROF']/ATTRIBUTES[@SLAB = '2']/@RATE_PER_HUNDRED"/>
				<xsl:variable name="VAR_RATE_FACTOR_THIRD_SLAB" select="$HOCoveragesDoc/PRODUCTMASTER/PRODUCT/FACTOR[@ID='ENDORSEMENTS']/NODE[@ID='CAMERA_PROF']/ATTRIBUTES[@SLAB = '3']/@RATE_PER_HUNDRED"/>
				
				<xsl:variable name="VAR_RATE_FIRST_SLAB_UPPER_LIMIT" select="$HOCoveragesDoc/PRODUCTMASTER/PRODUCT/FACTOR[@ID='ENDORSEMENTS']/NODE[@ID='CAMERA_PROF']/ATTRIBUTES[@SLAB = '1']/@AMOUNT_UP"/>
				<xsl:variable name="VAR_RATE_SECOND_SLAB_UPPER_LIMIT" select="$HOCoveragesDoc/PRODUCTMASTER/PRODUCT/FACTOR[@ID='ENDORSEMENTS']/NODE[@ID='CAMERA_PROF']/ATTRIBUTES[@SLAB = '2']/@AMOUNT_UP"/>
				<xsl:variable name="VAR_RATE_THIRD_SLAB_UPPER_LIMIT" select="$HOCoveragesDoc/PRODUCTMASTER/PRODUCT/FACTOR[@ID='ENDORSEMENTS']/NODE[@ID='CAMERA_PROF']/ATTRIBUTES[@SLAB = '3']/@AMOUNT_UP"/>
				<xsl:variable name="VAR_FISRT_SLAB_DIFF" select="$VAR_LIMIT - $VAR_RATE_FIRST_SLAB_UPPER_LIMIT"/>
				<xsl:variable name="VAR_SECOND_SLAB_DIFF" select="$VAR_FISRT_SLAB_DIFF - $VAR_RATE_SECOND_SLAB_UPPER_LIMIT"/>
				<xsl:choose>
					<xsl:when test="$VAR_LIMIT &gt; $VAR_RATE_FIRST_SLAB_UPPER_LIMIT">
						<xsl:variable name="VAR_RATE_FIRST_SLAB"><xsl:value-of select="round(round(($VAR_RATE_FIRST_SLAB_UPPER_LIMIT  div $VAR_RATE_PERVAL_IM )* $VAR_RATE_FACTOR_FIRST_SLAB))" /></xsl:variable>
						<xsl:variable name="VAR_TOTALFIRSTSLAB" select="user:GetTotalSum($VAR_RATE_FIRST_SLAB)" />
						<!--PREMIUM FOR SECOND SLAB  -->
						<xsl:choose>
							<xsl:when test="$VAR_FISRT_SLAB_DIFF &gt; 0 and $VAR_FISRT_SLAB_DIFF &lt;= $VAR_RATE_SECOND_SLAB_UPPER_LIMIT">
								<xsl:variable name="VAR_RATE_SECOND_SLAB"><xsl:value-of select="round(round(($VAR_FISRT_SLAB_DIFF  div $VAR_RATE_PERVAL_IM )* $VAR_RATE_FACTOR_SECOND_SLAB))" /></xsl:variable>
								<xsl:variable name="VAR_TOTALSECONDSLAB" select="user:GetTotalSum($VAR_RATE_SECOND_SLAB)" />
							</xsl:when>
						</xsl:choose>
						<!--Premium for second slab if VAR_FISRT_SLAB_DIFF also crosses second slab upper limit -->
						<xsl:choose>
							<xsl:when test="$VAR_FISRT_SLAB_DIFF &gt; 0 and $VAR_FISRT_SLAB_DIFF &gt; $VAR_RATE_SECOND_SLAB_UPPER_LIMIT">
								<xsl:variable name="VAR_RATE_SECOND_SLAB_UPPER"><xsl:value-of select="round(round(($VAR_RATE_SECOND_SLAB_UPPER_LIMIT  div $VAR_RATE_PERVAL_IM )* $VAR_RATE_FACTOR_SECOND_SLAB))" /></xsl:variable>
								<xsl:variable name="VAR_TOTALSECONDSLAB_UPPER" select="user:GetTotalSum($VAR_RATE_SECOND_SLAB_UPPER)" />
							</xsl:when>
						</xsl:choose>
						<!--Premium for third slab -->
						<xsl:choose>
							<xsl:when test="$VAR_SECOND_SLAB_DIFF &gt; 0">
								<xsl:variable name="VAR_RATE_THIRD_SLAB"><xsl:value-of select="round(round(($VAR_SECOND_SLAB_DIFF  div $VAR_RATE_PERVAL_IM )* $VAR_RATE_FACTOR_THIRD_SLAB))" /></xsl:variable>
								<xsl:variable name="VAR_TOTALTHIRDSLAB" select="user:GetTotalSum($VAR_RATE_THIRD_SLAB)" />								
							</xsl:when>
						</xsl:choose>
					</xsl:when>					
					<xsl:otherwise>
								<xsl:variable name="VAR_RATE_SLAB"><xsl:value-of select="round(round(($VAR_LIMIT  div $VAR_RATE_PERVAL_IM )* $VAR_RATE_FACTOR_FIRST_SLAB))" /></xsl:variable>
								<xsl:variable name="VAR_TOTALPRM" select="user:GetTotalSum($VAR_RATE_SLAB)" />
					</xsl:otherwise>
				</xsl:choose>
				<!--xsl:value-of select="round(round(($VAR_LIMIT  div $VAR_RATE_PERVAL_IM )* $VAR_RATE_FACTOR) * $VAR_TERMFACTOR)" /-->
				<xsl:variable name="VAR1"><xsl:value-of select="user:GetTotalSum(0)" /></xsl:variable>
				<xsl:value-of select="round($VAR1 * $VAR_TERMFACTOR)"/>	
			</xsl:when>
			<xsl:otherwise>0</xsl:otherwise>
		</xsl:choose>
	</xsl:template>
	<!-- ============================================================================================ -->
	<!--					Template INLAND MARINE Camaras professional Use (End)					  -->
	<!-- ============================================================================================ -->
	<!-- ============================================================================================ -->
	<!--					Template INLAND MARINE Musical Renumeration (Start)						  -->
	<!-- ============================================================================================ -->
	<xsl:template name="MUSICAL_RENUMERATION_PREMIUM">
		<xsl:variable name="VAR_SUMOFMUSICALPRM" select="user:GetTotalSum(-1)" />
		<xsl:choose>
			<xsl:when test="normalize-space(SCH_MUSICAL_REMUN_AMOUNT) != '' and SCH_MUSICAL_REMUN_AMOUNT &gt; 0"> 
				<xsl:variable name="VAR_RATE_PERVAL_IM" select="$HOCoveragesDoc/PRODUCTMASTER/PRODUCT/FACTOR[@ID='ENDORSEMENTS']/NODE[@ID='RATE_PERVAL_INLAND_MARINE']/ATTRIBUTES/@RATE_PERVAL_IM" />
				<xsl:variable name="VAR_TERMFACTOR">
					<xsl:call-template name="TERM_FACTOR" />
				</xsl:variable>
				<xsl:variable name="VAR_LIMIT" select="SCH_MUSICAL_REMUN_AMOUNT" />
				<xsl:variable name="VAR_DED_AMT" select="SCH_MUSICAL_REMUN_DED" />
				
				<xsl:variable name="VAR_RATE_FACTOR_FIRST_SLAB" select="$HOCoveragesDoc/PRODUCTMASTER/PRODUCT/FACTOR[@ID='ENDORSEMENTS']/NODE[@ID='MUSICAL_PROF']/ATTRIBUTES[@SLAB = '1']/@RATE_PER_HUNDRED"/>
				<xsl:variable name="VAR_RATE_FACTOR_SECOND_SLAB" select="$HOCoveragesDoc/PRODUCTMASTER/PRODUCT/FACTOR[@ID='ENDORSEMENTS']/NODE[@ID='MUSICAL_PROF']/ATTRIBUTES[@SLAB = '2']/@RATE_PER_HUNDRED"/>
				
				<xsl:variable name="VAR_RATE_FIRST_SLAB_UPPER_LIMIT" select="$HOCoveragesDoc/PRODUCTMASTER/PRODUCT/FACTOR[@ID='ENDORSEMENTS']/NODE[@ID='MUSICAL_PROF']/ATTRIBUTES[@SLAB = '1']/@AMOUNT_UP"/>
				<xsl:variable name="VAR_RATE_SECOND_SLAB_UPPER_LIMIT" select="$HOCoveragesDoc/PRODUCTMASTER/PRODUCT/FACTOR[@ID='ENDORSEMENTS']/NODE[@ID='MUSICAL_PROF']/ATTRIBUTES[@SLAB = '2']/@AMOUNT_UP"/>
				<xsl:variable name="VAR_FISRT_SLAB_DIFF" select="$VAR_LIMIT - $VAR_RATE_FIRST_SLAB_UPPER_LIMIT"/>
				...
				<xsl:choose>
					<xsl:when test="$VAR_LIMIT &gt; $VAR_RATE_FIRST_SLAB_UPPER_LIMIT">
						...
						<!--PREMIUM FOR SECOND SLAB  -->
						<xsl:choose>
							<xsl:when test="$VAR_FISRT_SLAB_DIFF &gt; 0">
								<xsl:variable name="VAR_RATE_THIRD_SLAB"><xsl:value-of select="round(round(($VAR_FISRT_SLAB_DIFF  div $VAR_RATE_PERVAL_IM )* $VAR_RATE_FACTOR_SECOND_SLAB))" /></xsl:variable>
								<xsl:variable name="VAR_TOTALTHIRDSLAB" select="user:GetTotalSum($VAR_RATE_THIRD_SLAB)" />								
							</xsl:when>
						</xsl:choose>
					</xsl:when>					
	...
	<!-- ============================================================================================ -->
	<!--					Template INLAND MARINE Musical Renumeration (End)						  -->
	<!-- ============================================================================================ -->
	<!-- ============================================================================================ -->
	...
	<xsl:template name="SEASONALSECONDARYCREDIT">
		...
		<xsl:choose>
			<xsl:when test="SEASONALSECONDARY='Y' and WOLVERINEINSURESPRIMARY='Y'">
				<xsl:choose>
					<xsl:when test="normalize-space($VAR_INSURE_PRIMARY) != '' and $VAR_INSURE_PRIMARY &gt; 0"><xsl:value-of select="round($VAR_INSURE_PRIMARY * $VAR_TERMFACTOR)" /></xsl:when>
					<xsl:otherwise>0.00</xsl:otherwise>
				</xsl:choose>
			</xsl:when>
			<xsl:when test="SEASONALSECONDARY='Y' and WOLVERINEINSURESPRIMARY='N'">
				<xsl:choose>
					<xsl:when test="normalize-space($VAR_INSURE_NON_PRIMARY) != '' and $VAR_INSURE_NON_PRIMARY &gt; 0"><xsl:value-of select="round($VAR_INSURE_NON_PRIMARY * $VAR_TERMFACTOR)" /></xsl:when>
					<xsl:otherwise>0.00</xsl:otherwise>
				</xsl:choose>
			</xsl:when>
			<xsl:otherwise>0.00</xsl:otherwise>
	...
	<xsl:template name="SEASONALSECONDARY_TEXT">
		<xsl:choose>
			<xsl:when test="SEASONALSECONDARY = 'Y' and WOLVERINEINSURESPRIMARY='Y'">
			-  Credit - Seasonal, Wolverine Insures Primary, $
			</xsl:when>
			<xsl:when test="SEASONALSECONDARY = 'Y' and WOLVERINEINSURESPRIMARY='N'">
			-  Credit - Seasonal, $
			</xsl:when>
	...
	<xsl:template name="REMARKAGEHOME">
		...
		<xsl:call-template name="AGE_OF_HOME" />
	</xsl:template>
	<!-- Protective Dvice-->
	...
	<xsl:template name="REMARKSUBURBAN">
		<xsl:text>- Discount - Suburban Protection Class </xsl:text>
	</xsl:template>
	<!--Loss free renewal    -->
	<xsl:template name="REMARKLOSSFREE">
		<xsl:variable name="VAR1"><xsl:call-template name="LOSSFREECREDIT"/></xsl:variable>
		<xsl:choose><xsl:when test="$VAR1 &lt; 1.00"><xsl:text>- Discount - Loss Free Renewal </xsl:text></xsl:when><xsl:otherwise><xsl:text>- Charge - Prior Loss </xsl:text></xsl:otherwise></xsl:choose>
	</xsl:template>
	...
	<xsl:template name="REMARKSEASONALSECONDRY">
		<xsl:choose>
			<xsl:when test="SEASONALSECONDARY = 'Y' and WOLVERINEINSURESPRIMARY='Y'">-  Credit - Secondary Residence</xsl:when>
			<xsl:when test="SEASONALSECONDARY = 'Y' and WOLVERINEINSURESPRIMARY='N'">-  Credit - Secondary Residence</xsl:when>
		</xsl:choose>
	</xsl:template>
  <xsl:template name="EXADSEASONALSECONDRY">
	<xsl:variable name="VAR_LIMIT" select="PERSONALLIABILITY_LIMIT" />
		<xsl:variable name="VAR_INSURE_PRIMARY" select="$HOCoveragesDoc/PRODUCTMASTER/PRODUCT/FACTOR[@ID='OPTIONALLIABILITYCOVG']/NODE[@ID='INSURE_PRIMARY']/ATTRIBUTES[@LIABILITY_LIMIT = $VAR_LIMIT]/@GROUP" />
		<xsl:variable name="VAR_INSURE_NON_PRIMARY" select="$HOCoveragesDoc/PRODUCTMASTER/PRODUCT/FACTOR[@ID='OPTIONALLIABILITYCOVG']/NODE[@ID='INSURE_NON_PRIMARY']/ATTRIBUTES[@LIABILITY_LIMIT = $VAR_LIMIT]/@GROUP" />
		<xsl:variable name="VAR_TERMFACTOR">
	...
	<xsl:template name="AGE_OF_HOME">
		<xsl:variable name="VAR_AGEOFHOME_FACTOR">
			<xsl:call-template name="AGEOFHOME_FACTOR"/>
		</xsl:variable>
			<xsl:choose>
			<xsl:when test="$VAR_AGEOFHOME_FACTOR &lt;= 1.00">
				<xsl:text>	-  Discount - New Home </xsl:text>
			</xsl:when>
			<xsl:otherwise>
				<xsl:text>  -  Charge - Older Home Property </xsl:text>
			</xsl:otherwise>
		</xsl:choose>
	</xsl:template>
	<xsl:template name="AGE_OF_HOME_TYPE">
		<xsl:variable name="VAR_AGEOFHOME_FACTOR">
			<xsl:call-template name="AGEOFHOME_FACTOR"/>
		</xsl:variable>
			<xsl:choose>
	...
	<xsl:template name="COMP_TYPE_LOSS">
		<xsl:variable name="VAR1"><xsl:call-template name="LOSSFREECREDIT"/></xsl:variable>
		<xsl:choose><xsl:when test="$VAR1 &lt; 1.00"><xsl:text>D</xsl:text></xsl:when><xsl:otherwise><xsl:text>S</xsl:text></xsl:otherwise></xsl:choose>
	</xsl:template>
	<!-- ============================================================================================ -->
...
<xsl:value-of select="EXTERIOR_CONSTRUCTION_DESC" /> (<xsl:value-of select="DOC" />)</xsl:template>
	...
	<xsl:template name="GROUPID3">
		<xsl:text>SECTION I - PROPERTY COVERAGES</xsl:text>
	</xsl:template>
	<xsl:template name="GROUPID4">
		<xsl:text>SECTION II - LIABILITY COVERAGES </xsl:text>
	</xsl:template>
	<xsl:template name="GROUPID5">
		<xsl:text>POLICY DISCOUNTS AND SURCHARGES </xsl:text>
	</xsl:template>
	<xsl:template name="GROUPID6">
		<xsl:text>ADDITIONAL COVERAGES</xsl:text>
	</xsl:template>
	...
	<xsl:template name="GROUPID8">
		<xsl:text>NODESCRIPTION</xsl:text>
	</xsl:template>
	<xsl:template name="GROUPID9">
		<xsl:text>NODESCRIPTION</xsl:text>
	</xsl:template>
	<!--Step Details-->
	<xsl:template name="STEPID0">
		<xsl:text>		-  Coverage A - Property </xsl:text>
	</xsl:template>
	<xsl:template name="STEPID1">
		<xsl:text>		-  Coverage B - Other Structures</xsl:text>
	</xsl:template>
	<xsl:template name="STEPID2">
		<xsl:text>		-  Coverage C - Personal Property</xsl:text>
	</xsl:template>
	<xsl:template name="STEPID3">
		<xsl:text>		-  Coverage D - Loss Of Use  </xsl:text>
	</xsl:template>
	<xsl:template name="STEPID4">
		<xsl:text>		-  Coverage E - Personal Liability - Each Occurrence</xsl:text>
	</xsl:template>
	<xsl:template name="STEPID5">
		<xsl:text>		-  Coverage F - Medical Payments to Others - Each Person</xsl:text>
	</xsl:template>
	<xsl:template name="STEPID6">
		...
		<xsl:choose>
			<xsl:when test="$VAR_INSURANCESCORE_FACTOR &gt; 1.00">
				...
				<xsl:call-template name="INSURANCE_SCORE_DISPLAY" />
				<xsl:text>) - </xsl:text>
				<xsl:value-of select="$VAR_INSURANCESCORE_PERCENT" />
			</xsl:when>
			<xsl:otherwise>
				<xsl:text>-  Discount - Insurance Score Credit (Score - </xsl:text>
				<xsl:call-template name="INSURANCE_SCORE_DISPLAY" />
	...
	<xsl:template name="STEPID8">
		<xsl:text>		-  NOTE: Discounts have 50% MAXIMUM DISCOUNT rule applied</xsl:text>
	</xsl:template>
	...
	<xsl:template name="STEPID19">
		<xsl:variable name="VAR1"><xsl:call-template name="LOSSFREECREDIT"/></xsl:variable>
		<xsl:choose><xsl:when test="$VAR1 &lt; 1.00"><xsl:text>- Discount - Loss Free Renewal </xsl:text></xsl:when><xsl:otherwise><xsl:text>- Charge - Prior Loss </xsl:text></xsl:otherwise></xsl:choose><xsl:call-template name="LOSSFREECREDITPERCENTAGE" />
	</xsl:template>
	...
	<xsl:template name="STEPID22">
		<xsl:call-template name="SEASONALSECONDARY_TEXT" /><xsl:call-template name="SEASONALSECONDARYCREDIT" />
	</xsl:template>
	<xsl:template name="STEPID23">
		<xsl:text>	-  </xsl:text>
		<xsl:call-template name="SELECT_HO_20_PREFERRED_PLUS_OR_HO_21_VIP"></xsl:call-template>
	</xsl:template>
	<xsl:template name="STEPID24">
		<xsl:text>	-  HO-64 Renters Deluxe Endorsement </xsl:text>
	</xsl:template>
	<xsl:template name="STEPID25">
		<xsl:text>	-  HO-66 Condominium Deluxe Endorsement</xsl:text>
	</xsl:template>
	<xsl:template name="STEPID26">
		<xsl:text>	-  HO-34 Replacement Cost Personal Property</xsl:text>
	</xsl:template>
	<!-- <xsl:template name = "STEPID1001">	-  HO-64 Renters Deluxe Endorsement </xsl:template> -->
	<xsl:template name="STEPID27">
		<xsl:text>	-  HO-11 Expanded Replacement Cost for Building</xsl:text>
	</xsl:template>
	<xsl:template name="STEPID28">
		<xsl:text>	-  HO-96 Fire Department Service Charge (</xsl:text>
		<xsl:value-of select="HO96ADDITIONAL" />
		<xsl:text> increase)</xsl:text>
	</xsl:template>
	<xsl:template name="STEPID29">
		<xsl:text>	-  HO-312 Business Property Increase Limits (</xsl:text>
		<xsl:value-of select="HO312ADDITIONAL" />
		<xsl:text> increase)</xsl:text>
	</xsl:template>
	<xsl:template name="STEPID30">
		<xsl:text>	-  HO-48 Other Structures Increased Limit</xsl:text>
	</xsl:template>
	<xsl:template name="STEPID31">
		<xsl:text>	-  HO-40 Other Structure Rented To Others (</xsl:text>
		<xsl:value-of select="HO40ADDITIONAL" />
		<xsl:text> increase)</xsl:text>
	</xsl:template>
	<xsl:template name="STEPID32">
		<xsl:text>	-  HO-42 Other Structure With Incidental</xsl:text>
	</xsl:template>
	<xsl:template name="STEPID33">
		<xsl:text>	-  HO-50 Personal Property Away From Premises (Increase </xsl:text>
		<xsl:value-of select="PERSONALPROPERTYAWAYADDITIONAL" />
		<xsl:text>)</xsl:text>
	</xsl:template>
	<xsl:template name="STEPID34">
		<xsl:text>	-  Building Additions and Alterations (HO-51) (Increase </xsl:text>
		<xsl:value-of select="format-number(PERSONALPROPERTYINCREASEDLIMITADDITIONAL,'#')" />
		<xsl:text> )  </xsl:text>
	</xsl:template>
	<xsl:template name="STEPID35">
		<xsl:text>	-  HO-53 Credit Card and Depositors Forgery (</xsl:text>
		<xsl:value-of select="HO53ADDITIONAL" />
		<xsl:text> increase)</xsl:text>
	</xsl:template>
	<xsl:template name="STEPID36">
		<xsl:call-template name="HO-65_HO-211" />
	</xsl:template>
	<xsl:template name="STEPID37">
		<xsl:text>	-:    Unscheduled Jewelry And Furs (</xsl:text>
		<xsl:value-of select="UNSCHEDULEDJEWELRYADDITIONAL" />
		<xsl:text> increase)</xsl:text>
	</xsl:template>
	<xsl:template name="STEPID38">
		<xsl:text>	-:    Money (</xsl:text>
		<xsl:value-of select="MONEYADDITIONAL" />
		<xsl:text> increase)</xsl:text>
	</xsl:template>
	<xsl:template name="STEPID39">
		<xsl:text>	-:    Securities (</xsl:text>
		<xsl:value-of select="SECURITIESADDITIONAL" />
		<xsl:text> increase)</xsl:text>
	</xsl:template>
	<xsl:template name="STEPID40">
		<xsl:text>	-:    Silverware, Goldware And Pewterware (</xsl:text>
		<xsl:value-of select="SILVERWAREADDITIONAL" />
		<xsl:text> increase)</xsl:text>
	</xsl:template>
	<xsl:template name="STEPID41">
		<xsl:text>	-:    Firearms (</xsl:text>
		<xsl:value-of select="FIREARMSADDITIONAL" />
		<xsl:text> increase)</xsl:text>
	</xsl:template>
	<xsl:template name="STEPID42">
		<xsl:text>	-  HO-35 Loss Assessment Coverage(</xsl:text>
		<xsl:value-of select="HO35ADDITIONAL" />
		<xsl:text> increase)</xsl:text>
	</xsl:template>
	<xsl:template name="STEPID43">
		<xsl:text>	-  HO-277 Ordinance or Law</xsl:text>
	</xsl:template>
	<xsl:template name="STEPID44">
		<xsl:text>	-  HO-32 Unit Owners Coverage A Special Coverage</xsl:text>
	</xsl:template>
	<xsl:template name="STEPID45">
		<xsl:text>	-  HO-455 Identity Fraud Expense Coverage</xsl:text>
	</xsl:template>
	<xsl:template name="STEPID46">
		<xsl:text>	-  HO-315 Earthquake (Zone </xsl:text>
		<xsl:value-of select="EARTHQUAKEZONE" />
		<xsl:text>)</xsl:text>
	</xsl:template>
	<xsl:template name="STEPID47">
		<xsl:text>	-  HO-327 Water Back-Up and Sump Pump Overflow</xsl:text>
	</xsl:template>
	<xsl:template name="STEPID48">
		<xsl:text>	-  HO-33 Condo-Unit Owners Rental to Others </xsl:text>
		<xsl:value-of select="HO33" />
	</xsl:template>
	<xsl:template name="STEPID49">
		<xsl:text>	-  HO-373 Contingent Workers Compensation</xsl:text>
	</xsl:template>
	<xsl:template name="STEPID50">
		<xsl:text>	-  HO-287 Mine Subsidence Coverage</xsl:text>
	</xsl:template>
	<xsl:template name="STEPID51">
		<xsl:text>	-  HO-9 Collapse From Sub-Surface Water</xsl:text>
	</xsl:template>
	<xsl:template name="STEPID52">
		<xsl:text>	-  HO-490 Specific Structures Away From Premises</xsl:text>
	</xsl:template>
	<xsl:template name="STEPID53">
		<xsl:text>	-  HO-489 Other Structures Repair Cost Coverage  (</xsl:text>
		<xsl:value-of select="REPAIRCOSTADDITIONAL" />
		<xsl:text> increase)</xsl:text>
	</xsl:template>
	<xsl:template name="STEPID54">
		<xsl:text>	-  Primary Residence</xsl:text>
	</xsl:template>
	<xsl:template name="STEPID55">
		<xsl:text>	-  Residence Employees(</xsl:text>
		<xsl:value-of select="RESIDENCE_EMP_NUMBER" />
		<xsl:text>)</xsl:text>
	</xsl:template>
	<xsl:template name="STEPID56">
		<xsl:text>	-  Additional Premises Occupied By Insured (</xsl:text>
		<xsl:value-of select="format-number(OCCUPIED_INSURED, '#')" />
		<xsl:text>)</xsl:text>
	</xsl:template>
	<xsl:template name="STEPID57">
		<xsl:text>	-  HO-40 Additional Premises Rented to Others - Residence Premises (</xsl:text>
		<xsl:value-of select="format-number(RESIDENCE_PREMISES,'#')"></xsl:value-of>
		<xsl:text>)</xsl:text>
	</xsl:template>
	<xsl:template name="STEPID58">
		<xsl:text>	-  HO-70 Additional Premises Rented - Other Location - 1 Family (</xsl:text>
		<xsl:value-of select="format-number(OTHER_LOC_1FAMILY,'#')" />
		<xsl:text>)</xsl:text>
	</xsl:template>
	<xsl:template name="STEPID59">
		<xsl:text>	-  HO-70 Additional Premises Rented - Other Location - 2 Family (</xsl:text>
		<xsl:value-of select="format-number(OTHER_LOC_2FAMILY,'#')" />
		<xsl:text>)</xsl:text>
	</xsl:template>
	<xsl:template name="STEPID60">
		<xsl:text>	-  HO-42 Incidental Office Private School or Studio On Premises</xsl:text>
	</xsl:template>
	<xsl:template name="STEPID61">
		<xsl:text>	-      Business Located in an Other Structure</xsl:text>
	</xsl:template> <!-- SKB-->
	<xsl:template name="STEPID62">
		<xsl:text>	-  HO-42 Incidental Office Private School or Studio Instruction Only</xsl:text>
	</xsl:template>
	<xsl:template name="STEPID63">
		<xsl:text>	-  HO-200 Waterbed Liability</xsl:text>
	</xsl:template>
	<xsl:template name="STEPID64">
		<xsl:text>	-  HO-43 Incidental Office Private School or Studio Off Premises</xsl:text>
	</xsl:template>
	<xsl:template name="STEPID65">
		<xsl:text>	-  HO-71 Business Pursuits</xsl:text>
	</xsl:template>
	<xsl:template name="STEPID66">
		<xsl:text>	-    Class A - (1 persons)</xsl:text>
	</xsl:template>
	<xsl:template name="STEPID67">
		<xsl:text>	-    Class B - (1 persons)</xsl:text>
	</xsl:template>
	<xsl:template name="STEPID68">
		<xsl:text>	-    Class C - (1 persons)</xsl:text>
	</xsl:template>
	<xsl:template name="STEPID69">
		<xsl:text>	-    Class D - (1 persons)</xsl:text>
	</xsl:template>
	<xsl:template name="STEPID70">
		<xsl:text>	-  HO-72 Farm Liability - Incidental Farming Residence Premises</xsl:text>
	</xsl:template>
	<xsl:template name="STEPID71">
		<xsl:text>	-  HO-73 Farm Liability - Owned By Insured Rented to Others(</xsl:text>
		<xsl:value-of select="format-number(OTH_LOC_OPR_OTHERS_HO73,'#')" />
		<xsl:text>)</xsl:text>
	</xsl:template>
	<xsl:template name="STEPID72">
		<xsl:text>	-  HO-73 Farm Liability - Owned/Operated By Insured/Insured Employee(</xsl:text>
		<xsl:value-of select="format-number(OTH_LOC_OPR_EMPL_HO73,'#')" />
		<xsl:text>)</xsl:text>
	</xsl:template>
	<xsl:template name="STEPID73">
		<xsl:text>	-  HO-82 Personal Injury</xsl:text>
	</xsl:template>
	<xsl:template name="STEPID74">
		<xsl:text>	-  Bicycles (HO-900)</xsl:text>
	</xsl:template>
	<xsl:template name="STEPID75">
		<xsl:text>	-  Cameras (Non Professional)</xsl:text>
	</xsl:template>
	<xsl:template name="STEPID76">
		<xsl:text>	-  Cellular Phones (HO-900)</xsl:text>
	</xsl:template>
	<xsl:template name="STEPID77">
		<xsl:text>	-  Furs</xsl:text>
	</xsl:template>
	<xsl:template name="STEPID78">
		<xsl:text>	-  Golf Equipment</xsl:text>
	</xsl:template>
	<xsl:template name="STEPID79">
		<xsl:text>	-  Guns</xsl:text>
	</xsl:template>
	<xsl:template name="STEPID80">
		<xsl:text>	-  Jewelry</xsl:text>
	</xsl:template>
	<xsl:template name="STEPID81">
		<xsl:text>	-  Musical Instruments(non professional)(HO-61)</xsl:text>
	</xsl:template>
	<xsl:template name="STEPID82">
		<xsl:text>	-  Personal Computers - Desktop (HO-214)</xsl:text>
	</xsl:template>
	<xsl:template name="STEPID83">
		<xsl:text>	-  Silver</xsl:text>
	</xsl:template>
	<xsl:template name="STEPID84">
		<xsl:text>	-  Stamps</xsl:text>
	</xsl:template>
	<xsl:template name="STEPID85">
		<xsl:text>	-  Rare Coins</xsl:text>
	</xsl:template>
	<xsl:template name="STEPID86">
		<xsl:text>	-  Fine Arts - Without Breakage</xsl:text>
	</xsl:template>
	<xsl:template name="STEPID87">
		<xsl:text>	-  Fine Arts - With Breakage</xsl:text>
	</xsl:template>
	<xsl:template name="STEPID88">
		<xsl:text>	-  Handicap Electronics</xsl:text>
	</xsl:template>
	<xsl:template name="STEPID89">
		<xsl:text>	-  Hearing Aids</xsl:text>
	</xsl:template>
	<xsl:template name="STEPID90">
		<xsl:text>	-  Insulin Pumps</xsl:text>
	</xsl:template>
	<xsl:template name="STEPID91">
		<xsl:text>	-  Mary Kay/Amway</xsl:text>
	</xsl:template>
	<xsl:template name="STEPID92">
		<xsl:text>	-  Personal Computers - Laptop (HO-214)</xsl:text>
	</xsl:template>
	<xsl:template name="STEPID93">
		<xsl:text>	-  Salesman Supplies</xsl:text>
	</xsl:template>
	<xsl:template name="STEPID94">
		<xsl:text>	-  Scuba Diving Equipment</xsl:text>
	</xsl:template>
	<xsl:template name="STEPID95">
		<xsl:text>	-  Snow Skis</xsl:text>
	</xsl:template>
	<xsl:template name="STEPID96">
		<xsl:text>	-  Tack and Saddle</xsl:text>
	</xsl:template>
	<xsl:template name="STEPID97">
		<xsl:text>	-  Tools (At Premises)</xsl:text>
	</xsl:template>
	<xsl:template name="STEPID98">
		<xsl:text>	-  Tools (At Business Location)</xsl:text>
	</xsl:template>
	<xsl:template name="STEPID99">
		<xsl:text>	-  Tractors</xsl:text>
	</xsl:template>
	<xsl:template name="STEPID100">
		<xsl:text>	-  Train Collections</xsl:text>
	</xsl:template>
	<xsl:template name="STEPID101">
		<xsl:text>	-  Wheelchairs/Scooters</xsl:text>
	</xsl:template>
	<xsl:template name="STEPID102">
		<xsl:text>	-  Cameras (Professional Use)</xsl:text>
	</xsl:template>
	<xsl:template name="STEPID103">
		<xsl:text>	-  Musical Instruments (Used for Remuneration)</xsl:text>
	</xsl:template>
	<xsl:template name="STEPID104">
		<xsl:text>	-  Property Expense Fee</xsl:text>
	</xsl:template>
	<xsl:template name="STEPID105">
		<xsl:text>	-   Additional Premium charged to meet the minimum premium ($</xsl:text>
		<xsl:value-of select="$HOCoveragesDoc/PRODUCTMASTER/PRODUCT/FACTOR[@ID='MINIMUM_HOME_PREMIUM']/NODE[@ID ='MINIMUM_PREMIUM']/ATTRIBUTES/@MINIMUM_VALUE" />
		<xsl:text>)</xsl:text>
	</xsl:template>
	<xsl:template name="STEPID106">
	...
	<xsl:template name="ADDITIONALCOVERAGE">
		...
		<xsl:variable name="V65">
			<xsl:choose>
				<xsl:when test="PRODUCTNAME='HO-6'">
					<!--xsl:call-template name="COVERAGE_D_PREMIUM" /-->0
				</xsl:when>
				<xsl:otherwise>0</xsl:otherwise>
			</xsl:choose>
		</xsl:variable>
		...
		<xsl:value-of select="$V1+$V2+$V4+$V5+$V6+$V7+$V8+$V9+$V10+$V11+$V12+$V13+$V14+$V15+$V16+$V17+$V18+$V19+$V20+$V21+$V22+$V23+$V24+$V25+$V26+$V27+$V28+$V29+$V30+$V31+$V32+$V33+$V34+$V35+$V36+$V37+$V38+$V39+$V40+$V41+$V42+$V43+$V44+$V45+$V46+$V47+$V48+$V49+$V50+$V51+$V52+$V53+$V54+$V55+$V56+$V57+$V59+$V60+$V61+$V62+$V63+$V64+$V65+$V66+$V67+$V68+$V69+$V70+$V71+$V72+$V73+$V74+$V75+$V76+$V77+$V78+$V79" />
	</xsl:template>
	<!-- ============================================================================================ -->
	...
	<xsl:template name="FINALPRIMIUM">
		...
		<!-- For Ho-4 and Ho-6 add increased coverage c And d premium in final premium -->
		...
		<xsl:choose>
			<xsl:when test="PRODUCTNAME = $POLICYTYPE_HO4 or PRODUCTNAME = $POLICYTYPE_HO6">
				<xsl:value-of select="$VAR_ADDITIONAL_AMOUNT_COVERAGE + $VAR_CALL_ADJUSTEDBASE + $VAR_ADDITIONALCOVERAGE + $VAR_MINIMUM_PREMIUM + $VAR_INCREASED_COV_C + $VAR_INCREASED_COV_D" />
			</xsl:when>
			<xsl:otherwise>
	...
	<!-- ###################################  START - MICHIGAN ###################################### -->
	<!-- ============================================================================================ -->
	<!--							ADJUSTEDBASE FOR MICHIGAN(REGULAR) HO2			
		1. Base (Find Base in Territory By Policy Form and Protection Class.) X
		2. Coverage A Factor X
		3. 1.00 if insured at 100% to Replacement Cost or 1.18 if less X
		...
		10. HO14 Dwelling Under Construction Credit (if applies then no: Age of Home, or Protective Device Credit) X
		11. Experience (Maturity or Age of Insured) Factor X
		12. Multiple Policy (Auto & Home) Factor X
		13. Suburban Protection Discount.
		14. Loss Free Renewal or Loss Adjustment.
		15. Surcharges
		16. Apply Section II Credits if any ($15 if Secondary and we have the Primary; or $2 for Secondary and No
		Primary).
		17. Add Any Other Endorsement Option Charges.
		18. Add Section II Liability Charges (if any).
		19. Add Michigan Property Expense Recoupment Charge.-->
	<!-- ============================================================================================ -->
	<xsl:template name="ADJUSTEDBASE_REGULAR_MICHIGAN_HO2_HO3">
		...
		<!--PLEASE DO NOT CHANGE THE SEQUENCE OF THE NEXT 6 -->
		<xsl:variable name="VAR_ACV_LOSS_SETL_WIND_HALL">
			<xsl:call-template name="GET_ACV_LOSS_SETL_WIND_HALL" />
		</xsl:variable>
		<xsl:variable name="VAR_AGEOFHOMEFACTOR">
			<xsl:call-template name="GET_AGE_OF_HOME_DISCOUNT" />
		</xsl:variable>
		...
		<xsl:variable name="VAR_SUB_PROTECTION_DISCOUNT">
			<xsl:call-template name="GET_SUBURBANPROTECTION_DISCOUNT" />
		</xsl:variable>
		<xsl:variable name="VAR_LOSSCREDIT">
			<xsl:call-template name="GET_LOSS_FREE_DISCOUNT" />
		</xsl:variable>
		...
		<!--xsl:variable name="VAR_MICHIGAN_PROPERTY_EXPENSE_RECOUPMENT">
			<xsl:call-template name="MICHIGAN_PROPERTY_EXPENSE_RECOUPMENT" />
		</xsl:variable-->
		<!-- return value of adjusted base -->
		...
		<xsl:variable name="FIFTH_STEP_1" select="$VAR5 * $VAR_ACV_LOSS_SETL_WIND_HALL" />
		<xsl:variable name="VAR_5" select="round(format-number($FIFTH_STEP_1,'##.0000'))" />
		<xsl:variable name="SIX_STEP" select="$VAR_5 * $VAR_AGEOFHOMEFACTOR" />
		...
		<xsl:variable name="ELEVENTH_STEP" select="$VAR10 * $VAR_SUB_PROTECTION_DISCOUNT" />
		<xsl:variable name="VAR11" select="round(format-number($ELEVENTH_STEP,'##.0000'))" />
		<xsl:variable name="TWLVENTH_STEP" select="$VAR11 * $VAR_LOSSCREDIT" />
		...
		<!--xsl:value-of select="($VAR13 -$VAR_SEASONALSECONDARYCREDIT) + $VAR_MEDICAL_CHARGES + $VAR_LIABLITY_CHARGES + $VAR_MICHIGAN_PROPERTY_EXPENSE_RECOUPMENT" /-->
...
((((((((((<xsl:call-template name="DEWLLING-MAIN"></xsl:call-template> )*
	...
	<!--    1. Base (Find Base in Territory By Policy Form and Protection Class.) X
		...
		11. Experience (Maturity or Age of Insured) Factor X
		12. Multiple Policy (Auto & Home) Factor X
		13. Valued CustomerRenewal Credit Factor X
		14. Surcharges
...
-->
	...
	11. HO14 Dwelling Under Construction Credit (if applies then no: Age of Home, or Protective Device Credit) X
	...
	<xsl:template name="ADJUSTEDBASE_PREMIER_MICHIGAN_HO3">
		...
		<xsl:variable name="FIFTH_STEP_1" select="$VAR5 * $VAR_ACV_LOSS_SETL_WIND_HALL" />
		<xsl:variable name="VAR_5" select="round(format-number($FIFTH_STEP_1,'##.0000'))" />
		<xsl:variable name="SIXTH_STEP" select="$VAR_5 * $VAR_MULTIPOLICY_DISCOUNT" />
		<xsl:variable name="VAR6" select="round(format-number($SIXTH_STEP,'##.0000'))" />
	...
	<!-- 
	1. Base (Find Base in Territory By Policy Form 4 or 6, COI or COII, and Protection Class.) X
	2. Coverage C Factor (Tables based on whether is HO4 with minimum $10,000 or HO-6 minimum $15,000.) X
	...
	8. Experience (Maturity or Age of Insured) Factor X
	9. Multiple Policy (Auto & Home) Factor (HO4 must be minimum $15,000 Coverage C to get credit. HO6 no
	minimum.) X
	10. Loss Free Renewal or Loss Adjustment.
	11. Coverage A is 10% of Coverage C ($2,000 minimum), if Coverage A increased and there is no HO66
	endorsement then rate is $2 per $1,000 increase. If an HO-66 Deluxe endorsement then charge is $2.20 per
	$1,000 increased above automatic amount.
	12. Surcharges
	13. Apply Section II Credits if any ($9 if Secondary and we have the Primary; or $2 for Secondary and No Primary).
	14. Add Any Other Endorsement Option Charges.
	15. Add Section II Liability Charges (if any).
	16. Add Michigan Property Expense Recoupment Fee.	
	 -->
	<!-- ============================================================================================ -->
	<xsl:template name="ADJUSTEDBASE_REGULAR_MICHIGAN_HO4_HO6">
		...
		<xsl:variable name="VAR_LOSSCREDIT">
			<xsl:call-template name="LOSSFREECREDIT" />
		</xsl:variable>
		<xsl:variable name="VAR_VALUEDCUSTOMER">
			<xsl:call-template name="VALUEDCUSTOMER" />
		</xsl:variable>
		...
		<xsl:variable name="THIRD_STEP" select="$VAR2 * $VAR_INSURANCESCORE" />
		<xsl:variable name="VAR3" select="round(format-number($THIRD_STEP,'##.0000'))" />
		<xsl:variable name="FOURTH_STEP" select="$VAR3 * $VAR_TERMFACTOR" />
		<xsl:variable name="VAR4" select="round(format-number($FOURTH_STEP,'##.0000'))" />
		<xsl:variable name="FIFTH_STEP" select="$VAR4 * $VAR_DFACTOR" />
		<xsl:variable name="VAR5" select="round(format-number($FIFTH_STEP,'##.0000'))" />
		<xsl:variable name="FIFTH_STEP_1" select="$VAR5 * $VAR_ACV_LOSS_SETL_WIND_HALL" />
		<xsl:variable name="VAR_5" select="round(format-number($FIFTH_STEP_1,'##.0000'))" />
		<xsl:variable name="SIXTH_STEP" select="$VAR_5 * $VAR_PROTECTIVE_DEVICE" />
		<xsl:variable name="VAR6" select="round(format-number($SIXTH_STEP,'##.0000'))" />
		<xsl:variable name="SEVENTH_STEP" select="$VAR6 * $VAR_EXPERIENCE_DISCOUNT" />
		<xsl:variable name="VAR7" select="round(format-number($SEVENTH_STEP,'##.0000'))" />
		<xsl:variable name="EIGHTTH_STEP" select="$VAR7 * $VAR_MULTIPOLICY_DISCOUNT" />
		<xsl:variable name="VAR8" select="round(format-number($EIGHTTH_STEP,'##.0000'))" />
		<xsl:variable name="NINTH_STEP" select="$VAR8 * $VAR_LOSSCREDIT" />
		<xsl:variable name="VAR9" select="round(format-number($NINTH_STEP,'##.0000'))" />
		<xsl:variable name="TENTH_STEP" select="$VAR9 * $VAR_WSTOVE" />
		<xsl:variable name="VAR10" select="round($TENTH_STEP)" />
		<!--xsl:value-of select="(($VAR14  - $VAR_SEASONALSECONDARYCREDIT) + $VAR_MEDICAL_CHARGES + $VAR_LIABLITY_CHARGES + $VAR_MICHIGAN_PROPERTY_EXPENSE_RECOUPMENT)" /-->
		<xsl:value-of select="(($VAR10  - $VAR_SEASONALSECONDARYCREDIT) + $VAR_MEDICAL_CHARGES + $VAR_LIABLITY_CHARGES)" />
	</xsl:template>
	<!-- ============================================================================================ -->
	<!--							ADJUSTEDBASE FOR MICHIGAN(REGULER) HO5					  -->
	<!-- 
	1. Base (Find Base in Territory By Policy Form and Protection Class.) X
	2. Coverage A Factor (Note Minimum Coverage A is $200,000.) X
	3. 1.00 if insured to Replacement Cost (HO5 Must Be at 100%Coverage A to Replacement Cost to be eligible.)
	X
	4. Term Factor: 1.00 Annual or .50 SemiAnnual X
	5. Deductible Factor X
	6. Add HO34 Replacement Cost Personal Property Endorsement = 10% Minimum $35.
	7. Add HO11 Expanded Replacement Coverage for Building = $15 Flat Charge
	8. Add HO23 Premier V.I.P. End = $20
	9. Insurance Score Discount Factor X
	10. Age of Home Factor X
	11. Protective Device Factor X
	12. HO14 Dwelling Under Construction Credit (if applies then no: Age of Home, or Protective Device Credit) X
	...
	<xsl:template name="ADJUSTEDBASE_REGULAR_MICHIGAN_HO5">
		...
		<xsl:variable name="VAR3" select="round(format-number($THIRD_STEP,'##.0000'))" />
		<xsl:variable name="FOURTH_STEP" select="$VAR3 * $VAR_DFACTOR" />
		<xsl:variable name="VAR4" select="round($FOURTH_STEP)" />
		<xsl:variable name="FIFTH_STEP" select="$VAR4 + $VAR_HO_34_REPLACEMENT_COST" />
		<xsl:variable name="VAR5" select="round(format-number($FIFTH_STEP,'##.0000'))" />
		<xsl:variable name="SIXTH_STEP" select="$VAR5 + $VAR_HO_11_REPLACEMENT_COST_PERSONAL_PROPERTY" />
		<xsl:variable name="VAR6" select="round(format-number($SIXTH_STEP,'##.0000'))" />
		<xsl:variable name="SEVENTH_STEP" select="$VAR6 + $VAR_HO_20_PREFERRED_PLUS_OR_HO_21_VIP" />
		<xsl:variable name="VAR7" select="round(format-number($SEVENTH_STEP,'##.0000'))" />
		<xsl:variable name="EIGHT_STEP" select="$VAR7 * $VAR_INSURANCESCORE" />
		<xsl:variable name="VAR9" select="round(format-number($EIGHT_STEP,'##.0000'))" />
		<xsl:variable name="EIGHT_STEP_1" select="$VAR9 * $VAR_ACV_LOSS_SETL_WIND_HALL" />
		<xsl:variable name="VAR_9" select="round(format-number($EIGHT_STEP_1,'##.0000'))" />
		<xsl:variable name="NINE_STEP" select="$VAR_9 * $VAR_AGEOFHOMEFACTOR" />
		<xsl:variable name="VAR10" select="round(format-number($NINE_STEP,'##.0000'))" />
		<xsl:variable name="TENTH_STEP" select="$VAR10 * $VAR_PROTECTIVE_DEVICE" />
		<xsl:variable name="VAR11" select="round(format-number($TENTH_STEP,'##.0000'))" />
		<xsl:variable name="ELEVENTH_STEP" select="$VAR11 * $VAR_DWELLING_UNDER_CONSTR" />
		<xsl:variable name="VAR12" select="round(format-number($ELEVENTH_STEP,'##.0000'))" />
		<xsl:variable name="TWELTH_STEP" select="$VAR12 * $VAR_EXPERIENCE_DISCOUNT" />
		<xsl:variable name="VAR13" select="round(format-number($TWELTH_STEP,'##.0000'))" />
		<xsl:variable name="THERTEENTH_STEP" select="$VAR13 * $VAR_MULTIPOLICY_DISCOUNT" />
		<xsl:variable name="VAR14" select="round(format-number($THERTEENTH_STEP,'##.0000'))" />
		<xsl:variable name="FOURTEENTH_STEP" select="$VAR14 * $VAR_LOSSCREDIT" />
		<xsl:variable name="VAR15" select="round(format-number($FOURTEENTH_STEP,'##.0000'))" />
		<xsl:variable name="FIFTEENTH_STEP" select="$VAR15 * $VAR_WSTOVE" />
		<xsl:variable name="VAR16" select="round($FIFTEENTH_STEP)" />
		<!--xsl:value-of select="(($VAR16 + $VAR_INCREASED_COV_C + $VAR_INCREASED_COV_D) - $VAR_SEASONALSECONDARYCREDIT) + $VAR_MEDICAL_CHARGES + $VAR_LIABLITY_CHARGES + $VAR_MICHIGAN_PROPERTY_EXPENSE_RECOUPMENT" /-->
		<xsl:value-of select="(($VAR16 + $VAR_INCREASED_COV_C + $VAR_INCREASED_COV_D) - $VAR_SEASONALSECONDARYCREDIT) + $VAR_MEDICAL_CHARGES + $VAR_LIABLITY_CHARGES" />
	</xsl:template>
	<!-- ============================================================================================ -->
	...
	<xsl:template name="ADJUSTEDBASE_PREMIER_MICHIGAN_HO5">
		...
		<xsl:variable name="SEVENTH_STEP" select="$VAR6 + $VAR_HO_20_PREFERRED_PLUS_OR_HO_21_VIP" />
		<xsl:variable name="VAR7" select="round(format-number($SEVENTH_STEP,'##.0000'))" />
		<xsl:variable name="NINTH_STEP" select="$VAR7 * $VAR_INSURANCESCORE" />
		<xsl:variable name="VAR9" select="round(format-number($NINTH_STEP,'##.0000'))" />
		...
		<xsl:variable name="FIFTEENTH_STEP" select="$VAR14 * $VAR_SUB_PROTECTION_DISCOUNT" />
		<xsl:variable name="VAR15" select="round(format-number($FIFTEENTH_STEP,'##.0000'))" />
		<xsl:variable name="SIXTEENTH_STEP" select="$VAR15 * $VAR_LOSSCREDIT" />
		...
		<!--xsl:value-of select="($VAR17 -$VAR_SEASONALSECONDARYCREDIT) + $VAR_INCREASED_COV_C + $VAR_INCREASED_COV_D + $VAR_MEDICAL_CHARGES + $VAR_LIABLITY_CHARGES + $VAR_MICHIGAN_PROPERTY_EXPENSE_RECOUPMENT" /-->
	...
	3. 1.00 if insured at 100% to Replacement Cost or 1.18 if less X
	4. Insurance Score Discount Factor X
	5. Term Factor: 1.00 Annual or 0.50 Semi-Annual X
	6. Deductible Factor X
	7. Age of Home Factor X
	8. Charge for Increased Coverage C or Coverage D (if any)
	9. Protective Device Factor X
	10. NonSmoker or Smoker Factor X
	11. HO14 Dwelling Under Construction Credit (if applies then no: Age of Home, or Protective Device Credit) X
	12. Experience (Maturity or Age of Insured) Factor X
	13. Multiple Policy (Auto & Home) Factor X
	14. Suburban Protection Discount X
	15. Loss Free Renewal Discount or Loss Surcharge X
	16. Surcharge  Woodstove
	17. ApplySection II Credits if any ($9 if Secondary and we have the Primary; or $2 for Secondary and No Primary).
	18. Add: Any Other Endorsement Option Charges.
	19. Add: Section II Liability Charges (if any).													-->
	<!-- ============================================================================================ -->
	<xsl:template name="ADJUSTEDBASE_REGULAR_INDIANA_HO2_HO3">
		...
		<xsl:variable name="VAR7" select="round(format-number($SEVENTH_STEP,'##.0000'))" />
		<xsl:variable name="EIGHT_STEP" select="$VAR7 * $VAR_SMOKER" />
		<xsl:variable name="VAR8" select="round(format-number($EIGHT_STEP,'##.0000'))" />
		<xsl:variable name="NINTH_STEP" select="$VAR8 * $VAR_DWELLING_UNDER_CONSTR" />
		<xsl:variable name="VAR9" select="round(format-number($NINTH_STEP,'##.0000'))" />
		<xsl:variable name="TENTH_STEP" select="$VAR9 * $VAR_EXPERIENCE_DISCOUNT" />
		...
		<xsl:variable name="VAR11" select="round(format-number($ELEVENTH_STEP,'##.0000'))" />
		<xsl:variable name="TWLVENTH_STEP" select="$VAR11 * $VAR_SUB_PROTECTION_DISCOUNT" />
		<xsl:variable name="VAR12" select="round(format-number($TWLVENTH_STEP,'##.0000'))" />
		<xsl:variable name="THERTEENTH_STEP" select="$VAR12 * $VAR_LOSSCREDIT" />
		<xsl:variable name="VAR13" select="round(format-number($THERTEENTH_STEP,'##.0000'))" />
		<xsl:variable name="FOURTEEN_STEP" select="$VAR13 * $VAR_WSTOVE" />
		<xsl:variable name="VAR14" select="round(format-number($FOURTEEN_STEP,'##.0000'))" />
...
(((((((((((((<xsl:call-template name="DEWLLING_MAIN_INDIANA"></xsl:call-template>)*
	...
	<!-- ============================================================================================ -->
	<!--								ADJUSTEDBASE FOR INDIANA (REGULAR) HO5						 
	1. Base (Find Base in Territory By Policy Form and Protection Class.) X
	2. Coverage A Factor (Note: Minimum Coverage A is $200,000.) X
	...
	5. Deductible Factor X
	6. Add HO34 Replacement Cost Personal Property Endorsement = 10% Minimum $35; Maximum $75
	7. Add HO11 Expanded Replacement Coverage for Building = $15 Flat Charge
	8. Add HO24 Premier V.I.P. End = $40
	9. Insurance Score Discount Factor X
	10. Age of Home Factor X
	11. Protective Device Factor X
	12. NonSmoker or Smoker Factor X
	13. HO14 Dwelling Under Construction Credit (if applies then no: Age of Home, or Protective Device Credit) X
	14. Experience (Maturity or Age of Insured) Factor X
	15. Multiple Policy (Auto & Home) Factor X
	16. Suburban Protection Discount X
	17. Loss Free Renewal Discount or Loss Surcharge X
	18. Surcharge  Woodstove
	19. Add Charge for Increased Coverage C or Coverage D (if any).
	20. Add Any Other Endorsement Option Charges.
	21. Apply Section II Credits if any ($9 if Secondary and we have the Primary; or $2 for Secondary and No Primary).
	22. Add Section II Liability Charges (if any).  -->
	<!-- ============================================================================================ -->
	<xsl:template name="ADJUSTEDBASE_REGULAR_INDIANA_HO5">
		...
		<xsl:variable name="VAR3" select="round(format-number($THIRD_STEP,'##.0000'))" />
		<xsl:variable name="FOURTH_STEP" select="$VAR3 * $VAR_DFACTOR" />
		<xsl:variable name="VAR6" select="round($FOURTH_STEP)" />
		...
		<xsl:variable name="VAR8" select="$EIGHT_STEP + $VAR_HO_20_PREFERRED_PLUS_OR_HO_21_VIP" />
		...
		<xsl:variable name="ELEVENTH_STEP" select="$VAR10 * $VAR_PROTECTIVE_DEVICE" />
		<xsl:variable name="VAR11" select="round(format-number($ELEVENTH_STEP,'##.0000'))" />
		<xsl:variable name="TWELTH_STEP" select="$VAR11 * $VAR_SMOKER_DISCOUNT" />
		<xsl:variable name="VAR12" select="round(format-number($TWELTH_STEP,'##.0000'))" />
		<xsl:variable name="THERTEENTH_STEP" select="$VAR12 * $VAR_DWELLING_UNDER_CONSTR" />
		<xsl:variable name="VAR13" select="round(format-number($THERTEENTH_STEP,'##.0000'))" />
		<xsl:variable name="FOURTEENTH_STEP" select="$VAR13 * $VAR_EXPERIENCE_DISCOUNT" />
		<xsl:variable name="VAR14" select="round(format-number($FOURTEENTH_STEP,'##.0000'))" />
		<xsl:variable name="FIFTEENTH_STEP" select="$VAR14 * $VAR_MULTIPOLICY_DISCOUNT" />
		<xsl:variable name="VAR15" select="round(format-number($FIFTEENTH_STEP,'##.0000'))" />
		<xsl:variable name="SIXTEENTH_STEP" select="$VAR15 * $VAR_SUB_PROTECTION_DISCOUNT" />
		<xsl:variable name="VAR16" select="round(format-number($SIXTEENTH_STEP,'##.0000'))" />
		<xsl:variable name="SEVENTEENTH_STEP" select="$VAR16 * $VAR_LOSSCREDIT" />
		<xsl:variable name="VAR17" select="round(format-number($SEVENTEENTH_STEP,'##.0000'))" />
		<xsl:variable name="EIGHTEENTH_STEP" select="$VAR17 * $VAR_WSTOVE" />
		<xsl:variable name="VAR18" select="round($EIGHTEENTH_STEP)" />
		<xsl:value-of select="(($VAR18 + $VAR_INCREASED_COV_C + $VAR_INCREASED_COV_D) - $VAR_SEASONALSECONDARYCREDIT) + $VAR_MEDICAL_CHARGES + $VAR_LIABLITY_CHARGES" />
	</xsl:template>
	<!-- ============================================================================================ -->
	...
	1. Base (Find Base in Territory By Policy Form and Protection Class.) X
	2. Coverage A Factor (Tables based on Protection Class and Construction Frame or Masonry.) X
	3. Repair Cost Factor 1.40
	4. Insurance Score Discount Factor X
	5. Go to Step 5 in HO2 or HO3 and follow to end.
	-->
	<!-- ============================================================================================ -->
	<xsl:template name="ADJUSTEDBASE_REPAIRCOST_INDIANA_HO2_HO3">
		...
		<xsl:variable name="SEVENTH_STEP" select="$VAR_2 * $VAR_PROTECTIVE_DEVICE" />
		<xsl:variable name="VAR7" select="round(format-number($SEVENTH_STEP,'##.0000'))" />
		<xsl:variable name="EIGHTTH_STEP" select="$VAR7 * $VAR_SMOKER" />
		<xsl:variable name="VAR8" select="round(format-number($EIGHTTH_STEP,'##.0000'))" />
		<xsl:variable name="NINTH_STEP" select="$VAR8 * $VAR_DWELLING_UNDER_CONSTR" />
		<xsl:variable name="VAR9" select="round(format-number($NINTH_STEP,'##.0000'))" />
		<xsl:variable name="TENTH_STEP" select="$VAR9 * $VAR_EXPERIENCE_DISCOUNT" />
		...
		<xsl:variable name="TWLVENTH_STEP" select="$VAR11 * $VAR_SUB_PROTECTION_DISCOUNT" />
		<xsl:variable name="VAR12" select="round(format-number($TWLVENTH_STEP,'##.0000'))" />
		<xsl:variable name="THRTEENTH_STEP" select="$VAR12 * $VAR_LOSSCREDIT" />
		<xsl:variable name="VAR13" select="round(format-number($THRTEENTH_STEP,'##.0000'))" />
		<xsl:variable name="FOURTEENTH_STEP" select="$VAR13 * $VAR_WSTOVE" />
		<xsl:variable name="VAR14" select="round(format-number($FOURTEENTH_STEP,'##.0000'))" />
		<xsl:value-of select="($VAR14 -$VAR_SEASONALSECONDARYCREDIT) + $VAR_MEDICAL_CHARGES + $VAR_LIABLITY_CHARGES" />
	</xsl:template>
	<!-- ============================================================================================ -->
	...
	<!-- 
	1. Base (Find Base in Territory By Policy Form 4 or 6, COI or COII, and Protection Class.) X
	2. Coverage C Factor (Tables based on whether is HO4 with minimum $10,000 or HO6 minimum $15,000.) X
	3. DELUXE Endorsement (if NO then X 1.00, if YES minimum Coverage C must be $25,000 and then take rate
	X 1.15 Min. $25.)
	4. Insurance Score Discount Factor X
	5. Term Factor: 1.00 Annual or 0.50 Semi-Annual X
	...
	10. Multiple Policy (Auto & Home) Factor (HO4 must be minimum $15,000 Coverage C to get credit. HO6 no
	minimum.) X
	11. Loss Free Renewal Discount or Loss Surcharge X
	12. Coverage A is 10% of Coverage C ($2,000 minimum), if Coverage A increased and there is no HO66
	endorsement then rate is $2 per $1,000 increase. If an HO66 Deluxe endorsement then charge is $2.20 per
	$1,000 increased above automatic amount.
	13. Surcharge  Woodstove.
	14. ApplySection II Credits if any ($9 if Secondary and we have the Primary; or $2 for Secondary and No Primary).
	15. AddAny Other Endorsement Option Charges.
	16. AddSection II Liability Charges (if any).
	 -->
	<!-- ============================================================================================ -->
	<xsl:template name="ADJUSTEDBASE_REGULAR_INDIANA_HO4_HO6">
		...
		<xsl:variable name="FIFTH_STEP_1" select="$VAR5 * $VAR_ACV_LOSS_SETL_WIND_HALL" />
		<xsl:variable name="VAR_5" select="round(format-number($FIFTH_STEP_1,'##.0000'))" />
		<xsl:variable name="SIX_STEP" select="$VAR_5 * $VAR_PROTECTIVE_DEVICE" />
		<xsl:variable name="VAR6" select="round($SIX_STEP)" />
		...
		<xsl:variable name="VAR9" select="round(format-number($NINTH_STEP,'##.0000'))" />
		<xsl:variable name="TENTH_STEP" select="$VAR9 * $VAR_LOSSCREDIT" />
		<xsl:variable name="VAR10" select="round(format-number($TENTH_STEP,'##.0000'))" />
		<xsl:variable name="ELEVENTH_STEP" select="$VAR10 * $VAR_WSTOVE" />
		<xsl:variable name="VAR11" select="round($ELEVENTH_STEP)" />
		<xsl:value-of select="($VAR11 - $VAR_SEASONALSECONDARYCREDIT) + $VAR_MEDICAL_CHARGES + $VAR_LIABLITY_CHARGES " />
	</xsl:template>
	<!-- ###################################  END - INDIANA  ######################################## -->
	<!-- ============================================================================================ -->
	...
	<!-- ============================================================================================ -->
	<!--============================================================================================ -->
	<!--									Home Discount Template 	(START)									  -->
	<!-- ============================================================================================ -->
	<xsl:template name="GET_ACV_LOSS_SETL_WIND_HALL">
		<xsl:variable name="VAR_ACV_LOSS_SETL_WIND_HALL">
			<xsl:variable name="VAR_ACV_LOSS_SETL_WIND_HALL_FACTORS">
				<xsl:call-template name="ACV_LOSS_SETL_WIND_HALL_FACTOR" />
			</xsl:variable>
			<xsl:variable name="VAR_MAXCREDIT_FACTOR" select="$HOCoveragesDoc/PRODUCTMASTER/PRODUCT/FACTOR[@ID='CREDITS']/@MAXCREDITFACTOR" />
			<xsl:variable name="LOSS_SETL_WIND_HALL_PERCENT" select="(1.00-$VAR_ACV_LOSS_SETL_WIND_HALL_FACTORS) * 100.00" />
			<xsl:variable name="VAR_MAXCREDIT_PERCENT" select="(1.00-$VAR_MAXCREDIT_FACTOR) * 100.00" />
			<xsl:choose>
				<xsl:when test="$LOSS_SETL_WIND_HALL_PERCENT &lt; $VAR_MAXCREDIT_PERCENT">
					<xsl:value-of select="$VAR_ACV_LOSS_SETL_WIND_HALL_FACTORS" />
				</xsl:when>
				<xsl:otherwise>
		...
		<xsl:value-of select="$VAR_ACV_LOSS_SETL_WIND_HALL" />
	</xsl:template>
	<xsl:template name="GET_AGE_OF_HOME_DISCOUNT">
		<xsl:choose>
			<xsl:when test="STATENAME=$STATE_INDIANA">
				<xsl:call-template name="ACV_LOSS_SETL_WIND_HALL_FACTOR" />
			</xsl:when>
			<xsl:otherwise>
				<xsl:variable name="VAR_ACV_LOSS_SETL_WIND_HALL_FACTOR">
					<xsl:call-template name="ACV_LOSS_SETL_WIND_HALL_FACTOR" />
				</xsl:variable>
				<xsl:variable name="VAR_MAXCREDIT_FACTOR" select="$HOCoveragesDoc/PRODUCTMASTER/PRODUCT/FACTOR[@ID='CREDITS']/@MAXCREDITFACTOR" />
				<xsl:variable name="VAR_MAXCREDIT_PERCENT" select="(1.00-$VAR_MAXCREDIT_FACTOR)*100" />
				<xsl:choose>
					<xsl:when test="$VAR_ACV_LOSS_SETL_WIND_HALL_FACTOR &gt; $VAR_MAXCREDIT_FACTOR">
						1.00	<!-- No need to calculate Age of Home. Device discount as credit limit has exceeded -->
					</xsl:when>
					<xsl:otherwise>
						...
						<xsl:variable name="FINALCREDIT_UPTO_ACVLOSS">
							<xsl:value-of select="(1- $VAR_ACV_LOSS_SETL_WIND_HALL_FACTOR)" />
						</xsl:variable>
						<xsl:choose>
							<xsl:when test="FINALCREDIT_UPTO_ACVLOSS  &lt;= $VAR_MAXCREDIT_FACTOR">
								<xsl:value-of select="$AGEOFHOMEFACTOR" />
							</xsl:when>
							<xsl:otherwise>
								<xsl:value-of select="(100 - ($VAR_MAXCREDIT_PERCENT - $FINALCREDIT_UPTO_ACVLOSS)) div 100.00" />
							</xsl:otherwise>
						</xsl:choose>
					</xsl:otherwise>
				</xsl:choose>
	...
	<xsl:template name="GET_PROTECTIVE_DEVICE_DISCOUNT">
		...
		<xsl:choose> <!-- Applicable only when building is not under construction-->
			<xsl:when test="STATENAME= $STATE_MICHIGAN and CONSTRUCTIONCREDIT = 'N'">
				<!--Actual Cash Value Loss setl wind hall -->
				<xsl:variable name="VAR_ACV_LOSS_SETL_WIND_HALL_FACTOR">
					<xsl:call-template name="ACV_LOSS_SETL_WIND_HALL_FACTOR" />
				</xsl:variable>
				<!--age of Home -->
				...
				<xsl:variable name="FINALCREDIT_AGEOFHOME"> <!-- Credit Percentage age of home -->
					<xsl:value-of select="((1.00 - $VAR_ACV_LOSS_SETL_WIND_HALL_FACTOR)*100)+((1.00 - $VAR_AOH_FACTOR)*100)" />
				</xsl:variable>
		...
		<xsl:choose>
			...
			<xsl:otherwise>
				...
				<xsl:variable name="FINALCREDIT_PROT_DEVICE_DISCOUNT">
					<xsl:value-of select="(1.00 - $VAR_ACV_LOSS_SETL_WIND_HALL_FACTOR) + (1.00 - $VAR1) + (1.00 - $VAR2)" />
				</xsl:variable>
		...
		<xsl:choose>
			...
			<xsl:otherwise>
				...
				<xsl:variable name="FINALCREDIT_DUC">
					<xsl:value-of select="(1.00 - $VAR_ACV_LOSS_SETL_WIND_HALL_FACTOR) + (1.00-$VAR1)+(1.00-$VAR2)+(1.00-$VAR3)" />
				</xsl:variable>
				<xsl:variable name="VAR_MAXCREDIT_FACTOR" select="$HOCoveragesDoc/PRODUCTMASTER/PRODUCT/FACTOR[@ID='CREDITS']/@MAXCREDITFACTOR" />
				<xsl:choose>
	...
	<xsl:template name="GET_MULTIPOLICY_DISCOUNT">
		<xsl:choose>
			...
			<xsl:otherwise>
				...
				<xsl:variable name="FINALCREDIT_EXPERIENCE">
					<xsl:value-of select="(1.00 - $VAR_ACV_LOSS_SETL_WIND_HALL_FACTOR) + (1.00-$VAR1)+(1.00-$VAR2)+(1.00-$VAR3)+(1.00-$VAR4)" />
				</xsl:variable>
				<xsl:variable name="VAR_MAXCREDIT_FACTOR" select="$HOCoveragesDoc/PRODUCTMASTER/PRODUCT/FACTOR[@ID='CREDITS']/@MAXCREDITFACTOR" />
				<xsl:choose>
					<xsl:when test="$FINALCREDIT_EXPERIENCE &gt; $VAR_MAXCREDIT_FACTOR">
	...
	<xsl:template name="GET_SUBURBANPROTECTION_DISCOUNT">
		<xsl:choose>
			<xsl:when test="STATENAME=$STATE_INDIANA">
				<xsl:call-template name="SUBURBANPROTECTIONDISCOUNT" /> <!-- No max discount check for Indiana-->
			</xsl:when>
			<xsl:otherwise>
				<!--Actual Cash Value Loss setl wind hall -->
				<xsl:variable name="VAR_ACV_LOSS_SETL_WIND_HALL_FACTOR">
				...
				<xsl:variable name="VAR6">
					<xsl:call-template name="GET_VALUEDCUSTOMER_DISCOUNT" />
				</xsl:variable>
				<xsl:variable name="FINALCREDIT_EXPERIENCE">
					<xsl:value-of select="(1.00-$VAR_ACV_LOSS_SETL_WIND_HALL_FACTOR)+(1.00-$VAR1)+(1.00-$VAR2)+(1.00-$VAR3)+(1.00-$VAR4)+(1.00-$VAR5)+(1.00-$VAR6)" />
				</xsl:variable>
				<xsl:variable name="VAR_MAXCREDIT_FACTOR" select="$HOCoveragesDoc/PRODUCTMASTER/PRODUCT/FACTOR[@ID='CREDITS']/@MAXCREDITFACTOR" />
				<xsl:choose>
					<xsl:when test="$FINALCREDIT_EXPERIENCE &gt; $VAR_MAXCREDIT_FACTOR">
					...
					<xsl:otherwise>
						<xsl:variable name="SUBURBANPROTECTION">
							<xsl:call-template name="SUBURBANPROTECTIONDISCOUNT" />
						</xsl:variable>
						<xsl:variable name="FINALCREDIT_SUBURBANPROTECTION">
							<xsl:value-of select="$FINALCREDIT_EXPERIENCE + (1 - $SUBURBANPROTECTION)" />
						</xsl:variable>
						<xsl:choose>
							<xsl:when test="$FINALCREDIT_SUBURBANPROTECTION &gt; $VAR_MAXCREDIT_FACTOR">
								<xsl:variable name="SUBURBANPROTECTIONFACTOR">
									<xsl:value-of select="($FINALCREDIT_SUBURBANPROTECTION - $VAR_MAXCREDIT_FACTOR) + $SUBURBANPROTECTION" />
								</xsl:variable>
								<xsl:value-of select="$SUBURBANPROTECTIONFACTOR" />
							</xsl:when>
							<xsl:when test="$FINALCREDIT_SUBURBANPROTECTION &lt;= $VAR_MAXCREDIT_FACTOR">
								<xsl:value-of select="$SUBURBANPROTECTION" />
							</xsl:when>
							<xsl:otherwise>1.00</xsl:otherwise>
						</xsl:choose>
					</xsl:otherwise>
	...
	<xsl:template name="GET_LOSS_FREE_DISCOUNT">
		<xsl:choose>
			<xsl:when test="STATENAME=$STATE_INDIANA">
				<xsl:call-template name="LOSSFREECREDIT" /> <!-- No max discount check for Indiana-->
			</xsl:when>
			<xsl:otherwise>
				<!--Actual Cash Value Loss setl wind hall -->
				<xsl:variable name="VAR_ACV_LOSS_SETL_WIND_HALL_FACTOR">
				...
				<xsl:variable name="FINALCREDIT_EXPERIENCE">
					<xsl:value-of select="(1.00-$VAR_ACV_LOSS_SETL_WIND_HALL_FACTOR)+(1.00-$VAR1)+(1.00-$VAR2)+(1.00-$VAR3)+(1.00-$VAR4)+(1.00-$VAR5)+(1.00-$VAR6)+(1.00-$VAR7)" />
				</xsl:variable>
				<xsl:variable name="VAR_MAXCREDIT_FACTOR" select="$HOCoveragesDoc/PRODUCTMASTER/PRODUCT/FACTOR[@ID='CREDITS']/@MAXCREDITFACTOR" />
				<xsl:choose>
					<xsl:when test="$FINALCREDIT_EXPERIENCE &gt; $VAR_MAXCREDIT_FACTOR">
					...
					<xsl:otherwise>
						<xsl:variable name="LOSSFREE">
							<xsl:call-template name="LOSSFREECREDIT" />
						</xsl:variable>
						<xsl:variable name="FINALCREDIT_LOSSFREE">
							<xsl:value-of select="$FINALCREDIT_EXPERIENCE + (1 - $LOSSFREE)" />
						</xsl:variable>
						<xsl:choose>
							<xsl:when test="$FINALCREDIT_LOSSFREE &gt; $VAR_MAXCREDIT_FACTOR">
								<xsl:variable name="LOSSFREEFACTOR">
									<xsl:value-of select="($FINALCREDIT_LOSSFREE - $VAR_MAXCREDIT_FACTOR) + $LOSSFREE" />
								</xsl:variable>
								<xsl:value-of select="$LOSSFREEFACTOR" />
							</xsl:when>
							<xsl:when test="$FINALCREDIT_LOSSFREE &lt;= $VAR_MAXCREDIT_FACTOR">
								<xsl:value-of select="$LOSSFREE" />
							</xsl:when>
							<xsl:otherwise>1.00</xsl:otherwise>
						</xsl:choose>
					</xsl:otherwise>
	...
	<!-- ============================================================================================ -->
	<!--									Home Discount Template 	(END)									  -->
	<!-- ============================================================================================ -->
	<xsl:template name="PREMIUM_GROUP">
		<xsl:variable name="TERCODES" select="TERRITORYCODES" />
		<xsl:variable name="GROUP_CYCLE" select="$HOCoveragesDoc/PRODUCTMASTER/PRODUCT/FACTOR[@ID='FORMGROUP']/NODE[@ID='GROUPCYCLE']/ATTRIBUTES[@TERRITORY=$TERCODES]/@CYCLE" />
		<xsl:variable name="MAXGROUPID" select="$HOCoveragesDoc/PRODUCTMASTER/PRODUCT/FACTOR[@ID='FORMGROUP']/NODE[@ID='GROUPCYCLE']/@MAXGROUP" />
		<xsl:variable name="F_CODE" select="FORM_CODE" />
		<xsl:variable name="FORMGROUP" select="$HOCoveragesDoc/PRODUCTMASTER/PRODUCT/FACTOR[@ID='FORMGROUP']/NODE[@ID='FORMGROUPING']/ATTRIBUTES[@FORMID=$F_CODE]/@GROUPID" />
		<xsl:value-of select="($MAXGROUPID * $GROUP_CYCLE) + $FORMGROUP" />
	</xsl:template>
	<xsl:template name="PREMIUM_GROUP_OLD">
		<!-- Set the Group -->
		<xsl:variable name="TERCODES" select="TERRITORYCODES" />
		<xsl:variable name="F_CODE" select="FORM_CODE" />
		<xsl:variable name="FORMGROUP" select="$HOCoveragesDoc/PRODUCTMASTER/PRODUCT/FACTOR[@ID='FORMGROUP']/NODE[@ID='FORMGROUPING']/ATTRIBUTES [@FORMID = $F_CODE]/@GROUPID" />
	...
	<xsl:template name="DISPLAY_INLAND_MARINE_HEADING">
		<xsl:choose>
			<xsl:when test="( SCH_BICYCLE_AMOUNT != '' and  SCH_BICYCLE_AMOUNT &gt; 0 ) 
						...
						or  (SCH_WHEELCHAIRS_AMOUNT  !='' and  SCH_WHEELCHAIRS_AMOUNT &gt; 0)
						or  (SCH_CAMERA_PROF_AMOUNT  !='' and  SCH_CAMERA_PROF_AMOUNT &gt; 0)
						or  (SCH_MUSICAL_REMUN_AMOUNT  !='' and  SCH_MUSICAL_REMUN_AMOUNT &gt; 0)">Y</xsl:when>
			<xsl:otherwise>N</xsl:otherwise>
		</xsl:choose>
	</xsl:template>
	<xsl:template name="COVERAGEADWELLING">
		<xsl:choose>
			...
			<xsl:otherwise>{<xsl:call-template name="CALL_ADJUSTEDBASE"></xsl:call-template>}</xsl:otherwise>
		</xsl:choose>
	</xsl:template>
	...
	<xsl:template name="MICHIGAN_PROPERTY_EXPENSE_RECOUPMENT">
		<xsl:variable name="POLICY_TERM" select="TERMFACTOR" />
		<xsl:variable name="VAR_PROPERTY_EXPENSE_FEE" select="$HOCoveragesDoc/PRODUCTMASTER/PRODUCT/FACTOR[@ID='RECOUPMENT']/NODE[@ID='HOME_PROPERTY_INS']/ATTRIBUTES[@TERM=$POLICY_TERM]/@FACTOR" />
		<xsl:value-of select="$VAR_PROPERTY_EXPENSE_FEE" />
	</xsl:template>
	<xsl:template name="COMPONENT_CODE">
		...
		<xsl:choose>
			...
			<xsl:when test="$FACTORELEMENT=normalize-space('CMRA_PROF')">
				<xsl:if test="STATENAME='MICHIGAN'">
					<xsl:text>10028</xsl:text>
				</xsl:if>
				<xsl:if test="STATENAME='INDIANA'">
					<xsl:text>10027</xsl:text>
				</xsl:if>
			</xsl:when>
			<xsl:when test="$FACTORELEMENT=normalize-space('MSC_PRF')">
				<xsl:if test="STATENAME='MICHIGAN'">
					<xsl:text>10030</xsl:text>
				</xsl:if>
				<xsl:if test="STATENAME='INDIANA'">
					<xsl:text>10029</xsl:text>
				</xsl:if>
			</xsl:when>
			<xsl:otherwise>0</xsl:otherwise>
		</xsl:choose>
