...
<![CDATA[ 
		...
		int intFinalPremium=0 ;
		public int CalculateFinalPremium(int tt)
		{
			intFinalPremium= intFinalPremium + tt;
			return intFinalPremium;
		}
	...
	<xsl:template match="/">
		<html>
			<head>
				<!-- <xsl:variable name="myName" select="PREMIUMXML/DWELLINGDETAILS/HEADER/@CSSNUM"></xsl:variable>  -->
			...
			<![CDATA[
				...
				{
					Mail='mailto:nsahay@ebix.com?subject=Rental Dwelling Quote';
	...
	<xsl:template match="PRIMIUM">
		...
		<!-- Display Dwelling -->
		<xsl:call-template name="PRIMIUM" />
		<!-- Total Premium -->
	...
	<xsl:template name="QDATE">
		<tr bgcolor="ffffff" valign="top">
			 ...
			 <xsl:value-of select="' '" />
			  ...
			  Agency :<xsl:value-of select="RISK/CLIENT_TOP_INFO/@AGENCY_NAME" />		
				...
				<br /><xsl:value-of select="RISK/CLIENT_TOP_INFO/@AGENCY_ADD1" />,
							...
							<xsl:otherwise>
								<br />
								<xsl:value-of select="RISK/CLIENT_TOP_INFO/@AGENCY_ADD1" />
							</xsl:otherwise>
						</xsl:choose>
				<xsl:choose>
							<xsl:when test="RISK/CLIENT_TOP_INFO/@AGENCY_ADD2!=''">
			...
			<xsl:value-of select="RISK/CLIENT_TOP_INFO/@PRIMARY_APP_LNAME" />	
			 ...
			 <br /><xsl:value-of select="RISK/CLIENT_TOP_INFO/@PRIMARY_APP_CITY" />, 
			 <xsl:value-of select="' '" /> 
			  <xsl:value-of select="RISK/CLIENT_TOP_INFO/@PRIMARY_APP_STATE" /> 
			 <xsl:value-of select="' '" />
			...
			<td colspan='2' class="boldtxt">		
				...
				<br /><xsl:value-of select="RISK/CLIENT_TOP_INFO/@AGENCY_PHONE" />
				<br /><xsl:value-of select="RISK/CLIENT_TOP_INFO/@AGENCY_ADD1" /> 
				<br /><xsl:value-of select="RISK/CLIENT_TOP_INFO/@AGENCY_ADD2" />
				<br /><xsl:value-of select="RISK/CLIENT_TOP_INFO/@AGENCY_CITY" /> 
				<BR /><xsl:value-of select="RISK/CLIENT_TOP_INFO/@AGENCY_STATE" /> 
				<xsl:value-of select="RISK/CLIENT_TOP_INFO/@AGENCY_ZIP" /> 				
			</td>
			...
			<br />Rates Effective - <xsl:value-of select="RISK/HEADER/@RATE_EFFECTIVE_DATE" />
				...
				<xsl:choose>
					<xsl:when test="RISK/CLIENT_TOP_INFO/@QQ_NUMBER != ''">
						<br /><xsl:value-of select="RISK/CLIENT_TOP_INFO/@QQ_NUMBER" /> 
					</xsl:when>
					<xsl:otherwise> 
						<xsl:choose>
							<xsl:when test="RISK/CLIENT_TOP_INFO/@APP_NUMBER != ''">
		...
		<tr bgcolor="ffffff">
			...
			<td colspan='2' style="FONT-SIZE: 8pt;vertical-align:TOP; COLOR: #000000; FONT-FAMILY: Verdana, 'MS Sans Serif', Arial; TEXT-ALIGN: LEFT">
				 <xsl:value-of select="RISK/HEADER/@RATE_EFFECTIVE_DATE" />  
				(<xsl:value-of select="RISK/HEADER/@BUSINESS_TYPE" />)
		 
			</td>--></tr>
		<tr class="headereffectCenter">
			<td colspan='4'>Rental Dwelling Quote ( <xsl:value-of select="RISK/STEP/@PRODUCTDESC" />)</td>
		</tr>
	</xsl:template>
	...
	<xsl:template name="SUBHEADINGS">
		...
		<tr class="midcolora">
			<td>
				<b>Dwelling: <xsl:value-of select="RISK/@ADDRESS" /></b>
			</td>
			<td></td>
			<td></td>
			<td></td>
	...
	<xsl:template name="TOTALPREMIUM">
		<tr class="midcolora">
			...
			<td></td>
			<td class="midcolorr">
				<b>$<xsl:value-of select="format-number(user:CalculatePremium(0),'###,###')" /></b>
			</td>
		</tr>
	</xsl:template>
	<!-- ********************************* TO DISPLAY PREMIUM ***************************************************************** -->
	<xsl:template name="PRIMIUM">
		<xsl:apply-templates select="RISK/STEP" />
	</xsl:template>
	<!-- ********************************* Call each step  ******************************************************** -->
	<xsl:template match="RISK/STEP">
		<xsl:choose>
	...
	<xsl:template name="ADD_NEW_ROW">
		<!-- <xsl:if test="contains(@STEPDESC,'-  Final Premium')"> -->
