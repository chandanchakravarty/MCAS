<%@ Page language="c#" Codebehind="DepositIndex.aspx.cs" AutoEventWireup="false" Inherits="Cms.Account.Aspx.DepositIndex" %>
...
<HTML>
	<HEAD>
		...
		//Called when user clicks on commit button on grid
		function OnCommit(num,msDg)
		{
			if (document.getElementById("hidlocQueryStr").value == "")
			{
				alert("Please select any Deposit from the grid, which you want to commit.");
				return;
			}
			...
			//Removing the seconf tab, which shows the details information
			RemoveTab(2,this.parent);
				
			document.getElementById("hidMode").value = "Commit";
			__doPostBack('','');
		}
		...
		function OnPreview(num,msDg)
		{
			if (document.getElementById("hidlocQueryStr").value == "")
			{
				alert("Please select any Deposit from the grid, which you want to preview.");
				return;
			}
		...
		function ShowDetails()
		{
			...
			if(parent.ClickRowNo == 0)
			{
				if (parent.JournalId != "")
				{
					var Deopsit_Id = window.parent.self.document.forms[0].hidSELECTED_DEP_ID.value ;
					var Cookie;
								...
								<tr>
									<td>
										<table class="tableWidthHeader" cellSpacing="0" cellPadding="0" border="0" align="center">
											<tr id="tabCtlRow">
												<td>
													<webcontrol:Tab ID="TabCtl" runat="server" TabURLs="AddDeposit.aspx?" TabTitles="Deposit Information"
