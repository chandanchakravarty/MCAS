...
{
	/// <summary>
	/// Summary description for AddResintatementFees.
	/// </summary>
	public class AddResintatementFees : Cms.Account.AccountBase
	{
		...
		private void SetValidators(System.Web.UI.WebControls.DataGridItemEventArgs e)
		{
			...
			csvValidate.ErrorMessage = ClsMessages.FetchGeneralMessage("1039");
		...
		private void Save()
		{
			...
			foreach(DataGridItem dgi in dgDepositDetails.Items)
			{
				if ( dgi.ItemType == ListItemType.Item || dgi.ItemType == ListItemType.AlternatingItem )
				{
					...
					objInfo.TransID =(Convert.ToInt32(enumTransType.IF));
...
//					}
//					if(hidPolicyVersion.Value != null && hidPolicyVersion.Value !="")
//					{		
...
//					}
			...
			if(alRecr.Count!=0)
			{
				returnResult=objNSFEntry.InsertReinstatementEntry(alRecr);
				if ( returnResult >0)
				{
					lblMessage.Text = ClsMessages.FetchGeneralMessage("1044");
				}
				else if(returnResult==-1)
				{
					lblMessage.Text = ClsMessages.FetchGeneralMessage("1045");
				}
				else
				{
			...
			{
				lblMessage.Visible =true;
				lblMessage.Text = ClsMessages.FetchGeneralMessage("1042");
