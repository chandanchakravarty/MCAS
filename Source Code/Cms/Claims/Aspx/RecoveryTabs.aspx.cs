...
namespace Cms.Claims.Aspx
{
	...
	/// 
	/// </summary>
	public class RecoveryTabs : Cms.Claims.ClaimBase
	{
		...
		private void GetQueryStringValues()
		{
			...
			if(Request["ACTION_ON_PAYMENT"]!=null && Request["ACTION_ON_PAYMENT"].ToString()!="")
				hidACTION_ON_RECOVERY.Value = Request["ACTION_ON_PAYMENT"].ToString();
			else
		...
		{
			string url,strLOBID="",strQueryString="";		
			if(GetLOBID()!=null && GetLOBID()!="")
				strLOBID = GetLOBID();

			strQueryString = "?CLAIM_ID=" + hidCLAIM_ID.Value + "&ACTIVITY_ID=" + hidACTIVITY_ID.Value + "&ACTION_ON_RECOVERY=" + hidACTION_ON_RECOVERY.Value;

			if(strLOBID==((int)enumLOB.BOAT).ToString())
				url = WATERCRAFT_RECOVERY_PAGE;
			else if(strLOBID==((int)enumLOB.REDW).ToString())
				url = RENTAL_RECOVERY_PAGE;
			else if(strLOBID==((int)enumLOB.HOME).ToString())
				url = HOME_RECOVERY_PAGE;
			else if(strLOBID==((int)enumLOB.CYCL).ToString())
				url = MOTOR_RECOVERY_PAGE;
			else if(strLOBID==((int)enumLOB.UMB).ToString())
				url = UMB_RECOVERY_PAGE;
			else
				url = "AddActivityRecovery.aspx";

			TabCtl.TabURLs = url + strQueryString;

			url = "RecoveryPayerIndex.aspx" + strQueryString + "&CALLED_FROM=" + CALLED_FROM_PAYMENT;
			TabCtl.TabURLs = TabCtl.TabURLs + "," + url;						
			TabCtl.TabTitles  = "Recovery Details,Payer Details";
		}
