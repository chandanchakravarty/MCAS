...
namespace Cms.Claims.Aspx
{
	...
	/// </summary>
	public class ReserveMotorDetails : Cms.Claims.ClaimBase
	{
		...
		private void Page_Load(object sender, System.EventArgs e)
		{
			...
			objResourceMgr = new System.Resources.ResourceManager("Cms.Claims.Aspx.ReserveMotorDetails"  ,System.Reflection.Assembly.GetExecutingAssembly());
			...
			try
			{
				...
				else
				{
					...
					if(dsOldData!=null && dsOldData.Tables.Count>POLICY_COVERAGE_TABLE)
					{
						...
						{
							//						if(dsOldData.Tables[POLICY_COVERAGE_TABLE].Rows[0]["CLM_RESERVE_CRACCTS"]!=null && dsOldData.Tables[POLICY_COVERAGE_TABLE].Rows[0]["CLM_RESERVE_CRACCTS"].ToString()!="")
							//							cmbCrAccts.SelectedValue = dsOldData.Tables[POLICY_COVERAGE_TABLE].Rows[0]["CLM_RESERVE_CRACCTS"].ToString();
							//						if(dsOldData.Tables[POLICY_COVERAGE_TABLE].Rows[0]["CLM_RESERVE_DRACCTS"]!=null && dsOldData.Tables[POLICY_COVERAGE_TABLE].Rows[0]["CLM_RESERVE_DRACCTS"].ToString()!="")
							//							cmbDrAccts.SelectedValue = dsOldData.Tables[POLICY_COVERAGE_TABLE].Rows[0]["CLM_RESERVE_DRACCTS"].ToString();
		...
		private void BindGrid(DataSet dsData)
		{
			...
			else
			{
				...
				if(dsData!=null && dsData.Tables.Count>VEHICLE_COVERAGE_TABLE && dsData.Tables[VEHICLE_COVERAGE_TABLE].Rows.Count>0)
				{
					...
					//ankit #itrack 7642
					int ActualRiskID = Convert.ToInt32(dsData.Tables[VEHICLE_COVERAGE_TABLE].Rows[0]["ACTUAL_RISK_ID"]);
					string ActualRiskType= dsData.Tables[VEHICLE_COVERAGE_TABLE].Rows[0]["ACTUAL_RISK_TYPE"].ToString(); 
					...
					for(i=0;i<dsData.Tables[VEHICLE_COVERAGE_TABLE].Rows.Count;i++)
					{
						...
						//ankit		#itrack 7642				
						objStRisk.rowId=i;
						objStRisk.riskId=Convert.ToString(dsData.Tables[VEHICLE_COVERAGE_TABLE].Rows[i]["ACTUAL_RISK_ID"]);
						objStRisk.riskType = dsData.Tables[VEHICLE_COVERAGE_TABLE].Rows[i]["ACTUAL_RISK_TYPE"].ToString();
						objStRisk.riskVehicleId=Convert.ToInt32(dsData.Tables[VEHICLE_COVERAGE_TABLE].Rows[i]["Vehicle_Id"]);
						//objStRisk.riskLimit = dsData.Tables[VEHICLE_COVERAGE_TABLE].Rows[i]["Limit"].ToString();
						...
						if(curVehicle !=  prevVehicle)
						{
							...
							dr["COV_DESC"]= VehicleText + dsData.Tables[VEHICLE_COVERAGE_TABLE].Rows[i]["VEHICLE"];
							//dr["COV_DESC"] = "SubHead";
							dr["LIMIT"] = SubHeading;
					...
					for(int j=0;j<arrLstRisk.Count;j++)
					{
						...
						curVehicle = objStRisks.riskVehicleId.ToString();
						if(curVehicle !=  prevVehicle)
						{							
							prevVehicle = curVehicle;					
							Insert_Collision_DataRows(dsData.Tables[VEHICLE_COVERAGE_TABLE],M15_IN_COV_ID,int.Parse(M_15_Collision_IN),int.Parse(M_15_Other_Than_Collision_IN), Convert.ToInt32(objStRisks.riskId), objStRisks.riskType,"Helmet & Riding Apparel Coverage (M-15) - Collision","Helmet & Riding Apparel Coverage (M-15) - Other Than Collision",objStRisks.riskVehicleId);
							Insert_Collision_DataRows(dsData.Tables[VEHICLE_COVERAGE_TABLE],M15_MI_COV_ID,int.Parse(M_15_Collision_MI),int.Parse(M_15_Other_Than_Collision_MI), Convert.ToInt32(objStRisks.riskId), objStRisks.riskType,"Helmet & Riding Apparel Coverage (M-15) - Collision","Helmet & Riding Apparel Coverage (M-15) - Other Than Collision",objStRisks.riskVehicleId);
							Insert_Collision_DataRows(dsData.Tables[VEHICLE_COVERAGE_TABLE],M14_IN_COV_ID,int.Parse(M_14_Collision_IN),int.Parse(M_14_Other_Than_Collision_IN), Convert.ToInt32(objStRisks.riskId), objStRisks.riskType,"Additional Physical Damage Coverage (M-14) - Collision","Additional Physical Damage Coverage (M-14) - Other Than Collision",objStRisks.riskVehicleId);
							Insert_Collision_DataRows(dsData.Tables[VEHICLE_COVERAGE_TABLE],M14_MI_COV_ID,int.Parse(M_14_Collision_MI),int.Parse(M_14_Other_Than_Collision_MI), Convert.ToInt32(objStRisks.riskId), objStRisks.riskType,"Additional Physical Damage Coverage (M-14) - Collision","Additional Physical Damage Coverage (M-14) - Other Than Collision",objStRisks.riskVehicleId);
						}						
					}

					hidVehicleRowCount.Value = dsData.Tables[VEHICLE_COVERAGE_TABLE].Rows.Count.ToString();
					dgCoverages.DataSource = dsData.Tables[VEHICLE_COVERAGE_TABLE];
					dgCoverages.DataBind();
				}
				else
				{
		...
		//Ankit #itrack 7642
		private int GetRowPositionWithRisk(DataTable dtTemp,string strCOV_ID,string risk_Id,string risk_Type,out string LimitValue)
		{
			int counter=0;
			//string LimitValue="";
        ...
        //Ankit #itrack 7642
		private void Insert_Collision_DataRows(DataTable dt,string COV_ID,int Collision_COV_ID,int OthCollision_COV_ID, int ActualRiskID , string ActualRiskType,
			string CollisionName,string OtherCollisionName,int VehicleId)
		{
			int position=0;			
			DataRow drow=null;
			string LimitValue="";
			...
			// string  LimitValue = "";
			position = GetRowPositionWithRisk(dt,COV_ID,ActualRiskID.ToString(),ActualRiskType,out LimitValue);
			if(position!=0)
			{
				...
				drow["COV_ID"]=OthCollision_COV_ID;
				drow["Vehicle_Id"] = VehicleId;
				drow["Limit"]=LimitValue;
		...
		private void OnItemDataBound(object sender,DataGridItemEventArgs e,string dataGridID)
		{
			if(e.Item.ItemType == ListItemType.AlternatingItem || e.Item.ItemType == ListItemType.Item)
			{
				...
				*/
				...
				//Added For Itrack Issue #6053 on 3 July 09.
		...
		private string CheckLOBAndRedirect()
		{
			...
			if(strLOB_ID==string.Empty)
				return "";
			else if(strLOB_ID==((int)enumLOB.AUTOP).ToString() || strLOB_ID==((int)enumLOB.CYCL).ToString())
		...
		private void AddNewReserveActivity(ClsActivityInfo objActivityInfo)
		{
//			ClsActivity  objActivity = new ClsActivity();
			//ClsActivityInfo objActivityInfo = new ClsActivityInfo();
			try
