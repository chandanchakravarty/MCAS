...
namespace Cms.Claims.Aspx
{
	...
	/// </summary>
	public class PaymentRentalDetails : Cms.Claims.ClaimBase
	{
		...
		protected System.Web.UI.WebControls.DataGrid dgSection1Payment;
		protected System.Web.UI.WebControls.DataGrid dgSection2Payment;		
...
//		protected System.Web.UI.WebControls.DropDownList cmbACTION_ON_PAYMENT;
		...
		public static string Section1PaymentDataGridID="";
		const int SECTION1_PAYMENT_TABLE=0;
		protected System.Web.UI.HtmlControls.HtmlTableRow trSection1Payment;
		protected System.Web.UI.HtmlControls.HtmlTableRow trSection2Payment;
		protected System.Web.UI.HtmlControls.HtmlTableRow trTotalPayments;
		protected System.Web.UI.HtmlControls.HtmlInputHidden hidPayeeID;
		const int SECTION2_PAYMENT_TABLE=1;
...
//		public DataTable gDtOldData=null;
		...
		private void Page_Load(object sender, System.EventArgs e)
		{
			...
			objResourceMgr = new System.Resources.ResourceManager("Cms.Claims.Aspx.PaymentRentalDetails"  ,System.Reflection.Assembly.GetExecutingAssembly());
			...
			if ( !Page.IsPostBack)
			{
				PaymentDataGridID = dgSection2Payment.ID;
				Section1PaymentDataGridID = dgSection1Payment.ID;
				SetCaptions();
				SetErrorMessages();
		...
		private void GetOldData()
		{
			...
			if(dsOldData==null ||  dsOldData.Tables.Count<2 || dsOldData.Tables[0].Rows.Count<1 && dsOldData.Tables[1].Rows.Count<1)
			{
				...
				if(dsOldData==null || dsOldData.Tables.Count<1)
				{
					...
					dgSection2Payment.Visible = false;
					dgSection1Payment.Visible = false;
					trSection1Payment.Attributes.Add("style","display:none");
					trTotalPayments.Attributes.Add("style","display:none");
					trSection2Payment.Attributes.Add("style","display:none");
...
//			capACTION_ON_PAYMENT.Text 	=		objResourceMgr.GetString("cmbACTION_ON_PAYMENT");
		...
		{
			if(dsData==null || dsData.Tables.Count<SECTION2_PAYMENT_TABLE || (dsData.Tables[SECTION1_PAYMENT_TABLE].Rows.Count<0 && dsData.Tables[SECTION2_PAYMENT_TABLE].Rows.Count<0))
			{
				lblMessage.Text = ClsMessages.FetchGeneralMessage("792");
				lblMessage.Visible = true;
				dgSection2Payment.Visible = false;
			...
			if(dsData.Tables[SECTION1_PAYMENT_TABLE].Rows.Count>0)
			{
				...
				for(i=0;i<dsData.Tables[SECTION1_PAYMENT_TABLE].Rows.Count;i++)
				{
					curVehicle = dsData.Tables[SECTION1_PAYMENT_TABLE].Rows[i]["DWELLING_ID"].ToString();
					if(curVehicle !=  prevVehicle)
					{
						prevVehicle = curVehicle;
						DataRow dr = dsData.Tables[SECTION1_PAYMENT_TABLE].NewRow();
						dr["COV_DESC"]= VehicleText + dsData.Tables[SECTION1_PAYMENT_TABLE].Rows[i]["DWELLING"];
						//dr["COV_DESC"] = "SubHead";
						dr["LIMIT"] = SubHeading;
						dsData.Tables[SECTION1_PAYMENT_TABLE].Rows.InsertAt(dr,i);						
					}
				}
				hidSection1RowCount.Value = dsData.Tables[SECTION1_PAYMENT_TABLE].Rows.Count.ToString();			
				dgSection1Payment.DataSource = dsData.Tables[SECTION1_PAYMENT_TABLE];
				dgSection1Payment.DataBind();	
			}
			else
			{
				dgSection1Payment.Visible = false;
			...
			if(dsData!=null && hidCustomerID.Value!="" && hidCustomerID.Value!="0" && hidPolicyID.Value!="" && hidPolicyID.Value!="0" && hidPolicyVersionID.Value!="" && hidPolicyVersionID.Value!="0" && dsData.Tables.Count>SECTION2_PAYMENT_TABLE && dsData.Tables[SECTION2_PAYMENT_TABLE].Rows.Count>0)
			{
				//Code for adding new subheading comes here
				string curVehicle,prevVehicle;
				TableRow row = new TableRow();
				int i=0;
				curVehicle = prevVehicle = "";
				for(i=0;i<dsData.Tables[SECTION2_PAYMENT_TABLE].Rows.Count;i++)
				{
					curVehicle = dsData.Tables[SECTION2_PAYMENT_TABLE].Rows[i]["DWELLING_ID"].ToString();
					if(curVehicle !=  prevVehicle)
					{
						prevVehicle = curVehicle;
						DataRow dr = dsData.Tables[SECTION2_PAYMENT_TABLE].NewRow();
						dr["COV_DESC"]= VehicleText + dsData.Tables[SECTION2_PAYMENT_TABLE].Rows[i]["DWELLING"];
						//dr["COV_DESC"] = "SubHead";
						dr["LIMIT"] = SubHeading;
						dsData.Tables[SECTION2_PAYMENT_TABLE].Rows.InsertAt(dr,i);						
					}
				}
				hidSection2RowCount.Value = dsData.Tables[SECTION2_PAYMENT_TABLE].Rows.Count.ToString();
				dgSection2Payment.DataSource = dsData.Tables[SECTION2_PAYMENT_TABLE];
				dgSection2Payment.DataBind();	
			}
			else
			{
				dgSection2Payment.Visible = false;
			...
			if(dgSection2Payment.Visible==false && dgSection1Payment.Visible==false)
			{
				trTotalPayments.Attributes.Add("style","display:none");
				trBody.Attributes.Add("style","display:none");
				btnSave.Visible = false;
		...
		private void dgSection2Payment_ItemDataBound(object sender, System.Web.UI.WebControls.DataGridItemEventArgs e)
		{
			OnItemDataBound(sender,e,PaymentDataGridID.ToUpper());
		}

		private void dgSection1Payment_ItemDataBound(object sender, System.Web.UI.WebControls.DataGridItemEventArgs e)
		{
			OnItemDataBound(sender,e,Section1PaymentDataGridID.ToUpper());
		}
		...
		{   
			this.dgSection1Payment.ItemDataBound += new System.Web.UI.WebControls.DataGridItemEventHandler(this.dgSection1Payment_ItemDataBound);
			this.dgSection2Payment.ItemDataBound += new System.Web.UI.WebControls.DataGridItemEventHandler(this.dgSection2Payment_ItemDataBound);
			...
			foreach(DataGridItem dgi in dtGrid.Items)
			{
				if ( dgi.ItemType == ListItemType.Item || dgi.ItemType == ListItemType.AlternatingItem )
				{
					...
					if(lblDWELLING_ID!=null && lblDWELLING_ID.Text.Trim()!="")
						objPaymentInfo.VEHICLE_ID = int.Parse(lblDWELLING_ID.Text.Trim());
		...
		private void btnSave_Click(object sender, System.EventArgs e)
		{
			try
			{
				...
				PopulateArray(PaymentArrayList,dgSection1Payment);
				PopulateArray(PaymentArrayList,dgSection2Payment);
