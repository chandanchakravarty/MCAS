/******************************************************************************************
	...
	<End Date				: - >
	<Description			: - >This file is being used for loading grid control to show watercraft company records
	<Review Date			: - >
	<Reviewed By			: - >
...
namespace Cms.Claims.Aspx
{
	...
	/// </summary>
	public class WatercraftCompanyIndex : Cms.Claims.ClaimBase
	{
		...
		private void Page_Load(object sender, System.EventArgs e)
		{
			...
			try
			{
				...
				((BaseDataGrid)c1).SelectClause ="CLAIM_ID,COMPANY_ID,AGENCY_ID,NAIC_CODE,REFERENCE_NUMBER,CAT_NUMBER, convert(char,EFFECTIVE_DATE,101) EFFECTIVE_DATE, convert(char,EXPIRATION_DATE,101) EXPIRATION_DATE,CCC.IS_ACTIVE";
				//specifying tables for from clause
				((BaseDataGrid)c1).FromClause=" CLM_CLAIM_COMPANY CCC";
				...
				((BaseDataGrid)c1).SearchColumnHeadings="NAIC Code^Reference #^CAT #^Effective Date^Expiration Date";
				//specifying column to be used for combo box				
				((BaseDataGrid)c1).SearchColumnNames="NAIC_CODE^REFERENCE_NUMBER^CAT_NUMBER^EFFECTIVE_DATE^EXPIRATION_DATE";
				...
				((BaseDataGrid)c1).OrderByClause="COMPANY_ID asc";
				//specifying column numbers of the query to be displyed in grid
				((BaseDataGrid)c1).DisplayColumnNumbers="1";
				//specifying column names from the query				
				((BaseDataGrid)c1).DisplayColumnNames="NAIC_CODE^REFERENCE_NUMBER^CAT_NUMBER^EFFECTIVE_DATE^EXPIRATION_DATE";
				//specifying text to be shown as column headings				
				((BaseDataGrid)c1).DisplayColumnHeadings="NAIC Code^Reference #^CAT #^Effective Date^Expiration Date";
				//specifying column heading display text length
				((BaseDataGrid)c1).DisplayTextLength="23^23^24^15^15";
				//specifying width percentage for columns
				((BaseDataGrid)c1).DisplayColumnPercent="23^23^24^15^15";
				...
				//specifying heading
				((BaseDataGrid)c1).HeaderString = "Company";
			...
			TabCtl.TabTitles = "Company";
			TabCtl.TabURLs="AddWatercraftCompany.aspx?CLAIM_ID=" + hidCLAIM_ID.Value + "&AGENCY_ID=" + hidAGENCY_ID.Value + "&";
