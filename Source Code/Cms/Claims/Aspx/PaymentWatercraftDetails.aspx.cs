...
namespace Cms.Claims.Aspx
{
	...
	/// </summary>
	public class PaymentWatercraftDetails : Cms.Claims.ClaimBase
	{
...
//		public DataTable gDtOldData=null;
		...
		private void Page_Load(object sender, System.EventArgs e)
		{
			...
			objResourceMgr = new System.Resources.ResourceManager("Cms.Claims.Aspx.PaymentWatercraftDetails"  ,System.Reflection.Assembly.GetExecutingAssembly());
...
//					dsOldData.Tables[i].Columns.Add(dtCol);
		...
		private void LoadData(DataTable dtOldData)
		{
			if(dtOldData!=null && dtOldData.Rows.Count>0)
			{
				...
				if(dtOldData.Rows[0]["DrAccts"]!=null && dtOldData.Rows[0]["DrAccts"].ToString()!="" && dtOldData.Rows[0]["DrAccts"].ToString()!="0")
					cmbDrAccts.SelectedValue = dtOldData.Rows[0]["DrAccts"].ToString();

				if(dtOldData.Rows[0]["CrAccts"]!=null && dtOldData.Rows[0]["CrAccts"].ToString()!="" && dtOldData.Rows[0]["CrAccts"].ToString()!="0")
			...
			if(dsData!=null && hidCustomerID.Value!="" && hidCustomerID.Value!="0" && hidPolicyID.Value!="" && hidPolicyID.Value!="0" && hidPolicyVersionID.Value!="" && hidPolicyVersionID.Value!="0" && dsData.Tables.Count>VEHICLE_PAYMENT_TABLE && dsData.Tables[VEHICLE_PAYMENT_TABLE].Rows.Count>0)
			{
				...
				while(position < RowCount)
				{
					...
					if(dsData.Tables[VEHICLE_PAYMENT_TABLE].Rows[position]["ACTUAL_RISK_TYPE"].ToString() == "TR")
					{
						...
						if(dsData.Tables[VEHICLE_PAYMENT_TABLE].Rows[position]["PAYMENT_ID"] != null && dsData.Tables[VEHICLE_PAYMENT_TABLE].Rows[position]["PAYMENT_ID"] != DBNull.Value)
						{
							TrailerRow["PAYMENT_ID"] = dsData.Tables[VEHICLE_PAYMENT_TABLE].Rows[position]["PAYMENT_ID"];
						}
			...
			foreach(DataGridItem dgi in dtGrid.Items)
			{
				if ( dgi.ItemType == ListItemType.Item || dgi.ItemType == ListItemType.AlternatingItem )
				{
					...
					if(lblBOAT_ID!=null && lblBOAT_ID.Text.Trim()!="")
						objPaymentInfo.VEHICLE_ID = int.Parse(lblBOAT_ID.Text.Trim());
