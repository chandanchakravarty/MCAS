...
namespace Cms.Claims.Aspx
{
	...
	/// </summary>
	public class WatercraftPropertyDamagedIndex : Cms.Claims.ClaimBase
	{
		...
		private void Page_Load(object sender, System.EventArgs e)
		{
			...
			try
			{
				...
				((BaseDataGrid)c1).SelectClause ="DESCRIPTION,CLAIM_ID,PROPERTY_DAMAGED_ID,OTHER_INSURANCE_NAME,OTHER_OWNER_NAME,ISNULL(ADDRESS1,'') + ' ' + ISNULL(ADDRESS2,'') AS ADDRESS,CITY,ZIP,MCSL.STATE_NAME AS STATENAME,CWPD.IS_ACTIVE";
				//specifying tables for from clause
				((BaseDataGrid)c1).FromClause=" CLM_WATERCRAFT_PROPERTY_DAMAGED CWPD LEFT OUTER JOIN MNT_COUNTRY_STATE_LIST MCSL ON CWPD.STATE=MCSL.STATE_ID";
				...
				((BaseDataGrid)c1).SearchColumnNames ="OTHER_INSURANCE_NAME^OTHER_OWNER_NAME^DESCRIPTION^CITY^MCSL.STATE_NAME^ZIP";
				//specifying column to be used for combo box				
				((BaseDataGrid)c1).SearchColumnHeadings="Other Insurance Company/Agency Name^Other Vehicle/Property Owner Name^Description^City^State^Zip";
				...
				((BaseDataGrid)c1).DisplayColumnNames="OTHER_INSURANCE_NAME^OTHER_OWNER_NAME^DESCRIPTION^CITY^STATENAME^ZIP";
				//specifying text to be shown as column headings				
				((BaseDataGrid)c1).DisplayColumnHeadings="Other Insurance Company/Agency Name^Other Vehicle/Property Owner Name^Description^City^State^Zip";
				...
				((BaseDataGrid)c1).DisplayColumnPercent="20^20^35^10^10^5";
				...
				//specifying heading
				((BaseDataGrid)c1).HeaderString = "Property Damaged";
				...
				TabCtl.TabURLs="AddWatercraftPropertyDamaged.aspx?CLAIM_ID=" + hidCLAIM_ID.Value + "&"; 
			}
			catch(Exception ex)
			{
