...
{
	/// <summary>
	/// Summary description for VoidClaimsCheck.
	/// </summary>
	public class VoidClaimsCheck  : Cms.Claims.ClaimBase
	{
...
//			base.ClearClaimsSessionValues();
			//Set Security XML
            objResourceMgr = new ResourceManager("Claims.Aspx.VoidClaimsCheck", Assembly.GetExecutingAssembly());
			strSecurity = gstrSecurityXML;
	
			if (Request.Form["__EVENTTARGET"] == "Void" && hidCheckedRowIDs.Value!="")
			{   
				hidCheckedRowIDs.Value = hidCheckedRowIDs.Value.Replace("~",",");
				hidCheckedRowIDs.Value = hidCheckedRowIDs.Value.Replace(" ","");
				ClsClaims.ClaimsCheckOperation(hidCheckedRowIDs.Value,"VOID");
			...
			try
			{
				...
				objWebGrid.WhereClause   = " CHECK_TYPE=9937  AND ISNULL(GL_UPDATE,1) =1 AND ISNULL(IS_BNK_RECONCILED,'N') <> 'Y'";

                objWebGrid.HeaderString = objResourceMgr.GetString("HeaderString"); //"Void Claims Checks" ;

				//objWebGrid.SearchColumnHeadings = "Check Date^Check Number^Manual Check^Check Amount^Payment Mode";
				...
				//objWebGrid.SearchColumnNames = "CHECK_DATE^CHECK_NUMBER^CASE WHEN ISNULL(MANUAL_CHECK,'N') = 'Y' THEN 'Yes' ELSE 'No' END^CHECK_AMOUNT^LOOKUP_VALUE_DESC";		
				...
				//objWebGrid.DisplayColumnHeadings ="Check Date^Check Number^Manual Check^Check Amount^Payment Mode^Payee Details";
				...
				//objWebGrid.DisplayColumnNames = "CHECK_DATE^CHECK_NUMBER^MANUAL_CHECK^CHECK_AMOUNT^LOOKUP_VALUE_DESC^PAYEE_FULL_ADD";
				...
				//objWebGrid.DisplayTextLength = "16^16^16^16^16^40";
				objWebGrid.DisplayTextLength = "16^16^10^10^16^16^50";
				//objWebGrid.DisplayColumnPercent = "16^16^16^16^16^40";
                ...
                if (ClsCommon.CheckSecurity(strSecurity))
                    objWebGrid.ExtraButtons = objResourceMgr.GetString("ExtraButtons"); //"1^Void Check^0^VoidClaimCheck";											
