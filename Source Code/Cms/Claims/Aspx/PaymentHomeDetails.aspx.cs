...
namespace Cms.Claims.Aspx
{
	...
	/// </summary>
	public class PaymentHomeDetails : Cms.Claims.ClaimBase
	{
...
//		protected System.Web.UI.WebControls.DropDownList cmbACTION_ON_PAYMENT;
		...
		protected System.Web.UI.HtmlControls.HtmlInputHidden hidScheduledPmntID;
...
//		public DataTable gDtOldData=null;
		...
		private void Page_Load(object sender, System.EventArgs e)
		{
			...
			objResourceMgr = new System.Resources.ResourceManager("Cms.Claims.Aspx.PaymentHomeDetails"  ,System.Reflection.Assembly.GetExecutingAssembly());			
...
//		}
		...
		private void GetOldData()
		{
			try
			{
				DataSet dsOldData = ClsPayment.GetOldDataForHome(hidCLAIM_ID.Value,hidACTIVITY_ID.Value);
...
//			capACTION_ON_PAYMENT.Text 	=		objResourceMgr.GetString("cmbACTION_ON_PAYMENT");
		...
		{
			if(dsData==null || dsData.Tables.Count<SCHEDULED_ITEMS_COVERAGE_TABLE || (dsData.Tables[SECTION1_COVERAGE_TABLE].Rows.Count<1 && dsData.Tables[SECTION2_COVERAGE_TABLE].Rows.Count<1 && dsData.Tables[SCHEDULED_ITEMS_COVERAGE_TABLE].Rows.Count<1 && dsData.Tables[WATERCRAFT_COVERAGE_TABLE].Rows.Count<1))
			{
...
//						dsData.Tables[i].Columns.Add(dtCol);
			...
			if(dsData!=null)
			{
				...
				if(dsData.Tables.Count>SECTION1_COVERAGE_TABLE && dsData.Tables[SECTION1_COVERAGE_TABLE].Rows.Count>0)
				{
					...
					hidPolicyRowCount.Value = dsData.Tables[SECTION1_COVERAGE_TABLE].Rows.Count.ToString();
				}
				else
				{
				...
				if(hidCustomerID.Value!="" && hidCustomerID.Value!="0" && hidPolicyID.Value!="" && hidPolicyID.Value!="0" && hidPolicyVersionID.Value!="" && hidPolicyVersionID.Value!="0" && dsData.Tables.Count>SECTION2_COVERAGE_TABLE && dsData.Tables[SECTION2_COVERAGE_TABLE].Rows.Count>0)
				{
					...
					hidVehicleRowCount.Value = dsData.Tables[SECTION2_COVERAGE_TABLE].Rows.Count.ToString();
				}
				else
				{
...
//					hidWatercraftRowCount.Value = dgWatercraftCoveragesGrid.Items.Count.ToString();
					...
					while(position < RowCount)
					{
						...
						if(dsData.Tables[WATERCRAFT_COVERAGE_TABLE].Rows[position]["ACTUAL_RISK_TYPE"].ToString() == "TR")
						{
							...
							if(dsData.Tables[WATERCRAFT_COVERAGE_TABLE].Rows[position]["PAYMENT_ID"] != null && dsData.Tables[WATERCRAFT_COVERAGE_TABLE].Rows[position]["PAYMENT_ID"] != DBNull.Value)
							{
								TrailerRow["PAYMENT_ID"] = dsData.Tables[WATERCRAFT_COVERAGE_TABLE].Rows[position]["PAYMENT_ID"];
							}
...
//					curVehicle = prevVehicle = "";
//					int cntTrailer=0;
//					for(i=0;i<dsData.Tables[WATERCRAFT_COVERAGE_TABLE].Rows.Count;i++)
//					{
//						curVehicle = dsData.Tables[WATERCRAFT_COVERAGE_TABLE].Rows[i]["DWELLING_ID"].ToString();
//						if(curVehicle !=  prevVehicle)
//						{
//							cntTrailer = 0;
//							prevVehicle = curVehicle;
//							DataRow dr = dsData.Tables[WATERCRAFT_COVERAGE_TABLE].NewRow();
//							dr["COV_DESC"]= BoatText + dsData.Tables[WATERCRAFT_COVERAGE_TABLE].Rows[i]["DWELLING"];
//							//dr["COV_DESC"] = "SubHead";
//							dr["LIMIT"] = SubHeading;
//							dsData.Tables[WATERCRAFT_COVERAGE_TABLE].Rows.InsertAt(dr,i);						
//						}
//						if(dsData.Tables[WATERCRAFT_COVERAGE_TABLE].Rows[i]["COV_ID"].ToString() !="")
//						{
//							SetTrailerCoverageID();
//							if(Convert.ToInt32(dsData.Tables[WATERCRAFT_COVERAGE_TABLE].Rows[i]["COV_ID"])>=TrailerCovID)
//							{
...
//								{
//									dsData.Tables[WATERCRAFT_COVERAGE_TABLE].Rows[i]["LIMIT"] = dsTrailer.Tables[0].Rows[cntTrailer]["INSURED_VALUE"];
//									dsData.Tables[WATERCRAFT_COVERAGE_TABLE].Rows[i]["DEDUCTIBLE"] = dsTrailer.Tables[0].Rows[cntTrailer]["TRAILER_DED"];
//								}
//								DataRow dr		= dsData.Tables[WATERCRAFT_COVERAGE_TABLE].NewRow();
...
//					}
		...
		private void PopulateArray(ArrayList aPaymentList,DataGrid dtGrid)
		{
			foreach(DataGridItem dgi in dtGrid.Items)
			{
				...
				{
					if(!(dtGrid.ID.ToUpper().Equals(ScheduledItemsCoverageGridID.ToUpper())))//Done for Itrack Issue 7663 on 19 Aug 2010
					{
		...
		private void PopulateScheduleArray(ArrayList aList)
		{
			...
			string [] SchedulePmntArray = hidScheduledPmntID.Value.Split('^');
			//Added for Itrack Issue 7663 on 19 Aug 2010
			...
			for(int iCounter=0;iCounter<ScheduleCovArray.Length;iCounter++)
			{
				...
				if (Request.Form[strTextBoxID.ToString()]!= null && Request.Form[strTextBoxID.ToString()].ToString()!="")				
					objPaymentInfo.PAYMENT_AMOUNT = Convert.ToDouble(Request.Form[strTextBoxID.ToString()].ToString());
				...
				objPaymentInfo.RESERVE_ID = int.Parse(ScheduleRsvArray[iCounter].ToString());
				if(SchedulePmntArray!=null && SchedulePmntArray[iCounter]!=null && SchedulePmntArray[iCounter]!="")
					objPaymentInfo.PAYMENT_ID = int.Parse(SchedulePmntArray[iCounter].ToString());
				...
				objPaymentInfo.ACTIVITY_ID = int.Parse(hidACTIVITY_ID.Value);	
				//Added for Itrack Issue 7663 on 19 Aug 2010
				objPaymentInfo.VEHICLE_ID = int.Parse(ScheduleDwelling_IdArray[iCounter]);
				objPaymentInfo.ACTUAL_RISK_ID = int.Parse(ScheduleActual_Risk_IdArray[iCounter]);
				objPaymentInfo.ACTUAL_RISK_TYPE = hidSch_ACTUAL_RISK_TYPE.Value;
		...
		private void PopulateWaterEquipArray(ArrayList aList)
		{
			...
			string [] SchedulePmntArray = hidWaterEquipPmntID.Value.Split('^');
			//Added for Itrack Issue 7663 on 19 Aug 2010
			...
			for(int iCounter=0;iCounter<ScheduleCovArray.Length;iCounter++)
			{
				...
				objPaymentInfo.ACTIVITY_ID = int.Parse(hidACTIVITY_ID.Value);
				//Added for Itrack Issue 7663 on 19 Aug 2010
				objPaymentInfo.VEHICLE_ID = int.Parse(EquipDwelling_IdArray[iCounter]);
				objPaymentInfo.ACTUAL_RISK_ID = int.Parse(EquipActual_Risk_IdArray[iCounter]);
				objPaymentInfo.ACTUAL_RISK_TYPE = hidEquip_ACTUAL_RISK_TYPE.Value;
...
//						objTableRow.Cells.Add(objTableCell);
						...
						//objTableCell.Text=String.Format("{0:,#,###}",Convert.ToInt64(dRow["LIMIT_DEDUC_AMOUNT"]));											
						objTableRow.Cells.Add(objTableCell);

						objTableCell = new TableCell();
						objTableCell.Attributes.Add("class","midcolora");
						strLabelOutstandingID.Length=0;
						strLabelOutstandingID.Append("lblOUTSTANDING_AMOUNT_" + i.ToString());
...
//							objTableCell.Text= Double.Parse(dRow["OUTSTANDING"].ToString()).ToString("N");
						...
						objTableCell = new TableCell();
						objTableCell.Attributes.Add("class","midcolora");		
						string strRI_RESERVE = objTableCell.ID = "tdRI_RESERVE_" + i.ToString();//Done for Itrack Issue 6635 on 29 Oct 09						
						...
						lblItemId.Text = dRow["ITEM_ID"].ToString();
						lblItemId.ID = "lbl_ " + dRow["ITEM_ID"].ToString();
						...
						hidScheduledPmntID.Value+="^" + dRow["PAYMENT_ID"].ToString();
...
//					objTableRow.Cells.Add(objTableCell);
				...
				if(hidScheduledPmntID.Value!="0" && hidScheduledPmntID.Value!="" && hidScheduledPmntID.Value.Length>0)
					hidScheduledPmntID.Value = hidScheduledPmntID.Value.Substring(1,hidScheduledPmntID.Value.Length-1);

				//Added for Itrack Issue 7663 on 19 Aug 2010
//				string [] ScheduleActual_Risk_IdArray = hidSch_ACTUAL_RISK_ID.Value.Split('^');
//				for(i=0;i<ScheduleActual_Risk_IdArray.Length;i++)
//				{
//					hidSch_ACTUAL_RISK_ID.Value +="!";
//				}
			...
			if(dtTemp!=null && dtTemp.Rows.Count>0)
			{
				...
				foreach(DataRow dRow in dtTemp.Rows)
				{
					if (dRow["EQUIP_ID"].ToString() != strPrevCarID)
					{
						...
						hidWaterEquipPmntID.Value+="^" + dRow["PAYMENT_ID"].ToString();
