/******************************************************************************************
	...
	<End Date				: - >June 06, 2006
	<Description			: - >This file is being used for loading grid control to show payment breakdown
	<Review Date			: - >
	<Reviewed By			: - >
...
namespace Cms.Claims.Aspx
{
	...
	/// </summary>
	public class PaymentBreakDownIndex : Cms.Claims.ClaimBase
	{
		...
		private void Page_Load(object sender, System.EventArgs e)
		{
			...
			try
			{
				...
				((BaseDataGrid)c1).SelectClause =" CLAIM_ID,ACTIVITY_ID,PAYMENT_BREAKDOWN_ID,COVERAGE_ID,CTD.DETAIL_TYPE_DESCRIPTION AS TRANSACTION_DESC, substring(convert(varchar(30),convert(money,PAID_AMOUNT),1),0,charindex('.',convert(varchar(30),convert(money,PAID_AMOUNT),1),0)) PAID_AMOUNT ";
				//specifying tables for from clause
				((BaseDataGrid)c1).FromClause=" CLM_ACTIVITY_PAYMENT_BREAKDOWN CAPB JOIN CLM_TYPE_DETAIL CTD ON CTD.DETAIL_TYPE_ID = CAPB.TRANSACTION_CODE AND CTD.TRANSACTION_CODE = " + ((int)enumTransactionLookup.CLAIM_PAYMENT).ToString();
				...
				((BaseDataGrid)c1).SearchColumnNames ="CTD.DETAIL_TYPE_DESCRIPTION^PAID_AMOUNT";
				//specifying column to be used for combo box				
				((BaseDataGrid)c1).SearchColumnHeadings="Transaction Code^Paid";
				...
				((BaseDataGrid)c1).OrderByClause="PAYMENT_BREAKDOWN_ID asc";
				...
				((BaseDataGrid)c1).DisplayColumnNames="TRANSACTION_DESC^PAID_AMOUNT";
				//specifying text to be shown as column headings				
				((BaseDataGrid)c1).DisplayColumnHeadings="Transaction Code^Paid";
				//specifying column heading display text length
				((BaseDataGrid)c1).DisplayTextLength="25^25";
				//specifying width percentage for columns
				((BaseDataGrid)c1).DisplayColumnPercent="25^25";
				...
				//specifying heading
				((BaseDataGrid)c1).HeaderString = "Claims Payment Breakdown";
				...
				((BaseDataGrid)c1).QueryStringColumns ="PAYMENT_BREAKDOWN_ID";
				((BaseDataGrid)c1).DefaultSearch="Y";     
                
				// to show completed task we have to give check box
				GridHolder.Controls.Add(c1);	
			
				TabCtl.TabURLs="PaymentBreakDownDetails.aspx?CLAIM_ID=" + hidCLAIM_ID.Value + "&ACTIVITY_ID=" + hidACTIVITY_ID.Value + "&"; 
			}
			catch(Exception ex)
			{
