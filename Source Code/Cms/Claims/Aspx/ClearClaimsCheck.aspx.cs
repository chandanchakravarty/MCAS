...
{
	/// <summary>
	/// Summary description for ClearClaimsCheck.
	/// </summary>
	public class ClearClaimsCheck : Cms.Claims.ClaimBase
	{
		...
		private void Page_Load(object sender, System.EventArgs e)
		{
            ...
            objResourceMgr = new ResourceManager("Claims.Aspx.ClearClaimsCheck", Assembly.GetExecutingAssembly());
//			base.ClearClaimsSessionValues();

			//Set Security XML
			strSecurity = gstrSecurityXML;
			if (Request.Form["__EVENTTARGET"] == "ClearChecks" && hidCheckedRowIDs.Value!="")
			{   
				hidCheckedRowIDs.Value = hidCheckedRowIDs.Value.Replace("~",",");
				hidCheckedRowIDs.Value = hidCheckedRowIDs.Value.Replace(" ","");
				ClsClaims.ClaimsCheckOperation(hidCheckedRowIDs.Value,"CLEAR");
			...
			try
			{
				...
				//objWebGrid.DisplayTextLength     = "16^16^10^10^16^40";
				objWebGrid.DisplayTextLength = "16^16^10^10^16^16^50";
				//objWebGrid.DisplayColumnPercent  = "16^16^10^10^16^40";
				objWebGrid.DisplayColumnPercent = "16^10^10^16^16^16^50";

                objWebGrid.HeaderString = objResourceMgr.GetString("HeaderString"); //"Clear Claims Checks";
                ...
                if (ClsCommon.CheckSecurity(strSecurity))
                    objWebGrid.ExtraButtons = objResourceMgr.GetString("ExtraButtons"); //"1^Clear Check^0^ClearClaimCheck";											
