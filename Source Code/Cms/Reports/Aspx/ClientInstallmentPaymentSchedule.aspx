<%@ Page language="c#" Codebehind="ClientInstallmentPaymentSchedule.aspx.cs" AutoEventWireup="false" Inherits="Reports.Aspx.ClientInstallmentPaymentSchedule" %>
...
<HTML>
	<HEAD>
		<title>Client Installment Payment Schedule Report</title>
		...
		{			
			...
			//Commented For Itrack Issue #6806
			/*if(Policy == "-1")
			{
				Policy = "0";
			}*/
			...
			if (Customer!= ''&& Policy!= '')
			{			
				var url="CustomReport.aspx?PageName=ClientInstallment&Customerid="+ Customer + "&Policyid="+ Policy + "&Agencyid="+ Agency; 
				//alert(url);
				var windowobj = window.open(url,'ClientInstallment','resizable=yes,scrollbar=yes,top=100,left=50')
				windowobj.focus();
			}
		}		
	      ...
	      <DIV><webcontrol:menu id="bottomMenu" runat="server"></webcontrol:menu></DIV>		
			...
			<!-- To add bottom menu ends here --><asp:panel id="Panel1" Runat="server">
				<TABLE class="tableWidth" cellSpacing="1" cellPadding="0" width="100%" align="center" border="0">
					...
					<TR>
						<TD class="headereffectCenter" colSpan="4">Customer Installment Payment Schedule Report Selection Criteria
						</TD>
					</TR>
					<TR>
