...
/****** Object:  StoredProcedure [dbo].[Proc_ProcessUploadedClaimData]    Script Date: 02/26/2015 14:37:20 ******/
IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[Proc_ProcessUploadedClaimData]') AND type in (N'P', N'PC'))
DROP PROCEDURE [dbo].[Proc_ProcessUploadedClaimData]
GO
...
GO
--begin tran
--drop proc [Proc_ProcessUploadedClaimData]
--GO

/*************************
Created by: Pravesh K Chandel
Created Date: 2 March 2015
Purpose	: Process uploaded CSV Claim Files Data which has been processed
drop proc [Proc_ProcessUploadedClaimData]
**************************/
...
begin try
	...
	begin
		...
		update MNT_FileUpload set FailedRecords = @FailedCount,SuccessRecords=@SuccessCount, LastModifiedDateTime=GETDATE(),ModifiedBy=@User, Status ='Failed', HasError='Y',ErrorMessage = 'Claim file missmatch from Standard Code file.'   where FileRefNo =@FileRefNo --and UploadType='CLM'
...
--------update/insert lookup value from Standard code file for operating hours
	...
	if exists( select CLM.IpNumber
		...
		begin
			...
			select @FailedCount = COUNT( case when HasError='Y' then 1 else null end ),@SuccessCount = COUNT( case when isnull(HasError,'')<>'Y' then 1 else null end ) 
					from STG_CLAIM_FILE_DATA CLM (nolock) where CLM.FileRefNo =@FileRefNo and CLM.HistoryIds=@HistoryId_CLAIM
			update MNT_FileUpload set FailedRecords = @FailedCount,SuccessRecords=@SuccessCount, Status ='Success', HasError='N'  where FileRefNo =@FileRefNo --and UploadType='CLM'
	end	
	-- select 'pass'
end try
begin catch
