IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[Proc_GetServiceProverList]') AND type in (N'P', N'PC'))
DROP PROCEDURE [dbo].[Proc_GetServiceProverList]
GO

CREATE Proc [dbo].[Proc_GetServiceProverList]
(
...
SELECT * INTO #TMP1 FROM (  select row_number() over(partition by ClaimTypeId order by ServiceProviderId) RecordNumber,a.ClaimRecordNo ,a.ClaimantName,
 ...
 SELECT *,lk.Description AS ClaimTypeDesc,lk.Lookupdesc AS ClaimTypeCode FROM #TMP1 ServiceProvider (NOLOCK) LEFT JOIN MNT_Lookups lk (NOLOCK) ON lk.lookupvalue = ServiceProvider.ClaimTypeId AND lk.Category = 'ClaimType' WHERE AccidentClaimId = @AccidentId ORDER BY ClaimTypeId
