IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[Proc_GetTransactionAuditLog]') AND type in (N'P', N'PC'))
DROP PROCEDURE [dbo].[Proc_GetTransactionAuditLog]
GO

CREATE PROC [dbo].[Proc_GetTransactionAuditLog] (@AccidentId int)
AS
  CREATE TABLE #Temp (
