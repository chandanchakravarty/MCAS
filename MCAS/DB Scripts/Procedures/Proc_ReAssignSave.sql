...
CREATE PROCEDURE [dbo].[Proc_ReAssignSave] @LISTID int = 0, @ReAssignTo int = 0,@ReAssignFrom int = 0
AS
  ...
  UPDATE #mytemptable  SET TOUSERID = @ReAssignTo,FROMUSERID = @ReAssignFrom, ModifiedDate=GETDATE(),ParentId=@LISTID,MovementType='S'  WHERE LISTID = @LISTID
